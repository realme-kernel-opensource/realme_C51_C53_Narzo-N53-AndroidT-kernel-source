...
static u64 android_oem_data1;

#define PGDAT_ITEM_DATA(pgdat) ((struct hybridswapd_task*)android_oem_data1)
...
static struct pglist_data *next_online_pgdat_local(struct pglist_data *pgdat)
{
...
struct zone *next_zone_local(struct zone *zone)
{
	...
	if (zone < pgdat->node_zones + MAX_NR_ZONES - 1)
		...
		pgdat = next_online_pgdat_local(pgdat);
		if (pgdat)
...
#define for_each_zone_local(zone)			        \
	for (zone = NODE_DATA(first_online_node)->node_zones; \
	     zone;					\
	     zone = next_zone_local(zone))
...
static int create_swapd_thread(struct zram *zram)
{
	...
	for_each_node(nid) {
		...
		if (!PGDAT_ITEM_DATA(pgdat)) {
			...
			android_oem_data1 = (u64)tsk_info;
		}
