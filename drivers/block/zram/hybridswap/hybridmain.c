...
mem_cg_oem_data_t hyb_oem_data[MAX_OEM_DATA_CNT] = {0};
...
memcg_hybs_t *hybridswap_cache_alloc(struct mem_cgroup *memcg, bool atomic)
{
	...
	if (hyb_oem_data[memcg->id.id].android_oem_data1)
		BUG();
	...
	ret = atomic64_cmpxchg((atomic64_t *)&hyb_oem_data[memcg->id.id].android_oem_data1, 0, (u64)hybs);
	if (ret != 0) {
...
static void mem_cgroup_free_hook(void *data, struct mem_cgroup *memcg)
{
	...
	hybs = (memcg_hybs_t *)hyb_oem_data[memcg->id.id].android_oem_data1;
	hyb_oem_data[memcg->id.id].android_oem_data1 = 0;
	put_memcg_cache(hybs);
}
