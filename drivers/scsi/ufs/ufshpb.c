...
struct ufshpb_dev_info *ufshcd_get_dev_info(struct ufs_hba *hba)
{
	return (struct ufshpb_dev_info *)(hba->android_kabi_reserved1);
}

static void ufshpb_update_active_info(struct ufshpb_lu *hpb, int rgn_idx,
				      int srgn_idx);
bool *ufshpb_hpb_enabled(struct ufs_hba *hba)
{
	return &(ufshcd_get_dev_info(hba)->hpb_enabled);
}

bool ufshpb_is_hpb_enabled(struct ufs_hba *hba)
{
	return (ufshcd_get_dev_info(hba)->hpb_enabled);
}
...
{
	return !(ufshcd_get_dev_info(hba)->hpb_disabled);
}

/* HPB version 1.0 is called as legacy version. */
bool ufshpb_is_legacy(struct ufs_hba *hba)
{
	return ufshcd_get_dev_info(hba)->is_legacy;
}
					...
					__be64 ppn, u8 transfer_len)
{
	...
	__be64 ppn_tmp = ppn;
	unsigned int ufshpb_follow_read16 = 0;
#if  defined(CONFIG_SCSI_UFS_HPB) && defined(CONFIG_SPRD_DEBUG)
	ufshpb_follow_read16 = ufshpb_follow;
#endif
	if (hba->dev_quirks & UFS_DEVICE_QUIRK_DISABLE_HARDWARE_CRYPTO_WHEN_SUPPORT_HPB_READ) {
		ufshpb_follow_read16 = (0x1<<0);
	}

	if (!(hba->dev_quirks & UFS_DEVICE_QUIRK_CMD_READ16_REPLACE_HPB_READ
				&& lrbp->crypto_key_slot >= 0) || (ufshpb_follow_read16&(0x1<<0))) {
		cdb[0] = UFSHPB_READ;
	...
	if (hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG ||
			(ufshpb_follow_read16&(0x1<<1))) {
		unsigned int transfer_len_new;
		unsigned char cmd[HPB_READ16_CMDLEN] = {0};


		transfer_len_new = SHIFT_BYTE_1(lrbp->cmd->cmnd[HPB_READ16_TRANSLEN7]) |
		...
		if (hba->dev_info.wmanufacturerid == UFS_VENDOR_TOSHIBA) {
			cdb[HPB_READ16_CONTROL] = HPB_READ16_CONTROL_VALUE;
		}
		if (hba->dev_info.wmanufacturerid == UFS_VENDOR_SKHYNIX) {
			cdb[HPB_READ16_TRANS_LEN] = transfer_len & ONE_BYTE_MASK;
			if (ufshcd_get_dev_info(hba)->hpb_version < 0x0200) {
				cdb[1] = 0x0;
				cdb[15] = 0x01; //NAND V5,V6 : Use Control field for HPB_READ
			} else {
				cdb[1] = 0x5; // NAND V7 : Use reseved fields for HPB_READ
				cdb[15] = 0x0;
			}
		}
	} else if (hba->dev_info.wmanufacturerid == UFS_VENDOR_YMTC ||
			(ufshpb_follow_read16&(0x1<<3))) {
		cdb[0] = READ_16;
		cdb[14] = 0x11;
		cdb[15] = transfer_len;
		...
		memcpy(&cdb[6], &ppn_tmp, sizeof(__be64));
		lrbp->cmd->cmd_len = MAX_CDB_SIZE;
	} else if (ufshpb_follow_read16&(0x1<<4)) {
		cdb[0] = READ_16;
		cdb[14] = 0x11;
		cdb[15] = transfer_len;
		...
		memcpy(&cdb[6], &ppn_tmp, sizeof(__be64));
		lrbp->cmd->cmd_len = MAX_CDB_SIZE;
	} else if (ufshpb_follow_read16&(0x1<<5)) {
		cdb[0] = READ_16;
		cdb[14] = 0x11;
		cdb[15] = transfer_len;
...
int ufshpb_prep(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	int err = 0;
#if  defined(CONFIG_SCSI_UFS_HPB) && defined(CONFIG_SPRD_DEBUG)
	if (ufshpb_prep_disable_ufshpb)
		return -ENODEV;
#endif
	if (hba->dev_quirks & UFS_DEVICE_QUIRK_NO_SUPPORT_HPB_READ_WHEN_HARDWARE_CRYPTO_ENABLE) {
		return -ENODEV;
	}

	if (!(hba->dev_quirks & UFS_DEVICE_QUIRK_CMD_READ16_REPLACE_HPB_READ) &&
		!(hba->dev_quirks & UFS_DEVICE_QUIRK_DISABLE_HARDWARE_CRYPTO_WHEN_SUPPORT_HPB_READ))
		return -ENODEV;
...
{
	struct victim_select_info *lru_info = &hpb->lru_info;
	struct ufshpb_region *rgn = NULL, *victim_rgn = NULL;
...
void ufshpb_rsp_upiu(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	struct utp_hpb_rsp *rsp_field = (struct utp_hpb_rsp *)(&(lrbp->ucd_rsp_ptr->sr));
	int data_seg_len;
				      ...
				      struct ufshpb_region *rgn, bool last)
{
	...
	if (unlikely(last && hpb->last_srgn_entries && srgn != NULL))
		srgn->is_last = true;
}
			      ...
			      struct ufshpb_lu_info *hpb_lu_info)
{
	...
	if (lu_enable != LU_ENABLED_HPB_FUNC) {
		/*Compitable to some Micron chip, to retry by selector = 1*/
		pm_runtime_get_sync(hba->dev);
		ret = ufshcd_query_descriptor_retry(hba,
						UPIU_QUERY_OPCODE_READ_DESC,
					    QUERY_DESC_IDN_UNIT, lun, 1,
					    desc_buf, &size);
		pm_runtime_put_sync(hba->dev);
...
void ufshpb_init_hpb_lu(struct ufs_hba *hba, struct scsi_device *sdev)
{
	...
	hpb = ufshpb_alloc_hpb_lu(hba, sdev, ufshcd_get_dev_info(hba),
				  &hpb_lu_info);
	if (!hpb)
		goto out;
...
out:
	...
	if (atomic_dec_and_test(&(ufshcd_get_dev_info(hba)->slave_conf_cnt)))
...
{
	struct ufshpb_dev_info *hpb_info = ufshcd_get_dev_info(hba);
...
{
	struct ufshpb_dev_info *hpb_dev_info = ufshcd_get_dev_info(hba);
	int version, ret;
	int max_single_cmd;

	hpb_dev_info->control_mode = desc_buf[DEVICE_DESC_PARAM_HPB_CONTROL];
	version = get_unaligned_be16(desc_buf + DEVICE_DESC_PARAM_HPB_VER);
	hpb_dev_info->hpb_version = (u16)version;
	if ((version != HPB_SUPPORT_VERSION) &&
	    ((version & HPB_SUPPORT_LEGACY_VERSION)
			!= HPB_SUPPORT_LEGACY_VERSION)) {
...
void ufshpb_init(struct ufs_hba *hba)
{
	...
	if (!ufshpb_is_allowed(hba) || !hpb_dev_info->hpb_enabled)
		return;
	...
	if (hba->dev_quirks & UFS_DEVICE_QUIRK_CMD_READ16_REPLACE_HPB_READ) {
		pr_err("UFS_DEVICE_QUIRK_CMD_READ16_REPLACE_HPB_READ\n");
	}
	pr_err("hba->dev_info.wmanufacturerid:%x\n", hba->dev_info.wmanufacturerid);
	pr_err("ufshcd_get_dev_info(hba)->hpb_version:%x",
			ufshcd_get_dev_info(hba)->hpb_version);
}
...
#if  defined(CONFIG_SCSI_UFS_HPB) && defined(CONFIG_SPRD_DEBUG)
void ufshcd_hpb_complete_command_trace(u8 opcode, u32 tag)
{
	if (opcode == UFSHPB_READ) {
		global_ufs_cmd_trace_single_0xf8_count++;
		global_ufs_cmd_trace_single_0xf8_all_start_to_end_us +=
			ktime_to_us(ktime_sub(ktime_get(), global_ufs_cmd_trace_single[tag].start));
		if (global_ufs_cmd_trace_single_0xf8_all_start_to_end_us&((u64)1<<63))
			pr_err("UFSHPB_READ too more");
		...
		global_ufs_cmd_trace_single_0x28_all_start_to_end_us +=
			ktime_to_us(ktime_sub(ktime_get(), global_ufs_cmd_trace_single[tag].start));
		if (global_ufs_cmd_trace_single_0x28_all_start_to_end_us&((u64)1<<63))
			pr_err("READ_10 too more");
		...
		global_ufs_cmd_trace_single_0x88_all_start_to_end_us +=
			ktime_to_us(ktime_sub(ktime_get(), global_ufs_cmd_trace_single[tag].start));
		if (global_ufs_cmd_trace_single_0x88_all_start_to_end_us&((u64)1<<63))
			pr_err("READ_16 too more");
		global_ufs_cmd_trace_single_0x88_ave_start_to_end_us =
			global_ufs_cmd_trace_single_0x88_all_start_to_end_us /
			global_ufs_cmd_trace_single_0x88_count;
	}
}
void ufshcd_hpb_add_command_trace(struct ufs_hba *hba,
		unsigned int tag, const char *str)
{
	...
	u64 op_cmnd = 0;
	u64 len_raw = 0;
	u8 cmd_fa_buf_id = 0, read_id = 0;
	struct ufshcd_lrb *lrbp;
	int transfer_len = -1;
	u64 now_ufs;
	u64 region_index = 0, subregion_index = 0;//for 0xf9
	struct utp_transfer_req_desc *req_desc;
	u32 dword_0;
	u8 crypto = 0;

	lrbp = &hba->lrb[tag];
	//pr_err("ufshcd_hpb_add_command_trace1 opcode:%x\n", opcode);
	if (lrbp->cmd) { /* data phase exists */
		//kui crypto
		req_desc = lrbp->utr_descriptor_ptr;
		dword_0 = le32_to_cpu(req_desc->header.dword_0);
		if (dword_0 & UTP_REQ_DESC_CRYPTO_ENABLE_CMD)
			crypto = 1;

		opcode = (u8)(*lrbp->cmd->cmnd);
		if (opcode == UFSHPB_WRITE_BUFFER) {
		//	pr_err("hba=%lx,tag=%lx,cmd=%lx\n",hba,tag,lrbp->cmd);
		//	dump_stack();
		//	panic("0xfa");
		}
		now_ufs = ktime_to_us(ktime_get());
		if ((opcode > (RECORD_CMD_NO_ADD_TO_TRACE_ALL-1)))
			return;
		if (!strcmp(str, "send"))
			record_cmd[opcode]++;
		transfer_len = be32_to_cpu(lrbp->ucd_req_ptr->sc.exp_data_transfer_len);
		if (((opcode == READ_10) || (opcode == UFSHPB_READ) || (opcode == READ_16)) && transfer_len == 0x1000) {
			if (!strcmp(str, "send")) {
				global_ufs_cmd_trace_single[tag].start = ktime_get();
			} else if (!strcmp(str, "complete")) {
				ufshcd_hpb_complete_command_trace(opcode, tag);
			}
		}
		if (((record_cmd[opcode])&0xffff) > RECORD_CMD_NO_ADD_TO_TRACE) {
			if ((opcode == WRITE_10)  || (opcode == READ_10)  ||
			(opcode == READ_16) || (opcode == UFSHPB_READ) ||
			(opcode == UFSHPB_READ_BUFFER) || (opcode == UFSHPB_WRITE_BUFFER))
				return;
		}
		...
		if ((opcode == WRITE_10)  || (opcode == READ_10)  ||
			...
			if (lrbp->cmd->request && lrbp->cmd->request->bio)
				lba =  lrbp->cmd->request->bio->bi_iter.bi_sector/2/4;
			transfer_len = be32_to_cpu(lrbp->ucd_req_ptr->sc.exp_data_transfer_len);

			op_cmnd = ((record_cmd[opcode]>>16)<<24)  |
				  ((u8)(((u8 *)lrbp->cmd->cmnd)[0]) << 16) |
				  (crypto<<8);
			start_blk = be32_to_cpu(*((u32 *)((u8 *)lrbp->cmd->cmnd+2)));
			if (opcode == UFSHPB_READ_BUFFER) {
				len_raw = (((u8)(((u8 *)lrbp->cmd->cmnd)[6]))<<16) |
					  (((u8)(((u8 *)lrbp->cmd->cmnd)[7]))<<8) |
					  (((u8)(((u8 *)lrbp->cmd->cmnd)[8])));
				//less or equal to the value of (HPB Sub-Region size /
				//Logical Block Size (4 KB)) x HPBEntry size (=8 Bytes).
				//len/4096*8 = len_raw
				//len(byte) = len_raw/8*4096;
				len_raw = len_raw/8*4096/4096;
				region_index = be16_to_cpu((*((u16 *)((u8 *)lrbp->cmd->cmnd+2))));
				subregion_index = be16_to_cpu((*((u16 *)((u8 *)lrbp->cmd->cmnd+4))));
				start_blk = (region_index * (512 * 512 * 1024) +
						subregion_index *  (512 * 512 * 1024)) / (4*1024);
				//subregion_size == region_size == 0x13

			} else if (opcode == UFSHPB_WRITE_BUFFER) {
				cmd_fa_buf_id = ((u8)(((u8 *)lrbp->cmd->cmnd)[1]));
				op_cmnd &= ~0xff;
				op_cmnd |= (u8)cmd_fa_buf_id;
				read_id = (u8)(((u8 *)lrbp->cmd->cmnd)[6]);


				if (cmd_fa_buf_id == 2) {
					len_raw = (be16_to_cpu(((u16)*((u16 *)(((u8 *)lrbp->cmd->cmnd)+7)))));//bytes/8= entrys
				} else if (cmd_fa_buf_id == 1) {
					region_index = be16_to_cpu((*((u16 *)((u8 *)lrbp->cmd->cmnd+2))));
					start_blk = (region_index * (512 * 512 * 1024))/(4*1024);
				}

			} else if (opcode == UFSHPB_READ) {
				read_id = (u8)(((u8 *)lrbp->cmd->cmnd)[15]);
				len_raw = ((u8)(((u8 *)lrbp->cmd->cmnd)[14]));
			} else if (opcode == READ_16) {
				start_blk = be64_to_cpu(*((u64 *)((u8 *)lrbp->cmd->cmnd+2)));
				len_raw = ((u8)(((u8 *)lrbp->cmd->cmnd)[15]));

			} else if (opcode == READ_10) {
				len_raw = be16_to_cpu((u16)*((u16 *)(((u8 *)lrbp->cmd->cmnd)+7)));
			}
			//pr_emerg
		       pr_err(
			    "%s: tag: %u, LBA: 0x%016llx,region_index:0x%llx, subregion_index:0x%llx,"
			    "op_cmnd_all_cnt_0xffff : %d,op_cmnd : 0x%x ,crypto:0x%x."
			    "cmd_fa_buf_id:0x%x ,  read_id : 0x%x ,start_blk:0x%016llx ,len:0x%llx\n",
				str,	tag, lba, region_index, subregion_index,
				(u32)(op_cmnd>>24), (u32)((op_cmnd>>16)&0xff), (u32)((op_cmnd>>8)&0xff),
				(u32)((op_cmnd)&0xff), read_id, start_blk, len_raw);
		       //pr_err("%s, ufshpb_follow:%x,hba->dev_info.wmanufacturerid %x",
		       //__func__, ufshpb_follow, hba->dev_info.wmanufacturerid);
		}
	}
}
#endif
