...
static char firmware_version[20] = {'H','U','L','K','_','L','D','_','M','D','T','_','F','T','0','4'};
/*****************************************************************************
...
void  get_tp_fw_ver(void)
{
    ...
    if(fwver)
    {
      snprintf(buf, sizeof(buf), "%02x", fwver);
      	firmware_version[16] = buf[0];
        firmware_version[17] = buf[1];
      get_hardware_info_data(HWID_CTP_FW_INFO,firmware_version);
    get_hardware_info_data(HWID_CTP_DRIVER, "FT_8057");
    }
}
