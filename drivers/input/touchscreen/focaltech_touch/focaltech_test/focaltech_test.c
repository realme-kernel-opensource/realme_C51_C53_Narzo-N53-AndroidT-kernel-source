...
int fts_self_test(struct seq_file *s, void *v)
{
	...
	if (black_test == 0){
        touch_black_test = 1;
	   fts_test_entry(FT_SELF_TEST_INI);
    }else{
		touch_black_test = 1;	
	   fts_test_entry(FT_SELF_BLACK_TEST_INI);
        touch_black_test = 0;
	}}
	udelay(200);
    ret = fts_enter_test_environment(0);
...
int fts_rawdata_differ(u8 raw_diff, char *buf)
{
	...
	get_tx_rx_num(FACTORY_REG_CHX_NUM, &tx_num, 60);
	get_tx_rx_num(FACTORY_REG_CHY_NUM, &rx_num, 100);
