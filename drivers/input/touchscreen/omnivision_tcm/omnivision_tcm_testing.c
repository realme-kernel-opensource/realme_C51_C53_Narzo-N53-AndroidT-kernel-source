...
int tp_gesture = 0;
static bool wakeup_gesture_enabled_bak = false;
EXPORT_SYMBOL(tp_gesture);
int touch_black_test = 0;
EXPORT_SYMBOL(touch_black_test);
struct test_proc_operations {
	void (*baseline_test)(struct seq_file *s);
	void (*black_screen_test)(struct seq_file *s);
	int tp_gesture_val;
...
static int testing_do_testing(void)
{
	...
	if (error_count) {
		//test fail result
		sprintf(file_path, "/sdcard/TpTestReport/tp_%s_test_data_%02d%02d%02d-%02d%02d%02d-fail.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	} else {
		//test pass result
		sprintf(file_path, "/sdcard/TpTestReport/tp_%s_test_data_%02d%02d%02d-%02d%02d%02d-success.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	}
...
static int testing_do_black_testing(void)
{
	int retval;
    mm_segment_t old_fs;
    int error_count = 0;
	...
	if (error_count) {
		//test fail result
		sprintf(file_path, "/sdcard/TpTestReport/tp_%s_black_test_data_%02d%02d%02d-%02d%02d%02d-fail.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	} else {
		//test pass result
		sprintf(file_path, "/sdcard/TpTestReport/tp_%s_black_test_data_%02d%02d%02d-%02d%02d%02d-success.csv", tcm_hcd->id_info.part_number,
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	}
...
//oppo test proc start
static void baesline_test_fun(struct seq_file *s);
static void black_screen_test_fun(struct seq_file *s);
static struct test_proc_operations td4160_test_proc_ops = {
	.baseline_test = baesline_test_fun,
	.black_screen_test = black_screen_test_fun,
	.tp_gesture_val = 0,
	.firmware_version = "HULK_CW_B8_TD"
};

static int baseline_test_function(void)
{
	int retval = 0;
	struct ovt_tcm_hcd *tcm_hcd = testing_hcd->tcm_hcd;
#ifndef OMNIVISION_FOR_GKI
	retval = testing_do_testing();
	printk("testing_do_testing: %d\n", retval);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	if (!testing_hcd->result) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to run testing_pt07_dynamic_range\n");
		retval = -EINVAL;
	}
	...
	if (!testing_hcd->result) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to run testing_pt10_noise\n");
		retval = -EINVAL;
	}
	...
	if (!testing_hcd->result) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to run testing_pt11_open_detection\n");
		retval = -EINVAL;
	}
#endif
	return retval;
...
static int black_screen_test_function(void)
{
	int retval = 0;
	struct ovt_tcm_hcd *tcm_hcd = testing_hcd->tcm_hcd;

#ifndef OMNIVISION_FOR_GKI
	retval = testing_do_black_testing();
	printk("testing_do_black_testing: %d\n", retval);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"black screen test failed.\n");
	}
#else
	testing_pt07_dynamic_range();
	if (!testing_hcd->result) {
...
static int tp_testing_do_testing_func(struct seq_file *s, void *v)
{
	struct test_proc_operations *ops = s->private;

    if (!ops)
        return 0;

    if (!ops->baseline_test) {
        seq_printf(s, "Not support baseline test proc node\n");
        return 0;
    }
    ops->baseline_test(s);
...
static int tp_testing_do_testing_black_func(struct seq_file *s, void *v)
{
	struct test_proc_operations *ops = s->private;
    ...
    if (!ops->baseline_test) {
        seq_printf(s, "Not support black screen test proc node\n");
        return 0;
    }
    ops->black_screen_test(s);

    return 0;
}
//proc/touchpanel/gesture
...
static void baesline_test_fun(struct seq_file *s)
{
	...
	snprintf(str, 3,
		"%02x", testing_hcd->tcm_hcd->app_info.customer_config_id[15]);
	td4160_test_proc_ops.firmware_version[13] = '0';
	if (g_firmware_version == 0) {
		td4160_test_proc_ops.firmware_version[14] = '1';
	} else if (g_firmware_version == 1) {
		td4160_test_proc_ops.firmware_version[14] = '2';
	} else if (g_firmware_version == 3) {
		td4160_test_proc_ops.firmware_version[14] = '6';
	} else {
		td4160_test_proc_ops.firmware_version[14] = '0';
	}
	if (testing_hcd->tcm_hcd->app_info.customer_config_id[15] < 16) {
		td4160_test_proc_ops.firmware_version[15] = '0';
	} else {
		td4160_test_proc_ops.firmware_version[15] = str[0];
	}
	td4160_test_proc_ops.firmware_version[16] = str[1];
	seq_printf(s, "firmware_version:%s\n", td4160_test_proc_ops.firmware_version);

	ret = baseline_test_function();
	if (ret < 0){
		seq_printf(s, "baesline_test fail\n");
	}
	else{
		seq_printf(s, "pass\n");
	}
...
static void black_screen_test_fun(struct seq_file *s)
{
	...
	seq_printf(s, "firmware_version:%s\n", td4160_test_proc_ops.firmware_version);
	msleep(500);
	ret = black_screen_test_function();
	if (ret < 0){
		seq_printf(s, "black_screen_test fail\n");
	}
	else{
		seq_printf(s, "pass\n");
	}
	if (!wakeup_gesture_enabled_bak) {
		testing_hcd->tcm_hcd->wakeup_gesture_enabled = false;
	}
	touch_black_test = 0;
}

static int baseline_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, tp_testing_do_testing_func, PDE_DATA(inode));
}
static int black_screen_test_open(struct inode *inode, struct file *file)
{
    return single_open(file, tp_testing_do_testing_black_func, PDE_DATA(inode));
}
static int tp_gesture_open(struct inode *inode, struct file *file)
{
    return single_open(file, tp_gesture_read_func, PDE_DATA(inode));
}
static ssize_t black_test_write(struct file *file, const char *buff,
									size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0') {
		...
		touch_black_test = 1;
		wakeup_gesture_enabled_bak = testing_hcd->tcm_hcd->wakeup_gesture_enabled;
		testing_hcd->tcm_hcd->wakeup_gesture_enabled = true;
		retval = tcm_hcd->set_dynamic_config(tcm_hcd,
		DC_IN_WAKEUP_GESTURE_MODE,
		1);
		printk("omnivision retval:%d.\n", retval);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
...
static ssize_t tp_gesture_write(struct file *file, const char *buff,
									size_t len, loff_t *pos)
{
	char buf[80] = {0};
	...
	if (buf[0] == '0') {
		tp_gesture = 0;
		td4160_test_proc_ops.tp_gesture_val = 0;
		testing_hcd->tcm_hcd->wakeup_gesture_enabled = false;
		printk("omnivision tp gesture write 0.\n");
	}
	else if (buf[0] == '1') {
		tp_gesture = 1;
		td4160_test_proc_ops.tp_gesture_val = 1;
		testing_hcd->tcm_hcd->wakeup_gesture_enabled = true;
		printk("omnivision tp gesture write 1.\n");
	}
	return len;
}
...
static const struct file_operations tp_black_screen_test_proc_fops = {
    .owner = THIS_MODULE,
    .open = black_screen_test_open,
    .read = seq_read,
	.write = black_test_write,
    .release = single_release,
};
static const struct file_operations tp_gesture_proc_fops = {
    .owner = THIS_MODULE,
    .open = tp_gesture_open,
    .read = seq_read,
	.write = tp_gesture_write,
    .release = single_release,
};

static int init_touchpanel_proc(struct test_proc_operations *ops)
{
	int ret = 0;
	struct ovt_tcm_hcd *tcm_hcd = testing_hcd->tcm_hcd;
    struct proc_dir_entry *prEntry_touchpanel = NULL;
	struct proc_dir_entry *prEntry_tmp = NULL;

	prEntry_touchpanel = proc_mkdir("touchpanel", NULL);
    if (prEntry_touchpanel == NULL) {
        ret = -ENOMEM;
		LOGI(tcm_hcd->pdev->dev.parent,
				"Couldn't create touchpanel proc entry\n");
    }

	// show baseline test interface
    prEntry_tmp = proc_create_data("baseline_test", 0666, prEntry_touchpanel, &tp_baseline_test_proc_fops, ops);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
		LOGI(tcm_hcd->pdev->dev.parent,
				"Couldn't create baseline_test proc entry\n");
    }

	// show black screen test interface
    prEntry_tmp = proc_create_data("black_screen_test", 0666, prEntry_touchpanel, &tp_black_screen_test_proc_fops, ops);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
		LOGI(tcm_hcd->pdev->dev.parent,
				"Couldn't create black_screen_test proc entry\n");
    }

	// show tp interface
    prEntry_tmp = proc_create_data("double_tap_enable", 0666, prEntry_touchpanel, &tp_gesture_proc_fops, ops);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
		LOGI(tcm_hcd->pdev->dev.parent,
				"Couldn't create double_tap_enable proc entry\n");
    }
...
static int testing_init(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	retval = init_touchpanel_proc(&td4160_test_proc_ops);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to init touchpanel proc for testing_hcd\n");
		goto err_sysfs_create_file;
	}
