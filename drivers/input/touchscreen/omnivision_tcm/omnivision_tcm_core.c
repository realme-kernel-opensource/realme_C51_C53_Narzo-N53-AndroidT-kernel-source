...
SHOW_STORE_PROTOTYPE(ovt_tcm, sensing_freq_switch)
...
static char firmware_version[18] = {'H','U','L','K','_','C','W','_','B','8','_','T','D'};
int g_firmware_version = 0; //0old,1new,2limu,3td4160B
EXPORT_SYMBOL(g_firmware_version);
static char lcd_name[100];
#define LCD_NAME_td4160b "lcd_td4160b_cw_boe_mipi_hd"
#define LCD_NAME_NEW "lcd_td4160_cw_new_mipi_hd"
#define LCD_NAME_OLD "lcd_td4160_cw_old_mipi_hd"
#define LCD_NAME_LIMU "lcd_td4160_cw_mipi_hd"

int g_usb_charger_connect_state = 0;
int g_earphone_connect_state = 0;
...
static void ovt_get_tp_fw_ver(struct ovt_tcm_hcd *tcm_hcd)
{
	struct ovt_tcm_hcd *tcm_hcd_tmp = tcm_hcd;
	char str[3] = {0};

	get_hardware_info_data(HWID_CTP_DRIVER, "omnivision_tcm");
	printk("driver_name:omnivision_tcm\n");

	snprintf(str, 3,
		"%02x", tcm_hcd_tmp->app_info.customer_config_id[15]);
	firmware_version[13] = '0';
	if (g_firmware_version == 0) {
		firmware_version[14] = '1';
	} else if (g_firmware_version == 1) {
		firmware_version[14] = '2';
	} else if (g_firmware_version == 3) {
		firmware_version[14] = '6';
	} else {
		firmware_version[14] = '0';
	}
	if (tcm_hcd_tmp->app_info.customer_config_id[15] < 16) {
		firmware_version[15] = '0';
	} else {
		firmware_version[15] = str[0];
	}
	firmware_version[16] = str[1];

	get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version);
	printk("firmware_version:%s\n",firmware_version);
}
...
dynamic_config_sysfs(roate_horizontal_level, DC_ENABLE_ROATE_HORIZONTAL_LEVEL)

dynamic_config_sysfs(ear_phone_connected, DC_ENABLE_EAR_PHONE)

dynamic_config_sysfs(sensing_freq_switch, DC_SENSING_FREQ)
...
static int ovt_tcm_enable_irq(struct ovt_tcm_hcd *tcm_hcd, bool en, bool ns)
{
	...
	if (en) {
		...
		if (irq_freed) {
			...
			LOGN(tcm_hcd->pdev->dev.parent, "level trigger low\n");
			retval = request_threaded_irq(tcm_hcd->irq, NULL,
					ovt_tcm_isr, IRQF_TRIGGER_LOW | IRQF_ONESHOT,
					PLATFORM_DRIVER_NAME, tcm_hcd);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
...
int ovt_tcm_usb_set_charger(struct ovt_tcm_hcd *tcm_hcd, bool is_connect)
{
	int retval = 0;
	if ((!tcm_hcd) || (tcm_hcd->ovt_tcm_driver_removing)) {
		printk("tcm shutdown, do not set charger\n");
		return 0;
	}
	...
	if (IS_NOT_FW_MODE(tcm_hcd->id_info.mode) || tcm_hcd->in_suspend || atomic_read(&tcm_hcd->host_downloading)) {
		...
		if (retval != 0) {
			...
			LOGN(tcm_hcd->pdev->dev.parent,"success to set charger connect command\n");
		}
	}
...
int ovt_tcm_set_earphone_mode(struct ovt_tcm_hcd *tcm_hcd, bool is_connect)
{
	int retval = 0;
	if ((!tcm_hcd) || (tcm_hcd->ovt_tcm_driver_removing)) {
		printk("tcm shutdown, do not set earphone mode\n");
		return 0;
	}
	...
	if (IS_NOT_FW_MODE(tcm_hcd->id_info.mode) || tcm_hcd->in_suspend || atomic_read(&tcm_hcd->host_downloading)) {
		...
		retval = ovt_tcm_set_dynamic_config(tcm_hcd, DC_ENABLE_EAR_PHONE, is_connect ? 1 : 0);
		if (retval != 0) {
			LOGE(tcm_hcd->pdev->dev.parent,"Failed to set earphone connect command\n");
		} else {
			LOGN(tcm_hcd->pdev->dev.parent,"success to set earphone connect command\n");
		}
	}
	g_earphone_connect_state = is_connect ? 1 : 0;
	return retval;
}
...
void switch_usb_state(int usb_state)
{
	...
	tcm_hcd = g_tcm_hcd;
	if (usb_state == 0) {
		ovt_tcm_usb_set_charger(tcm_hcd, false);
	} else if (usb_state == 1) {
...
void switch_headset_state(int headset_state)
{
	...
	tcm_hcd = g_tcm_hcd;
	if (headset_state == 0) {
		ovt_tcm_set_earphone_mode(tcm_hcd, false);
	} else if (headset_state == 1) {
		ovt_tcm_set_earphone_mode(tcm_hcd, true);
	}
}
...
static void ovt_tcm_reinit_func_when_hdl_done(void)
{
	...
	if (g_usb_charger_connect_state) {
		...
		retval = ovt_tcm_set_dynamic_config(tcm_hcd, DC_CHARGER_CONNECTED, 1);
		if (retval != 0) {
			LOGE(tcm_hcd->pdev->dev.parent,"Failed to set charger connect command\n");
		}
	}
	// if (tcm_hcd->func_face_detect_en) {
	// 	retval = ovt_tcm_set_dynamic_config(tcm_hcd, DC_ENABLE_FACE, 1);
	// 	if (retval != 0) {
	// 		LOGE(tcm_hcd->pdev->dev.parent,"Failed to set face detect enable command\n");
	// 	}
	// }
	if (g_earphone_connect_state) {
		retval = ovt_tcm_set_dynamic_config(tcm_hcd, DC_ENABLE_EAR_PHONE, 1);
		if (retval != 0) {
			LOGE(tcm_hcd->pdev->dev.parent,"Failed to set ear phone connected command\n");
		}
	}
	// if (tcm_hcd->func_roate_horizontal_level_en) {
	// 	retval = ovt_tcm_set_dynamic_config(tcm_hcd, DC_ENABLE_ROATE_HORIZONTAL_LEVEL, 1);
	// 	if (retval != 0) {
	// 		LOGE(tcm_hcd->pdev->dev.parent,"Failed to set ear phone connected command\n");
	// 	}
	// }
...
static void speedup_resume(struct work_struct *work)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"speed up resume enter\n");
	if (!tcm_hcd->in_suspend  || tcm_hcd->ovt_tcm_driver_removing)
		return;
...
exit:
	...
	LOGN(tcm_hcd->pdev->dev.parent,"speed up resume end\n");
	return;
}
#endif
static int ovt_tcm_suspend(struct device *dev)
...
static int ovt_tcm_probe(struct platform_device *pdev)
{
	...
	LOGN(&pdev->dev, "omnivision tptd4160 ovt_tcm_probe start.\n");
...
prepare_modules:
	...
	tp_interface.tp_type = TD4160;
	LOGN(&pdev->dev, "omnivision tptd4160 ovt_tcm_probe end.\n");
	return 0;
...
static int __init ovt_tcm_module_init(void)
{
	...
	if ( (!retval) && ((!strstr(lcd_name, LCD_NAME_NEW))&&(!strstr(lcd_name, LCD_NAME_OLD))
	&&(!strstr(lcd_name, LCD_NAME_LIMU))&&(!strstr(lcd_name, LCD_NAME_td4160b))) )
	{
		printk("%s: match %s fail,return\n",__func__,lcd_name);
		return 0;
	}

	if (strstr(lcd_name, LCD_NAME_OLD)) {
		g_firmware_version = 0;
	} else if (strstr(lcd_name, LCD_NAME_NEW)) {
		g_firmware_version = 1;
	} else if (strstr(lcd_name, LCD_NAME_LIMU)) {
		g_firmware_version = 2;
	} else if (strstr(lcd_name, LCD_NAME_td4160b)) {
		g_firmware_version = 3;
	}

	printk("omnivision ovt_tcm_module_init start match %s\n",lcd_name);
	retval += device_module_init();
	retval += diag_module_init();
	retval += recovery_module_init();
	retval += reflash_module_init();
	retval += testing_module_init();
	retval += zeroflash_module_init();
