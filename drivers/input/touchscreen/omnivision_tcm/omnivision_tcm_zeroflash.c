...
#define FW_IMAGE_NAME_PANEL_CHUANGWEI_0 "hdl_firmware_0.img"
#define FW_IMAGE_NAME_PANEL_CHUANGWEI_1 "hdl_firmware_1.img"
#define FW_IMAGE_NAME_PANEL_LIMU "hdl_firmware_limu.img"
#define FW_IMAGE_NAME_PANEL_CHUANGWEI_b "hdl_firmware_b.img"
...
static int zeroflash_get_fw_image(void)
{
	...
	if (zeroflash_hcd->image == NULL) {
		switch (g_firmware_version)
		{
		case 0:
			retval = request_firmware(&zeroflash_hcd->fw_entry,
					FW_IMAGE_NAME_PANEL_CHUANGWEI_0,
					tcm_hcd->pdev->dev.parent);
			printk("success to request %s\n",
					FW_IMAGE_NAME_PANEL_CHUANGWEI_0);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to request %s\n",
		...
		case 1:
			retval = request_firmware(&zeroflash_hcd->fw_entry,
					FW_IMAGE_NAME_PANEL_CHUANGWEI_1,
					tcm_hcd->pdev->dev.parent);
			printk("success to request %s\n",
					FW_IMAGE_NAME_PANEL_CHUANGWEI_1);
			if (retval < 0) {
		...
		case 3:
			retval = request_firmware(&zeroflash_hcd->fw_entry,
					FW_IMAGE_NAME_PANEL_CHUANGWEI_b,
					tcm_hcd->pdev->dev.parent);
			printk("success to request %s\n",
					FW_IMAGE_NAME_PANEL_CHUANGWEI_b);
			if (retval < 0) {
	...
	if (zeroflash_hcd->fw_entry == NULL) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"g_firmware_version error value:%d\n", g_firmware_version);
		return -EINVAL;
	}
