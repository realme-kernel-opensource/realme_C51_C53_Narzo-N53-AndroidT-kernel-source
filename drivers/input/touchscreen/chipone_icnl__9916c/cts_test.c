...
static void cts_dump_gstr_tsdata(struct cts_device *cts_dev,
    const char *desc, const u16 *data, bool to_console)
{
#define SPLIT_LINE_STR \
    "--------------------------------------------------------"\
...
#define DATA_FORMAT_STR     "%-5u "
    ...
    sum = 0;
    max_r = max_c = min_r = min_c = 0;
    for (r = 0; r < cts_dev->fwdata.gstr_rows; r++) {
        for (c = 0; c < cts_dev->fwdata.gstr_cols; c++) {
            u16 val = data[r * cts_dev->hwdata->num_col + c];
    ...
    average = sum / (cts_dev->fwdata.gstr_rows * cts_dev->fwdata.gstr_cols);
...
static int validate_gstr_tsdata(struct cts_device *cts_dev, const char *desc, u16 *data,
    u32 *invalid_nodes, u32 num_invalid_nodes, bool per_node, int *min, int *max)
{
#define SPLIT_LINE_STR \
    "------------------------------"
...
int cts_test_rawdata(struct cts_device *cts_dev, struct cts_test_param *param)
{
    ...
    ret = cts_tcs_set_mnt_enable(cts_dev, 0x01);
    if (ret) {
        cts_err("Enable firmware monitor mode failed %d", ret);
    }
...
#define CTS_BLACK_SCREEN_TEST_ABANDON_FRAMES		5

void cts_print_test_result_to_console(
    const char *test_case, int result, ktime_t delta_time)
{
    if (result > 0) {
        cts_info("%-10s test: %d nodes FAIL, ELAPSED TIME: %lldms",
            test_case, result, delta_time);
    } else if (result < 0) {
        cts_info("%-10s test: FAIL %d(%s), ELAPSED TIME: %lldms",
            test_case, result, cts_strerror(result), delta_time);
    } else {
        cts_info("%-10s test: PASS, ELAPSED TIME: %lldms",
            test_case, delta_time);
    }
}
	...
	struct cts_test_param *param)
{
	...
	if (!idle_mode) {
		cts_dev->fwdata.gstr_rows = cts_dev->fwdata.rows;
		cts_dev->fwdata.gstr_cols = 2;
		cts_dev->hwdata->gstr_num_row = cts_dev->hwdata->num_row;
		cts_dev->hwdata->gstr_num_col = 2;
	} else {
		cts_dev->fwdata.gstr_rows = cts_dev->fwdata.rows;
		cts_dev->fwdata.gstr_cols = cts_dev->fwdata.cols;
		cts_dev->hwdata->gstr_num_row = cts_dev->hwdata->num_row;
		cts_dev->hwdata->gstr_num_col = cts_dev->hwdata->num_col;
	}
	cts_info("cts_dev->fwdata.gstr_rows = %d, cts_dev->fwdata.gstr_cols = %d, "
		"cts_dev->hwdata->gstr_num_col = %d", cts_dev->fwdata.gstr_rows,
		cts_dev->fwdata.gstr_cols, cts_dev->hwdata->gstr_num_col);

    //num_nodes = cts_dev->hwdata->gstr_num_row * cts_dev->hwdata->gstr_num_col;
    num_nodes = cts_dev->hwdata->num_row * cts_dev->hwdata->num_col;
	tsdata_frame_size = 2 * num_nodes;
	...
	cts_info("Gesture rawdata test, flags: 0x%08x, frames: %d, num invalid node: %u, "
		"test data file: '%s' buf size: %d, drive log file: '%s' buf size: %d",
		param->flags, priv_param->frames, param->num_invalid_node,
		param->test_data_filepath, param->test_data_buf_size,
		param->driver_log_filepath, param->driver_log_buf_size);
	...
	for (i = 0; i < 3; i++) {
		if(cts_tcs_set_gstr_raw_dbg_mode(cts_dev, idle_mode))
			cts_err("Set gesture raw type: %d failed, retry: %d", idle_mode, i);
		else
			break;
	}
	if (i == 3) {
	...
	for (frame = 0; frame < priv_param->frames; frame++) {
		...
		for (j = 0; j < CTS_BLACK_SCREEN_TEST_ABANDON_FRAMES; j++) {
			for (i = 0; i < 3; i++) {
				r = cts_tcs_top_get_rawdata(cts_dev, (u8 *)gesture_rawdata,
					cts_dev->hwdata->num_row * cts_dev->hwdata->num_col * 2);
				if (r) {
...
show_test_result:
	...
	if (idle_mode) {
		cts_print_test_result_to_console("Gesture Noise", ret, delta_time);
	} else {
		cts_print_test_result_to_console("Gesture LP Noise", ret, delta_time);
	}
		...
		struct cts_test_param *param)
{
	...
	for (frame = 0; frame < priv_param->frames; frame++) {
		...
		for (j = 0; j < CTS_BLACK_SCREEN_TEST_ABANDON_FRAMES; j++) {
			for (i = 0; i < 3; i++) {
				r = cts_tcs_top_get_rawdata(cts_dev, (u8 *)curr_rawdata,
					cts_dev->hwdata->num_row * cts_dev->hwdata->num_col * 2);
				if (r) {
	...
	if (driver_validate_data) {
		...
		if (dump_test_data_to_user &&
			param->test_data_buf_size >= (frame * tsdata_frame_size + tsdata_frame_size)) {
