...
{
#ifdef CFG_CTS_FOR_GKI
    cts_info("forbiddon with GKI! Please use request_firmware!");
    return -EPERM;
#else
    struct file *file;
    u16 version;
        ...
        const char *name, struct device *device, int curr_version)
{
    ...
    /* Check firmware */
    if (!is_firmware_valid(firmware)) {
        cts_err("Request firmware file INVALID, size %zu", firmware->size);
        ret = -EINVAL;
        ...
        u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
    ...
    if (g_firmware_version_cts == 0) {
        firmware_filename = CFG_CTS_FIRMWARE_FILENAME;
        firmware_filepath = CFG_CTS_FIRMWARE_FILEPATH;
    } else  if (g_firmware_version_cts == 1) {
        firmware_filename = CFG_CTS_FIRMWARE_FILENAME_INX;
        firmware_filepath = CFG_CTS_FIRMWARE_FILEPATH_INX;
    }

#ifdef CFG_CTS_FIRMWARE_IN_FS
#ifndef CFG_CTS_FOR_GKI
    if (is_filesystem_mounted(firmware_filepath))
#endif
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
        cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : firmware_filename,
#else
        firmware_filename,
#endif
...
static int validate_flash_data(const struct cts_device *cts_dev, u32 flash_addr,
...
static int cts_program_firmware_from_sram_to_flash(const struct cts_device *cts_dev,
