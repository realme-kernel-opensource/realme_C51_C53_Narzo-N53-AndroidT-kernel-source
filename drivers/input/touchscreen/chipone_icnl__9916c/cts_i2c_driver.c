...
struct chipone_ts_data *hulk_tp_data;
EXPORT_SYMBOL(hulk_tp_data);
extern struct touch_panel tp_interface;
int g_firmware_version_cts = 0;
EXPORT_SYMBOL(g_firmware_version_cts);
...
#define LCD_NAME_LD_ICNL9916 "lcd_icnl9916c_ld_hjc_mipi_hd"
#define LCD_NAME_LD_ICNL9916_INX "lcd_icnl9916c_inx_mipi_hd"
...
#endif
{
    ...
    hulk_tp_data = cts_data;
	tp_interface.headset_switch_status = set_tp_earjack_state;
    tp_interface.charger_mode_switch_status = set_cts_tp_charger_mode;
	tp_interface.tp_type = ICNL9916C;
...
#ifdef USER_DEBUG
    cts_info("USER_DEBUG_VERSION GKI_OFF");
#endif

#ifdef CFG_CTS_FOR_GKI
    cts_info("USER_VERSION GKI_ON");
#endif

    cts_info("check lcd_name!");
	retval = get_bootargs(lcd_name, "lcd_name");
	if ((!retval) && (!strstr(lcd_name, LCD_NAME_LD_ICNL9916)) && (!strstr(lcd_name, LCD_NAME_LD_ICNL9916_INX)))
	{
		cts_info("%s: match %s fail,return\n",__func__,lcd_name);
		return 0;
	}
    if (strstr(lcd_name, LCD_NAME_LD_ICNL9916_INX)) {
        g_firmware_version_cts = 1;
    }
    cts_info("%s: lcd_name : %s , match pass! \n",__func__,lcd_name);
