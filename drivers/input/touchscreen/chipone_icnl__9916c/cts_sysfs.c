        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = cts_tcs_write_reg(cts_dev, addr, data, argc - 1);
    if (ret) {
...
#define PRINT_ROW_SIZE          (16)
    ...
    ret = cts_tcs_read_reg(cts_dev, addr, data, (size_t)size);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    manualdiff = (s16 *) kmalloc(DIFFDATA_BUFFER_SIZE(cts_dev), GFP_KERNEL);
    if (manualdiff == NULL) {
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return scnprintf(buf, PAGE_SIZE, "proximity mode: %d\n",
            cts_dev->rtdata.fw_status.proximity);
}
...
{
    if (buf[0] == '1')
        cts_suspend(dev_get_drvdata(dev));
    else if (buf[0] == '0')
        cts_resume(dev_get_drvdata(dev));

    return count;
}
static DEVICE_ATTR(ts_suspend,  S_IWUSR, NULL, cts_suspend_store);
