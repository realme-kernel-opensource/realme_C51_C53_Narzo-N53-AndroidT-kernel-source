...
/*flag for gesture mode*/
extern int tp_gesture;

/*flag for black test flow*/
extern int touch_black_test;

/*for *#808# hwinfo*/
extern void get_hardware_info_data(enum hardware_id id, const void *data);
static char firmware_version[25] = {'H','U','L','K','_','L','D','_','M','D','T','_','N','L','0','4','0','0'};
static char firmware_version_inx[25] = {'H','U','L','K','_','T','o','k','e','n','_','I','N','X','_','N','L','0','6','0','0'};

/*FOR realme tp interface*/
extern struct chipone_ts_data *hulk_tp_data;
bool earjack_state_changed = false;
bool charger_state_changed = false;
...
int cts_get_diffdata(const struct cts_device *cts_dev, void *buf)
{
    ...
    cache_buf = kzalloc((cts_dev->fwdata.rows + 2) *
            (cts_dev->fwdata.cols + 2) * 2, GFP_KERNEL);
    if (cache_buf == NULL) {
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    get_hardware_info_data(HWID_CTP_DRIVER, "chipone_9916c");
    cts_info("driver_name:chipone_9916c\n");

    sprintf(hex_str, "%04X", fwdata->version);

    if (g_firmware_version_cts == 0) {
        // firmware_version[18] = hex_str[0];
        // firmware_version[19] = hex_str[1];
        firmware_version[16] = hex_str[2];
        firmware_version[17] = hex_str[3];
        get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version);
        cts_info("Hardinfo_tp_fw_ver:%s\n",firmware_version);
    } else if (g_firmware_version_cts == 1) {
        firmware_version_inx[19] = hex_str[2];
        firmware_version_inx[20] = hex_str[3];
        get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_inx);
        cts_info("Hardinfo_tp_fw_ver:%s\n",firmware_version_inx);
    }
    ...
    if (fwdata->has_int_data) {
        ...
        ret = cts_tcs_init_int_data(cts_dev);
        if (ret < 0) {
            cts_err("init_int_data failed: %d", ret);
            return -EINVAL;
        }
    }
...
{
    return !!cts_dev->rtdata.fw_status.proximity;
}

int cts_enter_proximity_mode(struct cts_device *cts_dev)
{
...
*/
    if (tp_gesture == 1 || touch_black_test ==1) {
        cts_info("tp_gesture = %d, touch_black_test = %d",tp_gesture,touch_black_test);
        cts_dev->rtdata.gesture_wakeup_enabled = 1;
        cts_info("gesture_wakeup_enabled : %d",cts_dev->rtdata.gesture_wakeup_enabled);
    }
    if (cts_dev->rtdata.program_mode) {
	...
	if (cts_dev->rtdata.gesture_wakeup_enabled || tp_gesture || touch_black_test)
		gesture = true;
	...
	if (touch_black_test) {
		...
		if (ret) {
			cts_err("Set GSTR_DATA_DBG_EN failed %d", ret);
		}
#endif
	}

    cts_info("Set suspend mode:%s", gesture ? "gesture" : "sleep");
...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    if (retries < 0) {
        ...
        cts_info(" init fwdata_1 ");
		if (ret){
            cts_info(" init fwdata_1 failed ");
			return -EINVAL;
        }
    ...
    set_tp_earjack_state(earjack_state_changed);

    set_cts_tp_charger_mode(charger_state_changed);

    if (tp_gesture == 0) {
        cts_info("tp_gesture = %d, touch_black_test = %d",tp_gesture,touch_black_test);
        cts_dev->rtdata.gesture_wakeup_enabled = 0;
        cts_info("gesture_wakeup_enabled : %d",cts_dev->rtdata.gesture_wakeup_enabled);
    }
...
void cts_enable_gesture_wakeup(struct cts_device *cts_dev)
{
    ...
    tp_gesture = cts_dev->rtdata.gesture_wakeup_enabled;
    cts_info("Enable gesture wakeup , tp_gesture : %d",tp_gesture);
}
...
void cts_disable_gesture_wakeup(struct cts_device *cts_dev)
{
    ...
    tp_gesture = cts_dev->rtdata.gesture_wakeup_enabled;
    cts_info("disable gesture wakeup , tp_gesture : %d",tp_gesture);
}
...
void cts_show_touch_debug_msg(void *debug)
{
    ...
    cts_info("Charger     : %d", msg->fw_status.charger);
    cts_info("Proxmty     : %d", msg->fw_status.proximity);
    cts_info("Earjack     : %d", msg->fw_status.earjack);
    cts_info("Knuckle     : %d", msg->fw_status.knuckle);
    cts_info("Glove       : %d", msg->fw_status.glove);
    cts_info("Pocket      : %d", msg->fw_status.pocket);
    cts_info("Game        : %d", msg->fw_status.game);

    cts_info("High_Temp   : %d", msg->fw_status.high_temperature);
    cts_info("Low_Temp    : %d", msg->fw_status.low_temperature);
    cts_info("Bend        : %d", msg->fw_status.bend_mode);
    cts_info("Palm        : %d", msg->fw_status.palm_status);
    cts_info("Noise       : %d", msg->fw_status.noise_mode);
    cts_info("Base_Trace  : %d", msg->fw_status.base_trace);
    cts_info("Water       : %d", msg->fw_status.water_mode);
    cts_info("Ground      : %d", msg->fw_status.ground);
...
void cts_heartbeat_mechanism_work(struct work_struct *work)
{
    ...
    memcpy(&info, cts_dev->rtdata.int_data, sizeof(info));

    curr_status = info.debug_msg.fw_status.status & 0xFF;
    ...
    if (curr_status != (cts_dev->rtdata.fw_status.status & 0xFF)) {
...
/* for realme tp_interface.charger_mode_switch_status */
void set_cts_tp_charger_mode(int attached)
{
    int ret;
    u8 buf;
    struct cts_device *cts_dev = &hulk_tp_data->cts_dev;

    cts_info("Set TP charger mode %s", attached ? "ATTACHED" : "DETATCHED");
    buf = attached ? 1 : 0;
    ...
    charger_state_changed = attached;

}
EXPORT_SYMBOL(set_cts_tp_charger_mode);
...
// for realme tp_interface.headset_switch
void set_tp_earjack_state( int attached)
{
    int ret;
    u8 buf;
    struct cts_device *cts_dev = &hulk_tp_data->cts_dev;
    cts_info("Set Tp_earjack_mode : %s", attached ? "ATTACHED" : "DETATCHED");
    buf = attached ? 1 : 0;
    ret = cts_tcs_set_earjack_plug(cts_dev, buf);
    ...
    earjack_state_changed = attached;

}
EXPORT_SYMBOL(set_tp_earjack_state);
...
{
    cts_tcs_deinit_int_data(cts_dev);
}
