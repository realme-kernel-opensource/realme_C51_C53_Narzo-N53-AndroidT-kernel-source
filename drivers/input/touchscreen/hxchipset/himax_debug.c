...
static int chk_slave_reg(char *str, int str_len)
{
	...
	str[str_len] = '\0';
	private_ts->slave_read_reg =
		hx_parse_hexstr2int(buf_read_slave_cmd) % 0x100;

	I("%s: str=%s, str_len=%d\n", __func__, str, str_len);
	I("%s: buf_read_slave_cmd=%s, read_cmd_len=%d, slave_read_reg=%d\n",
		__func__, buf_read_slave_cmd,
		read_cmd_len, private_ts->slave_read_reg);

	private_ts->select_slave_reg = true;
		...
		struct himax_report_data *hx_touch_data)
{
	...
	int32_t *mutual_data;
	int32_t *self_data;
	int x_channel = hx_s_ic_data->HX_RX_NUM;
	int y_channel = hx_s_ic_data->HX_TX_NUM;
	int mul_num;
	int self_num;
	int update_idx;
	int data_status = 0;
	...
	self_data = getSelfBuffer();
	/* initiallize the block number of mutual and self */
	mul_num = hx_s_ic_data->HX_RX_NUM * hx_s_ic_data->HX_TX_NUM;
	self_num = hx_s_ic_data->HX_RX_NUM + hx_s_ic_data->HX_TX_NUM;

	data_status = g_core_fp.fp_diag_parse_raw_data(hx_touch_data, mul_num,
		self_num, hx_touch_data->diag_cmd, mutual_data,
		self_data);
	...
	// memcpy(&g_rb_frame.rawdata[update_idx].self[0], &self_data[0], self_num * sizeof(int32_t));
	// memcpy(&g_rb_frame.rawdata[update_idx].mutual[0], &mutual_data[0], mul_num * sizeof(int32_t));
	memcpy(&g_rb_frame.rawdata[update_idx].self[0], &hx_touch_data->hx_rawdata_buf[4 + 2 * (x_channel * y_channel)], 2 *(x_channel + y_channel) * sizeof(uint8_t));
	memcpy(&g_rb_frame.rawdata[update_idx].mutual[0], &hx_touch_data->hx_rawdata_buf[4], 2 * (x_channel * y_channel) * sizeof(uint8_t));
	...
	if (atomic_read(&(g_rb_frame.p_update)) == (HX_RB_FRAME_SIZE - 1))
		...
		atomic_set(&(g_rb_frame.p_update), atomic_read(&(g_rb_frame.p_update)) + 1);
...
void himax_log_touch_data(int start)
{
	...
	if (hx_touch_data->diag_cmd > 0) {
		if (hx_touch_data->touch_all_size > 128)
			print_size = 128+16;
		else
			print_size = 128;
		...
		memcpy(buf, private_ts->xfer_buff, print_size);
	}
#if defined(HX_SMART_WAKEUP)
	else if ((private_ts->SMWP_enable > 0|| himax_gesture)> 0 && private_ts->suspended) {
...
static ssize_t himax_diag_cmd_write(char *buf, size_t len)
{
	...
	if (buf[0] == 'r' && buf[1] == 'b'
		&& debug_data->is_stack_full_raw == true){
		if (len > 0) {
			...
			if (!kstrtoint(buf, 10, &rst)) {
				...
				if (frame_indx >= HX_RB_FRAME_SIZE)
					E("Bigger than rb max size, fail, %d\n", frame_indx);
				else {
					//I("Now set to idx %d\n", frame_indx);
	...
	case 1:/*raw out select - diag,X*/
		if (!kstrtoint(buf, 16, &rst)) {
			...
			g_core_fp.fp_diag_register_set(ts->diag_cmd, process_type, false);
			...
			if (ts->diag_cmd == 0) {
				...
				debug_data->is_stack_full_raw = false;
			}
...
bool himax_ts_diag_func(int dsram_type)
{
	...
	update_idx = atomic_read(&(g_rb_frame.p_update));
	// memcpy(&g_rb_frame.rawdata[update_idx].self[0], &self_data[0], (x_channel + y_channel) * sizeof(int32_t));
	// memcpy(&g_rb_frame.rawdata[update_idx].mutual[0], &mutual_data[0], (x_channel * y_channel) * sizeof(int32_t));
	memcpy(&g_rb_frame.rawdata[update_idx].self[0], &info_data[2 * (x_channel * y_channel)], 2 *(x_channel + y_channel) * sizeof(uint8_t));
	memcpy(&g_rb_frame.rawdata[update_idx].mutual[0], &info_data[0], 2 * (x_channel * y_channel) * sizeof(uint8_t));
	g_rb_frame.frame_idx[update_idx] = g_rb_frame.rawdata->index++;
...
static int find_idx_min_array(int* array, int size)
{
	int result = 0;
	int i = 0;
...
static int himax_diag_print(struct seq_file *s, void *v)
{
	...
	print_state_info(s);
		...
		if (g_rb_frame.frame_idx[output_p] < g_rb_frame.frame_idx[next_output_p])
			output_p = next_output_p;

		if (g_rb_frame.rawdata->cnt_update >= (HX_RB_FRAME_SIZE - 1))
			output_p = find_idx_min_array(g_rb_frame.frame_idx, HX_RB_FRAME_SIZE);
		...
		header[1]=0xff&output_p;
		header[2]=0xff&g_rb_frame.frame_idx[output_p];
		header[3]=0xff&(g_rb_frame.frame_idx[output_p]>>8);
		header[4]=0xff&(g_rb_frame.frame_idx[output_p]>>16);
		header[5]=0xff&(g_rb_frame.frame_idx[output_p]>>24);

		seq_write(s, header, RAW_DATA_HEADER_LENGTH);
		// Self
		//seq_write(s, &self_num , sizeof(short));
		seq_write(s, g_rb_frame.rawdata[output_p].self, sizeof(uint8_t) * self_num);
		// Mutual
		//seq_write(s, &mutual_num , sizeof(short));
		seq_write(s, g_rb_frame.rawdata[output_p].mutual, sizeof(uint8_t) * mutual_num);
...
static int hx_node_update(char *buf, size_t is_len)
{
	...
	if (strcmp(fileName, g_fw_mp_upgrade_name) == 0)
		fw_type = 1;
...
static void himax_himax_data_init(void)
{
	...
	debug_data->_raw_full_stack = hx_dbg_raw_full_stack;
	debug_data->flash_dump_going = false;
	debug_data->is_checking_irq = false;
	debug_data->is_call_help = false;
	debug_data->is_stack_full_raw = false;
	private_ts->select_slave_reg = false;
	private_ts->acc_slave_reg = false;
}
...
static void hx_dbg_rst_rb(void)
{
	...
	for (i = 0; i < HX_RB_FRAME_SIZE; i++) {
		memset(g_rb_frame.rawdata[i].mutual, 0x00, (sizeof(uint8_t) * (ic_data->HX_RX_NUM * ic_data->HX_TX_NUM) * 2));
		memset(g_rb_frame.rawdata[i].self, 0x00, (sizeof(uint8_t) * (ic_data->HX_RX_NUM + ic_data->HX_TX_NUM) * 2));
...
int himax_debug_init(void)
{
	...
	g_rb_frame.rawdata = kzalloc(sizeof(struct frame_data) * HX_RB_FRAME_SIZE, GFP_KERNEL);
	g_rb_frame.rawdata->index = 0;
	g_rb_frame.rawdata->cnt_update = 0;
	atomic_set(&(g_rb_frame.p_update), 0);
	atomic_set(&(g_rb_frame.p_output), 0);
	atomic_set(&(g_rb_frame.length), 0);
	for (i = 0; i < HX_RB_FRAME_SIZE; i++) {
		g_rb_frame.rawdata[i].mutual = kzalloc(sizeof(uint8_t) * (ic_data->HX_RX_NUM * ic_data->HX_TX_NUM) * 2, GFP_KERNEL);
		g_rb_frame.rawdata[i].self = kzalloc(sizeof(uint8_t) * (ic_data->HX_RX_NUM + ic_data->HX_TX_NUM) * 2, GFP_KERNEL);
	}
	return 0;
