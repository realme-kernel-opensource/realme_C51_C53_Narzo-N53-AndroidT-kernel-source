...
static bool himax_mcu_dd_clk_set(bool enable)
{
	...
	return (g_core_fp.fp_register_write(pic_op->adr_osc_en,
		 data, sizeof(pic_op->adr_osc_en)) == NO_ERR);
}
		...
		int len, uint8_t *data, uint8_t bank)
{
	...
	return (g_core_fp.fp_register_write(tmp_addr,  w_data, data_len)
			== NO_ERR);
}
...
/* CORE_FW */
/* FW side start*/
static int diag_mcu_parse_raw_data(struct himax_report_data *hx_touch_data,
		...
		int32_t *mutual_data, int32_t *self_data)
{
	...
	if (!debug_data->is_stack_full_raw) {
		...
		checksum += (((int8_t)hx_touch_data->
					hx_rawdata_buf[1]) * 256
					+ hx_touch_data->
					hx_rawdata_buf[0]);
		checksum += (((int8_t)hx_touch_data->
					hx_rawdata_buf[3]) * 256
					+ hx_touch_data->
					hx_rawdata_buf[2]);
		RawDataLen_word = hx_touch_data->rawdata_size / 2;
		for (i = 0; i < RawDataLen_word; i++) {
		...
		uint8_t storage_type, bool is_dirly)
{
	...
	private_ts->diag_storage_type = storage_type;
	private_ts->diag_dirly = is_dirly;
}
...
extern void ovt_get_tp_fw_ver(void);
...
static bool himax_mcu_read_event_stack(uint8_t *buf, uint32_t length)
{
...
static int himax_mcu_diag_parse_raw_data(
		struct himax_report_data *hx_touch_data,
		int mul_num, int self_num, uint8_t diag_cmd,
		int32_t *mutual_data, int32_t *self_data)
{
	return	diag_mcu_parse_raw_data(hx_touch_data, mul_num, self_num,
...
static int himax_mcu_excp_dd_recovery(void)
{
	return NO_ERR;
}
#endif
...
int code_overlay(struct zf_info *info, const struct firmware *fw, int type)
{
	int ret = 0;
	int retry = 0;
	...
	do {
		g_core_fp.fp_register_write(tmp_addr, code_sdata,
			DATA_LEN_4);
		usleep_range(1000, 1100);
		g_core_fp.fp_register_read(tmp_addr, rdata,
			DATA_LEN_4);
		retry++;
	...
	const struct firmware *fw)
{
	...
	retry = 0;
	do {
		g_core_fp.fp_register_write(tmp_addr, alg_sdata,
			DATA_LEN_4);
		usleep_range(1000, 1100);
		g_core_fp.fp_register_read(tmp_addr, rdata,
			DATA_LEN_4);
...
static void hx_setup_opt_hw_crc(const struct firmware *fw)
{
	uint32_t tmp_p_addr;
	if (g_zf_opt_crc.fw_addr != 0) {
		...
		I("%s: opt_crc start: R%02X%02X%02X%02XH <= 0x%02X%02X%02X%02X\n", __func__,
		...
		I("%s: opt_crc end: R%02X%02X%02X%02XH <= 0x%02X%02X%02X%02X\n", __func__,
...
int himax_zf_part_info(const struct firmware *fw, int type)
{
	...
	memset(g_update_cfg_buf, 0x00,
		private_ts->chip_max_dsram_size * sizeof(uint8_t));
	...
	cfg_crc_sw = g_core_fp.fp_Calculate_CRC_with_AP(g_update_cfg_buf, 0, cfg_sz);
	do {
		g_core_fp.fp_write_sram_0f(sram_min, g_update_cfg_buf, cfg_sz);
	...
	if (g_has_alg_overlay)
		ret = alg_overlay(alg_idx_t, info, fw);
	if (has_code_overlay)
		ret = code_overlay(info, fw, type);
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (strcmp(file_name, g_fw_mp_upgrade_name) == 0)
		type = 1;
...
static void himax_mcu_tx_chg_set(uint8_t cmd)
{
	...
	do {
		if (cmd == 1){
			...
			g_core_fp.fp_register_write(pfw_op->addr_tx_jump_test, tmp_data, DATA_LEN_4);
		...
		g_core_fp.fp_register_read(pfw_op->addr_tx_jump_test, tmp_data, DATA_LEN_4);
		/*I("%s: tmp_data[0]=%d, USB detect=%d, retry_cnt=%d \n", __func__, tmp_data[0],cable_config[1] ,retry_cnt);*/
		retry_cnt++;
...
static void himax_mcu_ultra_enter(void)
{
    ...
    do {
        tmp_data[0] = 0x11;
        if (himax_bus_write(0x34, NULL, tmp_data, 1 ) < 0) {
            I("%s: spi write fail!\n", __func__);
            continue;
        }
        tmp_data[0] = 0x00;
        if (himax_bus_read(0x34, tmp_data, 1 ) < 0) {
            I("%s: spi read fail!\n", __func__);
            continue;
        }
    ...
    do {
        tmp_data[0] = 0x33;
        if (himax_bus_write(0x33, NULL, tmp_data, 1) < 0) {
            I("%s: spi write fail!\n", __func__);
            continue;
        }
        tmp_data[0] = 0x00;
        if (himax_bus_read(0x33, tmp_data, 1) < 0) {
            I("%s: spi read fail!\n", __func__);
            continue;
        }
...
static bool himax_mcu_no_jiter_en(uint8_t no_jiter_en)
{
	...
	do {
		if (no_jiter_en) {
			...
			g_core_fp.fp_register_write(pfw_op->addr_fw_no_jiter_en, tmp_data, DATA_LEN_4);
		...
		g_core_fp.fp_register_read(pfw_op->addr_fw_no_jiter_en, tmp_data, DATA_LEN_4);
...
static bool himax_mcu_edge_limit_en(uint8_t edge_limit_en)
{
	...
	do {
		if (edge_limit_en) {
			himax_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_edge_limit, tmp_data, DATA_LEN_4);
			himax_parse_assign_cmd(fw_func_handshaking_pwd, back_data, 4);
		} else {
			himax_parse_assign_cmd(fw_data_clear, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_edge_limit, tmp_data, DATA_LEN_4);
		...
		g_core_fp.fp_register_read(pfw_op->addr_fw_edge_limit, tmp_data, DATA_LEN_4);
...
void himax_headset_mode_switch(int headset_sate)
{
	...
	private_ts->headset_mode = headset_sate;
	if (private_ts->suspended == 1) {
		I("%s:suspend mode,can not write\n", __func__);
		return;
	}
	...
	do {
		if (headset_sate) {
			himax_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_headline, tmp_data, DATA_LEN_4);
			himax_parse_assign_cmd(fw_func_handshaking_pwd, back_data, 4);
			I("%s: headline IN!\n", __func__);
		} else {
			himax_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
		...
		g_core_fp.fp_register_read(pfw_op->addr_headline, tmp_data, DATA_LEN_4);
...
void himax_usb_mode_switch(int val)
{
	...
	do {
		if (val) {
			himax_parse_assign_cmd(fw_func_handshaking_pwd,
				tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_usb_detect, tmp_data,
				DATA_LEN_4);
			himax_parse_assign_cmd(fw_func_handshaking_pwd,
				back_data, 4);
			I("%s: USB detect status IN!\n", __func__);
		...
		g_core_fp.fp_register_read(pfw_op->addr_usb_detect, tmp_data, DATA_LEN_4);
			I(PRT_DATA,	__func__,
			tmp_data[3],
			tmp_data[2],
			tmp_data[1],
...
/* CORE_DRIVER */
	...
	g_core_fp._ic_excp_dd_recovery = himax_mcu_excp_dd_recovery;
#endif
