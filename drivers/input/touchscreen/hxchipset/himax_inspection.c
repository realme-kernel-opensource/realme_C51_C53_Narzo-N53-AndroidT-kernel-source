...
char *g_hx_inspt_ver_name = "hx_criteria_version";
char *g_str_crtra_ver;
...
fail_wait_sorting_mode:
	...
	hx_test_data_get(RAW, g_start_log, g_rslt_log, ret_val);
...
int himax_self_test_data_init(void)
{
	...
	char *file_name_2 = "hx_criteria_BOE_HX03.csv";
	char *curScreen = file_name_2;
	if (g_firmware_id == 0) {
		file_name_2 = "hx_criteria_HKC_HX03.csv";
	} else if (g_firmware_id == 2) {
		file_name_2 = "hx_criteria_HKC_HX05.csv";
	}
...
#else
		g_rslt_data = kcalloc(
			g_1kind_raw_size * HX_CRITERIA_ITEM,
			sizeof(char),
			GFP_KERNEL);
#endif		
		if (g_rslt_data == NULL) {
	...
	sprintf(g_file_name, "Hx_result_%02d%02d%02d-%02d%02d%02d-utc_",
            (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
            rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	I("%s: try to mkdir! %d\n", __func__, dev_mkdir("/sdcard/TpTestReport", S_IRUGO | S_IWUSR));
if (ts->suspended == false) {
		snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH) + strlen(g_file_name)
			+ strlen(HX_RSLT_OUT_FILE)+1),
			"%s%s%s", HX_RSLT_OUT_PATH,g_file_name, HX_RSLT_OUT_FILE);
#if defined(HX_SMART_WAKEUP)
	} else {
		snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH) + strlen(g_file_name)
			+ strlen(HX_BLACK_RSLT_OUT_FILE)+1),
			"%s%s%s", HX_RSLT_OUT_PATH,g_file_name, HX_BLACK_RSLT_OUT_FILE);
#endif
	}
	file_w_flag = true;
#endif
...
static int himax_chip_self_test(struct seq_file *s, void *v)
{
	...
	if (private_ts->debug_log_level & BIT(4)) {
		...
		g_mp_fw_ver = (ic_data->vendor_cid_maj_ver << 8
			| ic_data->vendor_cid_min_ver);
	}
#endif
#if defined(HX_RW_FILE)
			...
			if (g_test_item_flag[i] == 1) {
				...
				if (file_w_flag &&
					...
					if (hx_write_file(g_rslt_data,
						g_rslt_data_len,
						pos) == -EFAULT){
						I("Write fs content Fail!\n");
					} else {
						I("Write fs content OK!\n");
						pos += g_rslt_data_len;
					}
				}
#endif
...
#if defined(HX_ZERO_FLASH)
	if (private_ts->debug_log_level & BIT(4))
		seq_printf(s, "MP FW CID : 0x%04X, ", g_mp_fw_ver);
#endif
	seq_printf(s, "Criteria Version : %s\n", g_str_crtra_ver);
if (ret == HX_INSP_OK)
		{
		seq_puts(s, "pass\n");
		I(" self test passed %s %d \n",__func__,__LINE__);
			}
		else
		{
		seq_puts(s, "fail\n");
		I(" self test Fail %s %d \n",__func__,__LINE__);
	}
...
int himax_black_chip_self_test(struct seq_file *s, void *v)
{
	...
	g_core_fp.fp_0f_op_file_dirly(g_fw_mp_upgrade_name);//HX108a
...
#endif
	/*Do normal test items*/
	for (i = HX_LP_WT_NOISE; i < HX_CRITERIA_ITEM; i++) {
			if (g_test_item_flag[i] == 1) {
				I("%d. %s Start\n", i,
					g_himax_inspection_mode[i]);
...
// #endif
	...
	if (ret == HX_INSP_OK)
		{
		seq_puts(s, "pass\n");
		I("black self test passed %s %d \n",__func__,__LINE__);
		}
		else
		{
		seq_puts(s, "fail\n");
		I("black self test Fail %s %d \n",__func__,__LINE__);
		}
	himax_self_test_data_deinit();
		touch_black_test = 0;
