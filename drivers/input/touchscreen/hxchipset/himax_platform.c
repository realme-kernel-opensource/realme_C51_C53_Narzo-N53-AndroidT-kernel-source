...
int g_firmware_id = 0;
EXPORT_SYMBOL(g_firmware_id);
static char firmware_version_hkc_hx03[20] = {'H','U','L','K','_','L','C','E','_','H','K','C','_','H','X','0','3'};
static char firmware_version_boe_hx03[20] = {'H','U','L','K','_','L','C','E','_','B','O','E','_','H','X','0','3'};
static char firmware_version_hkc_hx05[20] = {'H','U','L','K','_','L','C','E','_','H','K','C','_','H','X','0','5'};

static char lcd_name[100];
#define LCD_NAME_HKC_HX03 "lcd_hx83108a_lce_mipi_hd"
#define LCD_NAME_BOE_HX03 "lcd_hx83108a_boe_mipi_hd"
#define LCD_NAME_HKC_HX05 "lcd_hx83108a_lce_new_mipi_hd"
...
void ovt_get_tp_fw_ver(void)
{
	char str[3] = {0};
	int mp_fw_ver = 0;

	get_hardware_info_data(HWID_CTP_DRIVER, "himax_tcm");
	printk("driver_name:himax_tcm\n");

	mp_fw_ver = ic_data->vendor_touch_cfg_ver;
	snprintf(str, 3,
		"%02x", mp_fw_ver);

	if (g_firmware_id == 0) {
		firmware_version_hkc_hx03[17] = str[0];
		firmware_version_hkc_hx03[18] = str[1];
		get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_hkc_hx03);
		printk("firmware_version:%s\n",firmware_version_hkc_hx03);
	} else if (g_firmware_id == 1) {
		firmware_version_boe_hx03[17] = str[0];
		firmware_version_boe_hx03[18] = str[1];
		get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_boe_hx03);
		printk("firmware_version:%s\n",firmware_version_boe_hx03);
	} else if (g_firmware_id == 2) {
		firmware_version_hkc_hx05[17] = str[0];
		firmware_version_hkc_hx05[18] = str[1];
		get_hardware_info_data(HWID_CTP_FW_INFO, firmware_version_hkc_hx05);
		printk("firmware_version:%s\n",firmware_version_hkc_hx05);
	}
}
EXPORT_SYMBOL(ovt_get_tp_fw_ver);
...
/* add for esd recovery start */
int himax_chip_common_suspend_esd(void)
{
	return himax_chip_common_suspend(private_ts);
}
EXPORT_SYMBOL(himax_chip_common_suspend_esd);

int himax_chip_common_resume_esd(void)
{
	return himax_chip_common_resume(private_ts);
}
EXPORT_SYMBOL(himax_chip_common_resume_esd);
/* add for esd recovery end */
...
static struct attribute *himax_dev_suspend_atts[] = {
	&dev_attr_ts_suspend.attr,
	NULL
};
static const struct attribute_group himax_dev_suspend_atts_group = {
	.attrs = himax_dev_suspend_atts,
};
static const struct attribute_group *himax_dev_attr_groups[] = {
	&himax_dev_suspend_atts_group,
	NULL
};
int himax_sysfs_add_device(struct device *dev)
{
	int ret = 0, i;

	for (i = 0; himax_dev_attr_groups[i]; i++) {
		ret = sysfs_create_group(&dev->kobj, himax_dev_attr_groups[i]);
		if (ret) {
			while (--i >= 0)
				sysfs_remove_group(&dev->kobj,
						himax_dev_attr_groups[i]);
			break;
		}
	}
	return ret;
}
int himax_sysfs_remove_device(struct device *dev)
{
	int i;

	sysfs_remove_link(NULL, "touchscreen");
	for (i = 0; himax_dev_attr_groups[i]; i++)
		sysfs_remove_group(&dev->kobj, himax_dev_attr_groups[i]);
	return 0;
}
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	himax_sysfs_add_device(ts->dev);
	if (sysfs_create_link(NULL, &ts->dev->kobj, "touchscreen") < 0)
		E("Failed to create touchscreen link! %d\n", ret);
...
int himax_chip_common_remove(struct spi_device *spi)
{
	...
	himax_sysfs_remove_device(ts->dev);
...
static int __init himax_common_init(void)
{
	...
	if ((!retval) && (!strstr(lcd_name, LCD_NAME_HKC_HX03)) && (!strstr(lcd_name, LCD_NAME_BOE_HX03))
				  && (!strstr(lcd_name, LCD_NAME_HKC_HX05)))
	{
		printk("%s: match %s fail,return\n",__func__,lcd_name);
		return 0;
	}
	if (strstr(lcd_name, LCD_NAME_HKC_HX03)) {
		g_firmware_id = 0;
	} else if (strstr(lcd_name, LCD_NAME_BOE_HX03)) {
		g_firmware_id = 1;
	} else if (strstr(lcd_name, LCD_NAME_HKC_HX05)) {
		g_firmware_id = 2;
	} else {
		g_firmware_id = 3;
	}

	I("Himax common touch panel driver init match %s\n", lcd_name);
