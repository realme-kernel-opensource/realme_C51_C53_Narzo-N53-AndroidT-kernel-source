...
/**
 * enum sprd_tcpm_timestamp_sources - Timestamp sources for tcpm log messages.
 * @SPRD_TCPM_PRINTK_TIME_DISABLED: No time stamp.
 * @SPRD_TCPM_PRINTK_TIME_LOCAL: Local hardware clock timestamp.
 * @SPRD_TCPM_PRINTK_TIME_BOOT: Boottime clock timestamp.
 * @SPRD_TCPM_PRINTK_TIME_MONO: Monotonic clock timestamp.
 * @SPRD_TCPM_PRINTK_TIME_REAL: Realtime clock timestamp.
 */
enum sprd_tcpm_timestamp_sources {
	SPRD_TCPM_PRINTK_TIME_DISABLED = 0,
	SPRD_TCPM_PRINTK_TIME_LOCAL = 1,
	SPRD_TCPM_PRINTK_TIME_BOOT = 2,
	SPRD_TCPM_PRINTK_TIME_MONO = 3,
	SPRD_TCPM_PRINTK_TIME_REAL = 4,
};

static const char * const sprd_tcpm_timestamp_sources_str[5] = {
	"disabled",
	"local",
	"boottime",
	"monotonic",
...
static u64 (*sprd_tcpm_printk_get_ts)(void);
static int sprd_tcpm_printk_time = CONFIG_PRINTK_TIME_TYPE;
static bool sprd_tcpm_set_timestamp_source;

static int sprd_tcpm_printk_set_ts_source(enum sprd_tcpm_timestamp_sources ts_source)
{
	int err = 0;

	switch (ts_source) {
	case SPRD_TCPM_PRINTK_TIME_LOCAL:
		sprd_tcpm_printk_get_ts = local_clock;
		break;
	case SPRD_TCPM_PRINTK_TIME_BOOT:
		sprd_tcpm_printk_get_ts = ktime_get_boot_fast_ns;
		break;
	case SPRD_TCPM_PRINTK_TIME_MONO:
		sprd_tcpm_printk_get_ts = ktime_get_mono_fast_ns;
		break;
	case SPRD_TCPM_PRINTK_TIME_REAL:
		sprd_tcpm_printk_get_ts = ktime_get_real_fast_ns;
		break;
	case SPRD_TCPM_PRINTK_TIME_DISABLED:
	/*
	...
	if (!err)
		pr_info("tcpm printk: timestamp set to %s, tcpm_printk_get_ts => %ps\n",
			sprd_tcpm_timestamp_sources_str[ts_source], sprd_tcpm_printk_get_ts);

	sprd_tcpm_set_timestamp_source = true;
...
static void _sprd_tcpm_log(struct sprd_tcpm_port *port, const char *fmt, va_list args, const char *dev_tag)
{
	...
	if (likely(sprd_tcpm_set_timestamp_source))
		ts_nsec = sprd_tcpm_printk_get_ts();
	else
		ts_nsec = local_clock();
...
static void sprd_tcpm_debug_log_init(struct sprd_tcpm_port *port)
{
	...
	sprd_tcpm_printk_set_ts_source(sprd_tcpm_printk_time);
}
