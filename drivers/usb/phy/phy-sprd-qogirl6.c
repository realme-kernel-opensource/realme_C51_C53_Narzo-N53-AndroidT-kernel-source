				  ...
				  unsigned long event, void *data)
{
	...
	dev_info(phy->dev, "%s	event%d enter\n", __func__, event);
...
int sprd_hsphy_set_dpdm_high_impedance_state(void)
{
	u32 msk;
	int ret = 0;
	struct sprd_hsphy *phy = hsphy;
	u32 val = 0;
	...
	if (atomic_read(&phy->high_impedance_stated)) {
		pr_err("%s is already high impedance stated\n", __func__);
		return 0;
	}
	...
	regmap_read(phy->ana_g2, REG_ANLG_PHY_G2_ANALOG_USB20_USB20_UTMI_CTL1, &val);
	dev_info(phy->dev, "%s enter, utmi_ctl1=0x%X, line:%d\n", __func__, val, __LINE__);
	regmap_read(phy->hsphy_glb, REG_AON_APB_OTG_PHY_TEST, &val);
	dev_info(phy->dev, "%s enter, otg_phy_test=0x%X, line:%d\n", __func__, val, __LINE__);

	ret |= regmap_update_bits(phy->ana_g2, REG_ANLG_PHY_G2_ANALOG_USB20_USB20_ISO_SW,
			MASK_ANLG_PHY_G2_ANALOG_USB20_USB20_ISO_SW_EN, 0);
	...
	atomic_set(&phy->high_impedance_stated, 1);

	return ret;
}
EXPORT_SYMBOL(sprd_hsphy_set_dpdm_high_impedance_state);

int sprd_hsphy_cancel_dpdm_high_impedance_state(void)
{
    u32 reg, msk;
    int ret = 0;
    struct sprd_hsphy *phy = hsphy;
    ...
    regmap_read(phy->ana_g2, REG_ANLG_PHY_G2_ANALOG_USB20_USB20_UTMI_CTL1, &val);
    dev_info(phy->dev, "%s enter, utmi_ctl1=0x%04X, line:%d\n", __func__, val,__LINE__);
    regmap_read(phy->hsphy_glb, REG_AON_APB_OTG_PHY_TEST, &val);
    dev_info(phy->dev, "%s enter, otg_phy_test=0x%X, line:%d\n", __func__, val,__LINE__);

    if(!atomic_read(&phy->high_impedance_stated)) {
        pr_err("%s already cancel, line:%d\n", __func__, __LINE__);
        return 0;
    }
    ...
    atomic_set(&phy->high_impedance_stated, 0);
...
EXPORT_SYMBOL(sprd_hsphy_cancel_dpdm_high_impedance_state);

bool sprd_hsphy_get_dpdm_impedance_state(void)
{
	bool state = 0;
	struct sprd_hsphy *phy = hsphy;
	u32 val = 0;
	...
	state = val & MASK_ANLG_PHY_G2_ANALOG_USB20_USB20_VBUSVLDEXT;

	pr_err("%s enter, val=0x%04X, state=%d\n", __func__, val, state);

	return state;
}
EXPORT_SYMBOL(sprd_hsphy_get_dpdm_impedance_state);
