...
static void phx_choose_action_handler(const char *, const phx_action_mapping *,
				      size_t);
...
static phx_action_mapping errno_handle_action_mapping[] = {
	...
	{ ERROR_NATIVE_REBOOT_INTO_RECOVERY,
	  phx_handle_native_reboot_into_recovery },
};
...
static void phx_get_local_time(char *strtime, size_t strtime_length)
{
	...
	snprintf(strtime, strtime_length, "%04d-%02d-%02d %02d:%02d:%02d",
...
static void phx_update_current_stage(const char *stage)
{
	...
	snprintf(phx_curr_info->stage, sizeof(phx_curr_info->stage), "%s",
		 stage);
}
...
static void phx_update_current_error(const char *error)
{
	...
	snprintf(phx_curr_info->error, sizeof(phx_curr_info->error), "%s",
		 error);
}
...
static void phx_update_happen_time(const char *happen_time)
{
	...
	snprintf(phx_curr_info->happen_time, sizeof(phx_curr_info->happen_time),
		 "%s", happen_time);
}
...
static void
phx_choose_action_handler(const char *cmd,
			  const phx_action_mapping *action_map_array,
...
void phx_set_boot_stage(const char *stage)
{
	...
	if (!is_phoenix_boot_completed &&
	    !strcmp(stage, PHOENIX_BOOT_COMPLETED)) {
...
void phx_monit(const char *monitoring_command)
{
	...
	phx_choose_action_handler(stripped_command, monitor_action_mapping,
				  ARRAY_SIZE(monitor_action_mapping));
}
...
static int __init phoenix_base_init(void)
{
	...
	phx_curr_info =
		(phx_baseinfo *)kmalloc(sizeof(phx_baseinfo), GFP_KERNEL);
	if (!phx_curr_info) {
		PHX_KLOG_ERROR("kmalloc to phx_curr_info failed\n");
		return -1;
	}

	/* bootprof->device_initcall */
	init_boot_prof();
