...
#define charger_xlog_printk(num, fmt, ...)                                                                             \
	do {                                                                                                           \
		if (enable_charger_log >= (int)num) {                                                                  \
			printk(KERN_CRIT pr_fmt("[OPLUS_CHG][%s]" fmt), __func__, ##__VA_ARGS__);                    \
		}                                                                                                      \
	} while (0)
...
extern void oplus_vbus_short_cc_detect(void);
extern void voocphy_cpufreq_release(void);
extern void oplus_voocphy_dump_reg(void);
extern int oplus_set_charging_status(bool);
...
#if defined(CONFIG_OPLUS_CHARGER_MTK) || defined(CONFIG_OPLUS_CHARGER_UNISOC)
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
...
#ifndef CONFIG_OPLUS_CHARGER_UNISOC
	...
	chip->shell_themal = thermal_zone_get_zone_by_name("battery");
	if (IS_ERR(chip->shell_themal)) {
		chg_err("Can't get shell_back-> battery\n");
	}
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->cc_safe_enable = of_property_read_bool(node, "qcom,cc_safeguard_enable");
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
...
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	...
	if (in) {
		...
		oplus_set_charging_status(true);
	} else {
		if (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY) {
		...
		oplus_set_charging_status(false);
	}
...
/*	chip->pmic_spmi.usb_online = false;
	...
	chip->cc_allow_fastchg = true;
	chip->cc_safeguard = false;
}
...
/* remove high temp count for sprd os */
#if 0
	...
	if (BATTERY_STATUS__HIGH_TEMP == chip->tbatt_status) {
		if (chip->charger_exist) {
			chip->notify_code |= 1 << NOTIFY_BAT_OVER_TEMP;
			charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] bat_temp(%d) > 55'C BATTERY_STATUS__HIGH_TEMP\n", chip->temperature);
		}
	}
#endif
...
#elif defined(CONFIG_OPLUS_CHARGER_MTK)
				i = 0;
...
int oplus_chg_get_shell_temp(void)
{
	...
	if (rc) {
		g_charger_chip->shell_themal = thermal_zone_get_zone_by_name("battery");
		rc = thermal_zone_get_temp(g_charger_chip->shell_themal, &temp_val);
		if (rc) {
	...
	if (chip->shell_themal) {
		...
		if (rc) {
			...
			if (rc) {
				...
				chg_err("thermal_zone_get_temp  battery get error");
			} else {
