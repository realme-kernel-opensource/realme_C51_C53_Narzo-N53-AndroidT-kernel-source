...
static ssize_t lcd_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t lcd_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
	...
	if (kstrtos32(buf, 0, &val)) {
		chg_err("lcd_mode_store buf error\n");
		return -EINVAL;
	}
	chip->led_on = val;
    chip->led_on_change = true;
    chg_err("led_on = %d,led_on_change = %d\n",chip->led_on,chip->led_on_change);

	return count;
}
static DEVICE_ATTR_RW(lcd_mode);
...
static ssize_t vchg_status_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct oplus_chg_chip *chip = NULL;
	...
	return sprintf(buf, "%d\n", (chip->vchg_status == CHARGER_STATUS__VOL_HIGH)?true:false);
}
static DEVICE_ATTR_RO(vchg_status);
