...
struct oplus_chg_track_status {
	...
	u32 debug_soc;
	u32 debug_uisoc;
	u32 debug_normal_charging_state;
	u32 debug_fast_prop_status;
	u32 debug_normal_prop_status;
	u32 debug_no_charging;
	u32 debug_slow_charging_force;
	u32 debug_chg_notify_flag;
	u32 debug_chg_notify_code;
	u32 debug_slow_charging_reason;
...
int oplus_chg_track_check_wls_charging_break(int wls_connect)
{
	...
	if (wls_connect && (pre_wls_connect != wls_connect)) {
		track_status->wls_attach_time_ms = 0;//local_clock() / TRACK_LOCAL_T_NS_TO_MS_THD;
		...
		track_status->wls_detach_time_ms = 0;//local_clock() / TRACK_LOCAL_T_NS_TO_MS_THD;
	...
	if (vbus_rising && (pre_vbus_rising != vbus_rising)) {
		...
		if (!chip->chg_ops->check_chrdet_status())
			track_status->chg_attach_time_ms =
				0;//local_clock() / TRACK_LOCAL_T_NS_TO_MS_THD + TRACK_TIME_5MIN_JIFF_THD;
		else
			track_status->chg_attach_time_ms = 0;//local_clock() / TRACK_LOCAL_T_NS_TO_MS_THD;
		...
		if (!track_status->wired_online)
			...
			track_status->chg_detach_time_ms = 0;//local_clock() / TRACK_LOCAL_T_NS_TO_MS_THD;
		if (delayed_work_pending(&track_chip->check_wired_online_work))
			cancel_delayed_work_sync(&track_chip->check_wired_online_work);
		mutex_lock(&track_chip->online_hold_lock);
...
static int oplus_chg_track_debugfs_init(struct oplus_chg_track *track_dev)
{
	...
	debugfs_create_u32("debug_soc", 0644, debugfs_general, &(track_dev->track_status.debug_soc));
	debugfs_create_u32("debug_uisoc", 0644, debugfs_general, &(track_dev->track_status.debug_uisoc));
	debugfs_create_u32("debug_fast_prop_status", 0644, debugfs_general,
			  &(track_dev->track_status.debug_fast_prop_status));
	debugfs_create_u32("debug_normal_charging_state", 0644, debugfs_general,
			  &(track_dev->track_status.debug_normal_charging_state));
	debugfs_create_u32("debug_normal_prop_status", 0644, debugfs_general,
			  &(track_dev->track_status.debug_normal_prop_status));
	debugfs_create_u32("debug_no_charging", 0644, debugfs_general, &(track_dev->track_status.debug_no_charging));
	debugfs_create_u32("debug_slow_charging_force", 0644, debugfs_chg_slow,
			  &(track_dev->track_status.debug_slow_charging_force));
	debugfs_create_u32("debug_slow_charging_reason", 0644, debugfs_chg_slow,
