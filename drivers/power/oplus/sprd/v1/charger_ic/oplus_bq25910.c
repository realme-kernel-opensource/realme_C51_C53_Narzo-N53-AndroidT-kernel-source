...
static int bq25910_online = false;
...
int bq25910_input_current_limit_write(int current_ma)
{
	...
	if (bq25910_get_vbus_voltage() > AICL_POINT_VOL_9V) {
		chip->sw_aicl_point = AICL_POINT_VOL_9V;
		bq25910_set_vindpm_vol(chip->sw_aicl_point);
	} else {
...
int bq25910_get_fast_charging_current(void)
{
	int rc = 0;
	int reg_val = 0;
	struct chip_bq25910 *chip = charger_ic;
	...
	rc = bq25910_read_reg(REG01_BQ25910_ADDRESS, &reg_val);
	if (rc) {
		chg_err("Couldn't read 0x%02x rc = %d\n", REG01_BQ25910_ADDRESS, rc);
		return 0;
	}

	reg_val = (reg_val * REG01_BQ25910_CHARGE_CURRENT_SETTING_STEP);

	chg_err("get fast charge current = %d\n", reg_val);
...
int bq25910_set_enable_term(bool enable)
{
	int rc = 0;
	struct chip_bq25910 *chip = charger_ic;
	u8 val;
	...
	if (enable)
		val = REG05_BQ25910_CHARGE_TERMINATION_EN_ENABLE;
	else
		val = REG05_BQ25910_CHARGE_TERMINATION_EN_DISABLE;
	...
	rc = bq25910_config_interface(REG05_BQ25910_ADDRESS, val, REG05_BQ25910_CHARGE_TERMINATION_EN_MASK);
	if (rc) {
...
int bq25910_set_chging_term_disable(void)
{
	...
	rc = bq25910_set_enable_term(false);
...
void bq25910_dump_registers(void)
{
	...
	/*if(dump_count == DUMP_REG_LOG_CNT_30S) {
		...
		chg_err("bq25910[0-0D]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
...
int bq25910_input_current_limit_without_aicl(int current_ma)
{
	...
	reg_val = (current_ma - REG03_BQ25910_CURRENT_LIMIT_OFFSET) / REG03_BQ25910_CURRENT_LIMIT_STEP;
...
#endif
	.input_current_write_without_aicl = bq25910_input_current_limit_without_aicl,
	.get_charger_current = bq25910_get_fast_charging_current,
};
...
int bq25910_driver_init(void)
{
	return i2c_add_driver(&bq25910_i2c_driver);
}

void bq25910_driver_exit(void)
{
	i2c_del_driver(&bq25910_i2c_driver);
}
#else
