...
bool oplus_sgm41512_get_bc12_done(void)
{
	if(!charger_ic)
	    return false;
...
EXPORT_SYMBOL(oplus_sgm41512_get_bc12_done);
...
EXPORT_SYMBOL(oplus_sgm41512_get_charger_type);
...
int sgm41512_set_vindpm_vol(int vol)
{
	...
	if (((vol - REG06_SGM41512_VINDPM_OFFSET) % REG06_SGM41512_VINDPM_STEP_MV))
		tmp++;
...
int sgm41512_input_current_limit_write(int current_ma)
{
	...
	chg_err("icl_set: %d, pre_icl: %d, pre_icl_index: %d vbus:%d\n", current_ma, pre_icl, pre_icl_index,
	sgm41512_get_charger_vol());
	...
	if (current_ma < 500 || g_oplus_chip->mmi_chg == 0) {
...
aicl_rerun:
	...
	if (g_oplus_chip && g_oplus_chip->mmi_chg == 0){
		chg_err("set icl 100 when mmi_chg = %d\n",g_oplus_chip->mmi_chg);
		rc = sgm41512_input_current_limit_without_aicl(100);
	}else{
		rc = sgm41512_input_current_limit_without_aicl(sgm41512_usb_icl[i]);
	}
...
int sgm41512_unsuspend_charger(void)
{
	...
	if (g_oplus_chip) {
		...
		result = sgm41512_enable_charging();
		} else {
		chg_err("set icl 100 when mmi_chg = %d\n",g_oplus_chip->mmi_chg);
		sgm41512_input_current_limit_without_aicl(100);
		}
...
static int sgm41512_request_dpdm(struct chip_sgm41512 *chip, bool enable)
{
	...
	if (!chip)
		return 0;
	if(enable){
		//gpio_direction_output(chip->dpdm_switch, 1);//
		//oplus_sgm41512_set_chargerid_switch_val(0);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))     
		func = (int (*)(void))
			module_kallsyms_lookup_name("sprd_hsphy_set_dpdm_high_impedance_state");
		if (func){
           (*func)();
           	chg_err("sgm41512_request_dpdm=%d\n", enable);
          }
#else
		ret = sprd_hsphy_set_dpdm_high_impedance_state();
#endif
	}else{
		//gpio_direction_output(chip->dpdm_switch, 0);
        //oplus_sgm41512_set_chargerid_switch_val(1);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
			func = (int (*)(void))
	...
	chg_err("sgm41512_request_dpdm enable = %d\n", enable);
#if 0
...
int sgm41512_otg_enable_charger(struct regulator_dev *dev)
{
	int rc = 0;
	struct chip_sgm41512 *chip = rdev_get_drvdata(dev);
	chg_err("sgm41512_otg_enable_charger\n");
	...
	chg_err("22sgm41512_otg_enable_charger\n");
...
int sgm41512_otg_disable_charger(struct regulator_dev *dev)
{
	int rc = 0;
	struct chip_sgm41512 *chip = rdev_get_drvdata(dev);
	chg_err("sgm41512_otg_disable_charger\n");
...
{
	struct chip_sgm41512 *info = rdev_get_drvdata(dev);
	int ret;
	int val;

	ret = sgm41512_read_reg(info, REG01_SGM41512_ADDRESS, &val);
	if (ret) {
		dev_err(info->dev, "failed to get sgm41512 otg status\n");
		return ret;
	}

	val &= REG01_SGM41512_OTG_MASK;
	chg_err("sgm41512_charger_vbus_is_enabled %d\n", val);

	return val;
}

static const struct regulator_ops sgm41512_charger_vbus_ops = {
	.enable = sgm41512_otg_enable_charger,
	.disable = sgm41512_otg_disable_charger,
...
static int sgm41512_charger_register_vbus_regulator(struct chip_sgm41512 *info)
{
		...
		const struct i2c_device_id *id)
{
	...
	ret = sgm41512_charger_register_vbus_regulator(chip);
	if (ret) {
...
static void sgm41512_charger_shutdown(struct i2c_client *client)
{
	...
	sgm41512_set_wdt_timer(REG05_SGM41512_WATCHDOG_TIMER_DISABLE);
	sgm41512_charging_current_write_fast(360);
	sgm41512_disable_charging();
}
