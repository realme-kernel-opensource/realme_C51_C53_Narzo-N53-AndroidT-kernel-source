...
static int hl7138_dump_reg = 0;

module_param(hl7138_dump_reg, int, 0644);
MODULE_PARM_DESC(hl7138_dump_reg, "dump some hl7138 reg");
...
static void hl7138_update_data(struct oplus_voocphy_manager *chip)
{
	...
	chg_err("cp_ichg = %d cp_vbus = %d, cp_vsys = %d, cp_vbat = %d, int_flag = %d",
...
/* turn off system clk for chipid = 0x43 only */
static int hl7138_turnoff_sys_clk(struct oplus_voocphy_manager *chip)
{
	...
	if (0x43 == i2c_smbus_read_byte_data(chip->client, 0x00)) {
REENABLE:
		/* test mode unlock & lock avoid burnning out the chip */
		hl7138_write_byte(chip->client, HL7138_REG_A0, 0xF9);
		hl7138_write_byte(chip->client, HL7138_REG_A0, 0x9F);	/* Unlock test register */
		...
		/* check A3 ==0x01 ,0xA0== 0x00 if not should retry 3 times */
		if ((reg_data[0] != 0x01) || (reg_data[1] != 0x00)) {
			retry++;
			chg_err("reg err retry times = %d;0xA3=0x%02x,0xA0=0x%02x\n", retry,reg_data[0], reg_data[1]);
			if (retry <= 3)
				goto REENABLE;
		}
		chg_err("hl7138_turnoff_sys_clk done\n");
	}
	return 0;
}
/* turn on system clk for chipid = 0x43 only */
...
REENABLE:
		...
		hl7138_read_byte(chip->client, HL7138_REG_A3, &reg_data[0]);
		hl7138_read_byte(chip->client, HL7138_REG_A0, &reg_data[1]);
		chg_err("0xA3=0x%02x,0xA0=0x%02x\n", reg_data[0], reg_data[1]);
		/* check A3 ==0x00 ,0xA0== 0x00 if not should retry 3 times */
		if ((reg_data[0] != 0x00) || (reg_data[1] != 0x00)) {
			retry++;
			chg_err("reg err retry times = %d;0xA3=0x%02x,0xA0=0x%02x\n", retry,reg_data[0], reg_data[1]);
			if (retry <= 3)
				goto REENABLE;
		...
		hl7138_write_byte(chip->client, HL7138_REG_40, 0x0D);	//set default mode;
		hl7138_write_byte(chip->client, HL7138_REG_14, 0xC8);	//set default mode;
		mdelay(2);
		chg_err("hl7138_turnon_sys_clk done\n");
	}
	return 0;
}
...
int hl7138_init_vooc(struct oplus_voocphy_manager *chip)
{
	...
	if (hl7138_dump_reg){
		hl7138_read_byte(chip->client, HL7138_REG_01, &reg_data[0]);
		hl7138_read_byte(chip->client, HL7138_REG_03, &reg_data[1]);
		hl7138_read_byte(chip->client, HL7138_REG_04, &reg_data[2]);
		hl7138_read_byte(chip->client, HL7138_REG_05, &reg_data[3]);
		hl7138_read_byte(chip->client, HL7138_REG_06, &reg_data[4]);
		hl7138_read_byte(chip->client, HL7138_REG_14, &reg_data[5]);
		hl7138_read_byte(chip->client, HL7138_REG_37, &reg_data[6]);
		hl7138_read_byte(chip->client, HL7138_REG_3B, &reg_data[7]);
		hl7138_read_byte(chip->client, HL7138_REG_40, &reg_data[8]);
		hl7138_read_byte(chip->client, HL7138_REG_41, &reg_data[9]);
		chg_err("0x01=0x%02x,0x03=0x%02x,0x04=0x%02x,0x05=0x%02x,0x06=0x%02x,0x14=0x%02x,0x37=0x%02x,0x3B=0x%02x,0x40=0x%02x,0x41=0x%02x\n",
			reg_data[0], reg_data[1], reg_data[2],
			reg_data[3], reg_data[4], reg_data[5],
			reg_data[6], reg_data[7], reg_data[8],
			reg_data[9]);
	}
...
static int hl7138_svooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	hl7138_write_byte(chip->client, HL7138_REG_0B, 0x89);	/* VBUS_OVP:13V */
...
static void hl7138_charger_shutdown(struct i2c_client *client)
{
	...
	if (oplus_voocphy_mg) {
		hl7138_read_byte(oplus_voocphy_mg->client, HL7138_REG_0B, &reg);
		...
		chg_err("%s reg%d\n",__func__, reg);
		hl7138_write_byte(oplus_voocphy_mg->client, HL7138_REG_0B, reg);	/* VBUS_OVP=7V */
		/* turn off system clk for chipid = 0x43 only */
