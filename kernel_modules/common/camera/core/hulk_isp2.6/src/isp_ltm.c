...
static void ispltm_sync_fid_set(void *handle)
{
	...
	ltm_ctx = (struct isp_ltm_ctx_desc *)handle;
	sync = ltm_ctx->sync;
	if (ltm_ctx->sub_img_num)
		fid = ltm_ctx->sub_img_idx + ltm_ctx->fid * ltm_ctx->sub_img_num;
	else
		fid = ltm_ctx->fid;
	atomic_set(&sync->pre_fid, fid);
}
...
static int ispltm_histo_config_gen_soft(struct isp_ltm_ctx_desc *ctx, struct isp_ltm_stat_info *tuning, struct isp_sw_context *pctx)
{
	int ret = 0;
	struct isp_ltm_hist_param hist_param = {0};
	struct isp_ltm_hist_param *param = &hist_param;
	struct isp_ltm_hists *hists = &ctx->hists;
	struct camera_frame *ltm_hist_frame = NULL;
	...
	ispltm_histo_param_calc(param, ISP_LTM_TILE_W_ALIGNMENT);
	...
	ltm_hist_frame = cam_queue_dequeue(&pctx->ltmhist_result_queue, struct camera_frame, list);
	if (!ltm_hist_frame) {
		pr_err("fail to ctx id %d, frame %px\n", ctx->ctx_id, ltm_hist_frame);
		return -1;
	}
	ltm_hist_frame->tile_width = param->tile_width;
	ltm_hist_frame->tile_height = param->tile_height;
	ltm_hist_frame->tile_x_num = param->tile_num_x;
	ltm_hist_frame->tile_y_num = param->tile_num_y;
	ltm_hist_frame->width = ctx->frame_width;
	ltm_hist_frame->height = ctx->frame_height;
	ltm_hist_frame->fid = ctx->fid;
	...
	hists->addr = ltm_hist_frame->buf.iova[0];
	hists->pitch = param->tile_num_x - 1;
	hists->wr_num = param->tile_num_x * 16;

	ret = cam_queue_enqueue(&pctx->ltmhist_out_queue, &ltm_hist_frame->list);
	if (ret) {
		pr_err("ltmhist_out_queue overflow\n");
...
static int ispltm_map_config_gen_soft(struct isp_ltm_ctx_desc *ctx, struct isp_ltm_map_info *tuning, int type)
{
	...
	map->mem_init_addr = ctx->ltm_map_buf.iova[0];
			...
			struct isp_ltm_stat_info *tuning)
{
	...
	if (ctx->sub_img_num)
		idx = (ctx->fid * ctx->sub_img_num + ctx->sub_img_idx) % ISP_LTM_BUF_NUM;
	else
...
static int ispltm_pipe_proc(void *handle, void *param, void *param1)
{
	int ret = 0;
	struct isp_ltm_ctx_desc *ctx = NULL;
	struct isp_ltm_info *ltm_info = NULL;
	struct isp_ltm_sync *sync = NULL;
	struct isp_hw_k_blk_func ltm_cfg_func = {0};
	int pre_fid = 0, map_fid = 0;
	struct isp_hw_ltm_func ltm_func = {0};
	long timeout = 0;
	uint32_t ratio_delta = 0;
	struct isp_sw_context *pctx = NULL;

	if (!handle || !param || !param1) {
	...
	pctx = (struct isp_sw_context *)param1;
	sync = ctx->sync;
	...
	switch (ctx->mode) {
	case MODE_LTM_PRE:
		if (ctx->calc_mode == LTM_SW_CALC) {
			ret = ispltm_histo_config_gen_soft(ctx, &ltm_info->ltm_stat, pctx);
			if (ret < 0) {
				pr_err("faill to preview hist config, ctx id %d, fid %d\n", ctx->ctx_id, ctx->fid);
				break;
			}
			ltm_func.index = ISP_K_LTM_HIST_SET;
			ctx->hw->isp_ioctl(ctx->hw, ISP_HW_CFG_LTM_FUNC_GET, &ltm_func);
			ltm_func.k_blk_func(ctx);

			ispltm_map_config_gen_soft(ctx, &ltm_info->ltm_map, ISP_PRO_LTM_PRE_PARAM);
			ltm_func.index = ISP_K_LTM_MAPPING_SET;
			ctx->hw->isp_ioctl(ctx->hw, ISP_HW_CFG_LTM_FUNC_GET, &ltm_func);
			ltm_func.k_blk_func(ctx);
		} else {
	...
	case MODE_LTM_CAP:
		...
		if (ctx->calc_mode == LTM_SW_CALC) {
			...
			ispltm_map_config_gen_soft(ctx, &ltm_info->ltm_map, ISP_PRO_LTM_CAP_PARAM);
			ltm_func.index = ISP_K_LTM_MAPPING_SET;
			ctx->hw->isp_ioctl(ctx->hw, ISP_HW_CFG_LTM_FUNC_GET, &ltm_func);
			ltm_func.k_blk_func(ctx);
		} else {
			ctx->hists.bypass = 1;
			if (ctx->sub_img_num)
				map_fid = ctx->fid * ctx->sub_img_num + ctx->sub_img_idx;
			else
				map_fid = ctx->fid;
			...
			if (!ctx->map.bypass) {
				while (map_fid > pre_fid) {
					...
					ispltm_sync_completion_set(ctx, map_fid);
				...
				/*
				* Capture size ratio must be similar to preview size ratio
				* Because Capture Stats depend on Preview Stats
				* Ratio_prv(w/h) / Ratio_cap(w/h) should be limited in [0.95, 1.05] according to algorithm
				*/
				ratio_delta = abs((sync->pre_frame_w * ctx->frame_height) * 100 / (sync->pre_frame_h * ctx->frame_width) - 100);
				if (ratio_delta <= 5) {
					pr_debug("fid %d delta %d prv size [%d %d] cap size [%d %d]\n",
						ctx->fid, ratio_delta, sync->pre_frame_w, sync->pre_frame_h, ctx->frame_width, ctx->frame_height);
				} else {
					ctx->map.bypass = 1;
					pr_warn("warning: can not match prv size with cap size, fid %d delta %d prv size [%d %d] cap size [%d %d]\n",
						ctx->fid, ratio_delta, sync->pre_frame_w, sync->pre_frame_h, ctx->frame_width, ctx->frame_height);
				}
			}
			/*
		...
		enum isp_ltm_cfg_cmd cmd, void *param)
{
	...
	struct camera_buf ltm_map_buf = {0};
	...
	case ISP_LTM_CFG_FRAME_ID:
		fid = *(uint32_t *)param;
		if (ltm_ctx->mode == MODE_LTM_PRE) {
			if (ltm_ctx->calc_mode == LTM_SW_CALC) {
				if (fid == 0) {
					pr_debug("sw ltm_ctx->fid %d fid %d\n",ltm_ctx->fid, fid);
				}
			} else {
				if (((fid - ltm_ctx->fid) != 1) || (fid == 0)) {
					pr_debug("ltm_ctx->fid %d fid %d\n",ltm_ctx->fid, fid);
	...
	case ISP_LTM_CFG_MAP_BYPASS:
		...
		break;
	case ISP_LTM_CFG_CALC_MODE:
		ltm_ctx->calc_mode |= *(uint32_t *)param;
		pr_debug("LTM ctx_id %d, calc mode %d\n", ltm_ctx->ctx_id, ltm_ctx->calc_mode);
		break;
	case ISP_LTM_CFG_CALC_MAP_BUF:
		ltm_map_buf = *(struct camera_buf *)param;
		if (ltm_map_buf.mapping_state & CAM_BUF_MAPPING_DEV)
			ltm_ctx->ltm_map_buf = ltm_map_buf;
		pr_debug("LTM ctx_id %d, ltm_ctx->ltm_map_frame->buf %px iova %px\n", ltm_ctx->ctx_id, ltm_ctx->ltm_map_buf, ltm_ctx->ltm_map_buf.iova[0]);
		break;
	case ISP_LTM_CFG_SUB_IMG_NUM:
		ltm_ctx->sub_img_num = (*(uint32_t *)param);
		break;
	case ISP_LTM_CFG_SUB_IMG_IDX:
		ltm_ctx->sub_img_idx = (*(uint32_t *)param);
		break;
	default:
...
int isp_ltm_map_slice_config_gen_soft(struct isp_ltm_ctx_desc *ctx, struct isp_ltm_rtl_param *prtl, uint32_t *slice_info, void *ltminfo)
{
	struct isp_ltm_hist_param map_param = {0};
	struct isp_ltm_hist_param *param = &map_param;
	struct isp_ltm_tile_num_minus1 mnum = {0};
