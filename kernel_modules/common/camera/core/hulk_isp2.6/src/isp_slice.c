	...
	for (j = 0; j < ISP_SPATH_FD; j++) {
		if (slc_cfg_input->calc_dyn_ov.path_en[j] == 0) {
			...
			for (i = 0; i < slice_ctx->slice_num; i++) {
				...
				//cur_slc->slice_scaler[j]->scaler_bypass = slc_cfg_input->frame_scaler[j]->scaler_bypass;
				if (slc_cfg_input->calc_dyn_ov.path_scaler[j]->scaler.scaler_bypass) {
					if (slc_cfg_input->sw_slice_num) {
						...
						pr_debug("start %d %d end %d %d, frm trim0 %d %d %d %d\n", start_col, start_row, end_col, end_row, frm_trim0->start_x, frm_trim0->start_y, frm_trim0->size_x, frm_trim0->size_y);
						if (cur_slc->x == 0) {
						...
						cur_slc->slice_scaler[j].src_size_x = cur_slc->slice_scaler[j].trim0_size_x;
						cur_slc->slice_scaler[j].src_size_y = cur_slc->slice_scaler[j].trim0_size_y;
	...
	for (i = 0; i < SLICE_NUM_MAX; i++, cur_slc++) {
		...
		for (j = 0; j < ISP_SPATH_NUM; j++) {
			...
			if (j == ISP_SPATH_FD) {
				...
				if (cur_slc->x == 0)
					start_col_out[j][cur_slc->x] = in_ptr->frame_trim0[j]->start_x;
				if (cur_slc->y == 0)
					start_row_out[j][cur_slc->y] = in_ptr->frame_trim0[j]->start_y;
				if ((cur_slc->y == 0) && ((cur_slc->x + 1) < SLICE_W_NUM_MAX))
					start_col_out[j][cur_slc->x + 1] = slc_store->size.w + start_col_out[j][cur_slc->x];
				pr_debug("slice %d, path %d,  size(w %d, h %d), overlap: (%d %d %d %d), cur_slc(x %d, y %d)  start_col_out[j][cur_slc->x] %d, start_row_out[j][cur_slc->y] %d\n",
					i, j, slc_store->size.w, slc_store->size.h,
					overlap_up, overlap_down, overlap_left, overlap_right,
					cur_slc->x, cur_slc->y, start_col_out[j][cur_slc->x], start_row_out[j][cur_slc->y]);
			} else {
				start_col = cur_slc->slice_pos.start_col;
				start_row = cur_slc->slice_pos.start_row;
				end_col = cur_slc->slice_pos.end_col;
				...
				if (cur_slc->y == (slc_ctx->slice_row_num - 1))
					slc_store->border.down_border = 0;

				if (cur_slc->x == 0)
					slc_store->border.left_border =  0;

				if (cur_slc->x == (slc_ctx->slice_col_num - 1))
...
static int ispslice_ltm_info_cfg(struct isp_ltm_ctx_desc *ltm_ctx,
		struct isp_slice_context *slc_ctx, enum isp_ltm_region ltm_id, void *cfg_in)
{
	int ret = 0, idx = 0;
	...
	for (idx = 0; idx < SLICE_NUM_MAX; idx++) {
		...
		if (ltm_ctx->calc_mode == LTM_SW_CALC)
			isp_ltm_map_slice_config_gen_soft(ltm_ctx, prtl, slice_info, &in_ptr->nofilter_ctx->ltm_rgb_info);
		else
		...
		pr_debug("ltm slice info: tile_num_x[%d], tile_num_y[%d], tile_right_flag[%d] \
			tile_left_flag[%d], tile_start_x[%d], tile_start_y[%d], mem_addr[0x%x], start addr %x\n",
...
int isp_slice_info_cfg(void *cfg_in, struct isp_slice_context *slc_ctx)
{
	...
	if (in_ptr->ltm_rgb_eb)
		ispslice_ltm_info_cfg(in_ptr->rgb_ltm, slc_ctx, LTM_RGB, cfg_in);
	if (in_ptr->ltm_yuv_eb)
		ispslice_ltm_info_cfg(in_ptr->yuv_ltm, slc_ctx, LTM_YUV, cfg_in);
...
int isp_slice_update(void *pctx_handle, uint32_t ctx_id, uint32_t slice_id)
{
	...
	ltm.ltm_id = ctx_id;
	ltm.map = &cur_slc->slice_ltm_map[LTM_RGB];
	hw->isp_ioctl(hw, ISP_HW_CFG_SLICE_LTM_INFO, &ltm);
	fetch.ctx_id = ctx_id;
