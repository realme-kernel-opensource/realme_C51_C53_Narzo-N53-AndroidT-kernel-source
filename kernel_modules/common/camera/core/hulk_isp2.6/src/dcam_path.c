		...
		void *param)
{
	...
	case DCAM_PATH_FULL:
		...
		dcam_sw_ctx->slc_splice_mode = ch_desc->slc_splice_mode;
		pr_debug("path %d splice mode %d\n", path->path_id, dcam_sw_ctx->slc_splice_mode);
			...
			struct camera_frame *frame, struct dcam_hw_slw_fmcu_cmds *slw, uint32_t idx)
{
	...
	if (slw == NULL) {
		if (frame->is_compressed) {
			...
			if ((ctx->slice_count == ctx->slice_num) || (ctx->slc_splice_mode != ISP_SPLICE))
				hw->dcam_ioctl(hw, DCAM_HW_CFG_STORE_ADDR, &store_arg);
			else
				pr_info("fail to set path %d buf, count %d num %d\n", idx, ctx->slice_count, ctx->slice_num);
		}
