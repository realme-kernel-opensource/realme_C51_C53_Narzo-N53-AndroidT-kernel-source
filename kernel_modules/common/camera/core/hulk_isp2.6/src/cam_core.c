...
struct camera_uinfo {
	...
	uint32_t need_nonzsl_cap_statis;
...
struct channel_context {
	...
	int32_t isp_ctx_id;
	int32_t isp_path_id;
	int32_t ultra_ltm_hist_isp_ctx_id;
	int32_t ultra_ltm_hist_isp_path_id;
	...
	int32_t ltm_subimg_idx;
	int32_t ltm_statis_sub_img_idx;
...
static int camcore_resframe_set(struct camera_module *module)
{
	...
	for (i = 0; i < CAM_CH_MAX; i++) {
		...
		if (ch->isp_path_id >= 0 && ch->ch_uinfo.dst_fmt != IMG_PIX_FMT_GREY) {
			...
			if (ch->ultra_ltm_hist_isp_ctx_id >= 0) {
				struct camera_frame *pframe2 = NULL;
				pframe2 = cam_queue_empty_frame_get();
				pframe2->is_reserved = 1;
				pframe2->buf.type = CAM_BUF_USER;
				pframe2->buf.mfd[0] = pframe->buf.mfd[0];
				pframe2->buf.offset[0] = pframe->buf.offset[0];
				pframe2->buf.offset[1] = pframe->buf.offset[1];
				pframe2->buf.offset[2] = pframe->buf.offset[2];
				pframe2->channel_id = ch->ch_id;
				ret = cam_buf_ionbuf_get(&pframe2->buf);
				if (ret) {
					pr_err("fail to get ionbuf on cam%d, ch %d\n", module->idx, i);
					cam_queue_empty_frame_put(pframe2);
					ret = -EFAULT;
					break;
				}
				for (j = 0; j < 3; j++)
					pframe2->buf.size[j] = max_size;
				ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle, cmd,
					ch->ultra_ltm_hist_isp_ctx_id, ch->ultra_ltm_hist_isp_path_id, pframe2);
			}
		} else {
			cmd = DCAM_PATH_CFG_OUTPUT_RESERVED_BUF;
			for (j = 0; j < 3; j++)
	...
	uint32_t index)
{
	...
	struct camera_frame **buf_array = NULL;
	...
	if ((!ch_pre->enable && ch_cap->enable && (ch->ultra_ltm_hist_isp_ctx_id < 0)) || (index == CAM_CH_VID && ch_pre->enable))
		return 0;
	...
	if (ch_pre->enable)
		buf_array = &ch_pre->ltm_bufs[LTM_RGB][0];
	else if (index == CAM_CH_CAP)
		buf_array = &ch_cap->ltm_bufs[LTM_RGB][0];
	if (module->cam_uinfo.is_rgb_ltm) {
		for (j = 0; j < ISP_LTM_BUF_NUM; j++) {
			ch->ltm_bufs[LTM_RGB][j] = buf_array[j];
			...
			if (ch->ultra_ltm_hist_isp_ctx_id >= 0)
				ret |= module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
					ISP_PATH_CFG_RGB_LTM_BUF,
					ch->ultra_ltm_hist_isp_ctx_id, ch->ultra_ltm_hist_isp_path_id,
					ch->ltm_bufs[LTM_RGB][j]);
			if (ret) {
	...
	if (channel->mode_ltm != MODE_LTM_OFF) {
		...
		if (channel->ltm_rgb) {
			for (i = 0; i < ISP_LTM_BUF_NUM; i++) {
				if (channel->ch_id == CAM_CH_PRE ||
					((channel_pre->enable == 0) && (channel->ultra_ltm_hist_isp_ctx_id >= 0))) {
					pframe = cam_queue_empty_frame_get();
...
static int camcore_4in1_aux_deinit(struct camera_module *module)
{
	...
	module->dcam_dev_handle->dcam_pipe_ops->close(module->aux_dcam_dev);
	dcam_core_pipe_dev_put(module->aux_dcam_dev);
	dcam_ctx->slice_num = 0;
	dcam_ctx->slice_count = 0;
	module->aux_dcam_dev = NULL;
	module->aux_dcam_id = DCAM_HW_CONTEXT_MAX;
...
*/
static void camcore_nonzsl_xtm_img_cut_proc(struct camera_module *module,
		struct camera_frame *pframe, struct channel_context *channel)
{
	int ret = 0;
	struct camera_frame *pframe1 = NULL;
	uint32_t ratio = 2, align_coeff_w = 1, align_coeff_h = 1, width_aligned = 0, height_aligned = 0;
	...
	uint32_t ltm_subimg_idx = 0, ltm_statis_subimg_idx = 0;
	uint32_t width = 0, height = 0, dcam_output_end_pos_x = 0, dcam_output_end_pos_y = 0;
	uint32_t subimg_end_x = 0, subimg_end_y = 0;
	uint32_t tile_x_align_coeff = 1, tile_y_align_coeff = 1;
	uint32_t frm_trim_offset_x = 0, frm_trim_offset_y = 0;
	struct img_trim sub_img_trim[ISP_LARGE_LTM_SUBIMG_NUM] = {0};
	struct img_trim sub_img_out_trim[ISP_LARGE_LTM_SUBIMG_NUM] = {0};
	struct isp_ctx_size_desc ctx_size = {0};
	struct img_trim path_trim = {0};
	...
	if (pframe->nonzsl_xtm.use_for_xtm_hist == 0) {
		if (height > DCAM_SW_SLICE_HEIGHT_MAX)
			ratio = 4;
		align_coeff_w = 2 * ratio * 14;
		align_coeff_h = 2 * ratio * 14;
		width_aligned = (width / align_coeff_w) * align_coeff_w;
		height_aligned = (height / align_coeff_h) * align_coeff_h;
		...
		pr_info("large tile size %d %d, align size %d %d\n", tile_x, tile_y, width_aligned, height_aligned);
		pr_info("large img, (%d %d %d %d) (%d %d %d %d) (%d %d %d %d) (%d %d %d %d)\n",
			sub_img_trim[0].start_x, sub_img_trim[0].start_y, sub_img_trim[0].size_x, sub_img_trim[0].size_y,
			sub_img_trim[1].start_x, sub_img_trim[1].start_y, sub_img_trim[1].size_x, sub_img_trim[1].size_y,
			sub_img_trim[2].start_x, sub_img_trim[2].start_y, sub_img_trim[2].size_x, sub_img_trim[2].size_y,
			sub_img_trim[3].start_x, sub_img_trim[3].start_y, sub_img_trim[3].size_x, sub_img_trim[3].size_y);
	} else {
		uint32_t large_size_x = 0, large_size_y = 0;
		large_size_x = pframe->nonzsl_xtm.large_size_x;
		large_size_y = pframe->nonzsl_xtm.large_size_y;
		if (large_size_y > DCAM_SW_SLICE_HEIGHT_MAX)
			ratio = 4;
		align_coeff_w = 2 * ratio * 14;
		align_coeff_h = 2 * ratio * 14;
		width_aligned = (large_size_x / align_coeff_w) * align_coeff_w;
		height_aligned = (large_size_y / align_coeff_h) * align_coeff_h;
		bin_width_aligned = width_aligned / ratio;
		bin_height_aligned = height_aligned / ratio;
		tile_x = (width_aligned + tile_x_align_coeff - 1) / tile_x_align_coeff * ISP_LTM_TILE_W_ALIGNMENT;
		tile_y = (height_aligned + tile_y_align_coeff - 1) / tile_y_align_coeff * ISP_LTM_TILE_H_ALIGNMENT;
		img0_size_x = 8 * tile_x / ratio;
		img0_size_y = 8 * tile_y / ratio;
		...
		sub_img_trim[3].size_x = img0_size_x;
		sub_img_trim[3].size_y = img0_size_y;
		sub_img_out_trim[0].size_x = sub_img_trim[0].size_x;
		sub_img_out_trim[0].size_y = sub_img_trim[0].size_y;
		sub_img_out_trim[1].size_x = sub_img_trim[1].size_x;
		sub_img_out_trim[1].size_y = sub_img_trim[1].size_y;
		sub_img_out_trim[2].size_x = sub_img_trim[2].size_x;
		sub_img_out_trim[2].size_y = sub_img_trim[2].size_y;
		sub_img_out_trim[3].size_x = sub_img_trim[3].size_x;
		sub_img_out_trim[3].size_y = sub_img_trim[3].size_y;
		pr_info("ratio %d, tile size %d %d, align bin size %d %d\n", ratio, tile_x, tile_y, bin_width_aligned, bin_height_aligned);
		pr_info("bin img, (%d %d %d %d) (%d %d %d %d) (%d %d %d %d) (%d %d %d %d)\n",
			sub_img_trim[0].start_x, sub_img_trim[0].start_y, sub_img_trim[0].size_x, sub_img_trim[0].size_y,
			sub_img_trim[1].start_x, sub_img_trim[1].start_y, sub_img_trim[1].size_x, sub_img_trim[1].size_y,
			sub_img_trim[2].start_x, sub_img_trim[2].start_y, sub_img_trim[2].size_x, sub_img_trim[2].size_y,
			sub_img_trim[3].start_x, sub_img_trim[3].start_y, sub_img_trim[3].size_x, sub_img_trim[3].size_y);
	...
	dcam_output_end_pos_x = pframe->slice_trim.start_x + pframe->slice_out_trim.start_x + pframe->slice_out_trim.size_x;
	dcam_output_end_pos_y = pframe->slice_trim.start_y + pframe->slice_out_trim.start_y + pframe->slice_out_trim.size_y;
	ltm_subimg_idx = channel->ltm_subimg_idx;
	ltm_statis_subimg_idx = channel->ltm_statis_sub_img_idx;
	if (pframe->nonzsl_xtm.use_for_xtm_hist) {
		pr_debug("bin img enter\n");
		if (ltm_statis_subimg_idx >= ISP_LARGE_LTM_SUBIMG_NUM) {
			pr_err("fail to get target bin subimg idx, %d\n", ltm_statis_subimg_idx);
			cam_queue_empty_frame_put(pframe);
			return;
		}
		dcam_output_end_pos_x = dcam_output_end_pos_x / ratio;
		dcam_output_end_pos_y = dcam_output_end_pos_y / ratio;
		if (pframe->sw_slice_no == 0) {
			ctx_size.src.w = width;
			ctx_size.src.h = height;
			ctx_size.crop.start_x = frm_trim_offset_x;
			ctx_size.crop.start_y = frm_trim_offset_y;
			ctx_size.zoom_conflict_with_ltm = module->cam_uinfo.zoom_conflict_with_ltm;
			memset(&path_desc, 0, sizeof(path_desc));
			path_desc.out_fmt = channel->ch_uinfo.dst_fmt;
			path_desc.endian.y_endian = ENDIAN_LITTLE;
			path_desc.endian.uv_endian = ENDIAN_LITTLE;
			path_desc.regular_mode = channel->ch_uinfo.regular_desc.regular_mode;
			path_desc.data_bits = DCAM_STORE_10_BIT;
			path_trim.start_x = 0;
			path_trim.start_y = 0;
			ctx_size.crop.size_x = bin_width_aligned;
			ctx_size.crop.size_y = bin_height_aligned;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_CTX_SIZE, channel->ultra_ltm_hist_isp_ctx_id, 0, &ctx_size);
			path_desc.output_size.w = bin_width_aligned;
			path_desc.output_size.h = bin_height_aligned;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_PATH_BASE, channel->ultra_ltm_hist_isp_ctx_id,
				channel->ultra_ltm_hist_isp_path_id, &path_desc);
			path_trim.size_x = bin_width_aligned;
			path_trim.size_y = bin_height_aligned;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_PATH_SIZE, channel->ultra_ltm_hist_isp_ctx_id,
				channel->ultra_ltm_hist_isp_path_id, &path_trim);
		}
		subimg_end_x = sub_img_trim[ltm_statis_subimg_idx].start_x + sub_img_trim[ltm_statis_subimg_idx].size_x;
		subimg_end_y = sub_img_trim[ltm_statis_subimg_idx].start_y + sub_img_trim[ltm_statis_subimg_idx].size_y;
		pr_debug("%d %d, target %d %d\n", dcam_output_end_pos_x, dcam_output_end_pos_y, subimg_end_x, subimg_end_y);
		if (dcam_output_end_pos_y >= subimg_end_y &&
			dcam_output_end_pos_x >= subimg_end_x) {
			pframe1 = cam_queue_empty_frame_get();
			memcpy(pframe1, pframe, sizeof(struct camera_frame));
			pframe1->sw_slice_num = ISP_LARGE_LTM_SUBIMG_NUM;
			pframe1->sw_slice_no = ltm_statis_subimg_idx;
			pframe1->slice_trim = sub_img_trim[ltm_statis_subimg_idx];
			pframe1->slice_out_trim = sub_img_out_trim[ltm_statis_subimg_idx];
			pframe1->nonzsl_xtm.tile_x = tile_x;
			pframe1->nonzsl_xtm.tile_y = tile_y;
			pframe1->not_srcbuf = 0;
			ret = module->isp_dev_handle->isp_ops->proc_frame(module->isp_dev_handle, pframe1,
					channel->ultra_ltm_hist_isp_ctx_id);
			pr_info("send bin sub img %d to isp\n", ltm_statis_subimg_idx);
			channel->ltm_statis_sub_img_idx++;
		}
	} else {
		pr_debug("full img enter\n");
		if (ltm_subimg_idx >= ISP_LARGE_LTM_SUBIMG_NUM) {
			pr_err("fail to get target full subimg idx, %d\n", ltm_subimg_idx);
			cam_queue_empty_frame_put(pframe);
			return;
		}
		if (pframe->sw_slice_no == 0) {
			...
			path_trim.start_x = 0;
			path_trim.start_y = 0;
			ctx_size.crop.size_x = width_aligned;
			ctx_size.crop.size_y = height_aligned;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_CTX_SIZE, channel->isp_ctx_id, 0, &ctx_size);
			path_desc.output_size.w = width_aligned;
			path_desc.output_size.h = height_aligned;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_PATH_BASE, channel->isp_ctx_id,
				channel->isp_path_id, &path_desc);
			path_trim.size_x = width_aligned;
			path_trim.size_y = height_aligned;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_PATH_SIZE, channel->isp_ctx_id,
				channel->isp_path_id, &path_trim);
		}
		subimg_end_x = sub_img_trim[ltm_subimg_idx].start_x + sub_img_trim[ltm_subimg_idx].size_x;
		subimg_end_y = sub_img_trim[ltm_subimg_idx].start_y + sub_img_trim[ltm_subimg_idx].size_y;
		pr_debug("%d %d, target %d %d\n", dcam_output_end_pos_x, dcam_output_end_pos_y, subimg_end_x, subimg_end_y);
		if (dcam_output_end_pos_y >= subimg_end_y &&
			dcam_output_end_pos_x >= subimg_end_x) {
			pframe1 = cam_queue_empty_frame_get();
			memcpy(pframe1, pframe, sizeof(struct camera_frame));
			pframe1->sw_slice_num = ISP_LARGE_LTM_SUBIMG_NUM;
			pframe1->sw_slice_no = ltm_subimg_idx;
			pframe1->slice_trim = sub_img_trim[ltm_subimg_idx];
			pframe1->slice_out_trim = sub_img_out_trim[ltm_subimg_idx];
			pframe1->nonzsl_xtm.tile_x = tile_x;
			pframe1->nonzsl_xtm.tile_y = tile_y;
			pframe1->not_srcbuf = 0;
			cam_queue_enqueue(&channel->share_buf_queue, &pframe1->list);
			pr_info("send full sub img %d to isp in_queue\n", ltm_subimg_idx);
			channel->ltm_subimg_idx++;
		}
	}
...
{
	int ret;
	uint32_t start_offline_proc = 0;
	struct camera_frame *msg = NULL;
	struct dcam_sw_context *dcam_sw_ctx = NULL;
	struct dcam_sw_context *dcam_sw_aux_ctx = NULL;
	...
	if (pframe->irq_type == CAMERA_IRQ_SUPERSIZE_DONE) {
		...
		if (module->dcam_cap_status == DCAM_CAPTURE_START_FROM_NEXT_SOF
			...
			start_offline_proc = 1;
		if (module->dcam_cap_status != DCAM_CAPTURE_START_FROM_NEXT_SOF)
			start_offline_proc = 1;
		if (!start_offline_proc) {
			/* set buffer back to dcam0 full path, to out_buf_queue */
		...
		if (dev->hw->ip_dcam[0]->save_band_for_bigsize || (channel->ultra_ltm_hist_isp_ctx_id >= 0)) {
			uint32_t shutoff = 1;
			struct dcam_hw_path_stop patharg;
			patharg.path_id = channel->dcam_path_id;
			patharg.idx = module->dcam_dev_handle->sw_ctx[module->cur_sw_ctx_id].hw_ctx_id;
		...
		if (ret == 0) {
			msg = cam_queue_empty_frame_get();
			if (msg) {
				msg->evt = IMG_TX_DONE;
				msg->irq_type = CAMERA_IRQ_SUPERSIZE_DONE;
				ret = cam_queue_enqueue(&module->frm_queue, &msg->list);
				if (ret)
					cam_queue_empty_frame_put(msg);
				else
					complete(&module->frm_com);
			}
		} else {
		...
		pr_info("[finish] frame[%d] fd %d, size[%d %d], 0x%x, channel_id %d %d\n",
			pframe->fid, pframe->buf.mfd[0], pframe->width,
			pframe->height, (uint32_t)pframe->buf.addr_vir[0], pframe->channel_id, pframe->need_ltm_map);

		if (channel->ultra_ltm_hist_isp_ctx_id >= 0) {
			camcore_nonzsl_xtm_img_cut_proc(module, pframe, channel);
			return NULL;
		} else {
...
static int camcore_isp_callback(enum isp_cb_type type, void *param, void *priv_data)
{
	...
	case ISP_CB_RET_SRC_BUF:
		...
		if ((atomic_read(&module->state) != CAM_RUNNING) ||
			...
			if (pframe->nonzsl_xtm.use_for_xtm_hist) {
				struct camera_frame *pframe1 = NULL;
				pframe1 = cam_queue_dequeue(&channel->share_buf_queue, struct camera_frame, list);
				if (pframe1)
					ret = module->isp_dev_handle->isp_ops->proc_frame(module->isp_dev_handle, pframe1,
						channel->isp_ctx_id);
			}

			if ((pframe->sw_slice_no + 1) < pframe->sw_slice_num)
				cam_queue_empty_frame_put(pframe);
			else {
				if (pframe->nonzsl_xtm.use_for_xtm_hist) {
					struct cam_dump_ctx dump_base = {0};
					dump_base.ch_id = pframe->channel_id;
					dump_base.dcam_out_bits = 10;
					dump_base.dcam_out_fmt = DCAM_STORE_RAW_BASE;
					dump_base.pack_bits = CAM_RAW_PACK10;
					dump_base.is_pack = 1;
		...
		if (atomic_read(&module->state) == CAM_RUNNING) {
			if (module->cap_status == CAM_CAPTURE_RAWPROC) {
				if (pframe->buf.type == CAM_BUF_KERNEL) {
					...
					pr_info("raw proc return dst frame %px, %d x %d\n", pframe, pframe->width, pframe->height);
					cam_buf_ionbuf_put(&pframe->buf);
...
static int camcore_dcam_callback(enum dcam_cb_type type, void *param, void *priv_data)
{
	...
	case DCAM_CB_IRQ_EVENT:
		if (pframe->irq_property == IRQ_DCAM_SN_EOF && module->cam_uinfo.ipg_skip_first_frm == 0) {
			cam_queue_empty_frame_put(pframe);
			break;
		}
		...
		struct channel_context *channel)
{
	...
	if (module->cam_uinfo.need_nonzsl_cap_statis) {
		ret = module->dcam_dev_handle->dcam_pipe_ops->get_path(&dev->sw_ctx[module->offline_cxt_id],
			DCAM_PATH_BIN);
		if (ret)
			pr_err("fail to get bin path for ltm hist\n");

		ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(&dev->sw_ctx[module->offline_cxt_id],
			DCAM_PATH_CFG_BASE, DCAM_PATH_BIN, &ch_desc);
	}
...
static int camcore_virtualsensor_bigsize_aux_init(struct camera_module *module,
		struct channel_context *channel)
{
	int ret = 0;
	struct dcam_path_cfg_param ch_desc;
	...
	ch_desc.is_4in1 = module->cam_uinfo.is_4in1;
	dcam_sw_ctx->fetch.fmt= DCAM_STORE_RAW_BASE;
	dcam_sw_ctx->pack_bits = channel->ch_uinfo.sensor_raw_fmt;
	...
	pr_debug("fetch packbit %d, out fmt %d, packbit %d\n",
		dcam_sw_ctx->pack_bits,
		ch_desc.dcam_out_fmt,
		ch_desc.raw_fmt);
	ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(dcam_sw_ctx,
		DCAM_PATH_CFG_BASE, channel->dcam_path_id, &ch_desc);
	channel->dcam_out_fmt = ch_desc.dcam_out_fmt;
	channel->aux_dcam_path_id = channel->dcam_path_id;
	if (module->cam_uinfo.need_nonzsl_cap_statis) {
		ret = module->dcam_dev_handle->dcam_pipe_ops->get_path(dcam_sw_ctx,DCAM_PATH_BIN);
		if (ret)
			pr_err("fail to get bin path for ltm hist\n");

		ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(dcam_sw_ctx,DCAM_PATH_CFG_BASE, DCAM_PATH_BIN, &ch_desc);
...
static int camcore_bigsize_aux_deinit(struct camera_module *module)
{
	...
	if (!module->cam_uinfo.virtualsensor)
		...
		sw_ctx = &module->dcam_dev_handle->sw_ctx[module->cur_sw_ctx_id];
		if (module->cam_uinfo.need_nonzsl_cap_statis) {
			ret = module->dcam_dev_handle->dcam_pipe_ops->put_path(sw_ctx, DCAM_PATH_BIN);
			if (ret) {
	...
	struct channel_context *channel)
{
	...
	uint32_t width = 0, height = 0, pack_bits = 0, size = 0, bin_size = 0, is_pack = 0, dcam_out_bits = 0;
	uint32_t ratio = 1;
	...
	if (!module->cam_uinfo.virtualsensor)
		...
		dcam_sw_aux_ctx = &module->dcam_dev_handle->sw_ctx[module->cur_sw_ctx_id];
	...
	if (height > DCAM_SW_SLICE_HEIGHT_MAX)
		...
		ratio = 2;
	bin_size = cal_sprd_raw_pitch(width / ratio, pack_bits) * height / ratio;
	...
	for (i = 0; i < total; i++) {
		do {
			...
			if (module->cam_uinfo.need_nonzsl_cap_statis == 0)
				continue;
			/* bin buf for xtm statis */
			pframe1 = cam_queue_empty_frame_get();
			pframe1->channel_id = channel->ch_id;
			pframe1->is_compressed = channel->compress_offline;
			pframe1->compress_4bit_bypass =
					channel->compress_4bit_bypass;
			pframe1->width = width;
			pframe1->height = height;
			pframe1->endian = ENDIAN_LITTLE;
			pframe1->pattern = module->cam_uinfo.sensor_if.img_ptn;
			pframe1->buf.buf_sec = 0;
			pframe1->need_ltm_hist = module->cam_uinfo.is_rgb_ltm;
			pframe1->need_ltm_map = module->cam_uinfo.is_rgb_ltm;
			pframe1->need_gtm_hist = module->cam_uinfo.is_rgb_gtm;
			pframe1->need_gtm_map = module->cam_uinfo.is_rgb_gtm;
			pframe1->gtm_mod_en = module->cam_uinfo.is_rgb_gtm;
			if (module->cam_uinfo.is_pyr_rec && channel->ch_id != CAM_CH_CAP)
				pframe1->need_pyr_rec = 1;
			if (module->cam_uinfo.is_pyr_dec && channel->ch_id == CAM_CH_CAP)
				pframe1->need_pyr_dec = 1;
			pframe1->nonzsl_xtm.use_for_xtm_hist = 1;
			pframe1->nonzsl_xtm.large_size_x = width;
			pframe1->nonzsl_xtm.large_size_y = height;
			pframe1->width = width / ratio;
			pframe1->height= height / ratio;
			ret = cam_buf_alloc(&pframe1->buf, bin_size, iommu_enable);
			if (ret) {
				pr_err("fail to alloc buf: %d ch %d\n",
					i, channel->ch_id);
				cam_queue_empty_frame_put(pframe1);
	...
	if (ch_desc.output_size.h > DCAM_SW_SLICE_HEIGHT_MAX)
		ch_desc.slc_splice_mode = ISP_SPLICE;
	else
		ch_desc.slc_splice_mode = DCAM_SPLICE;
	...
	if (module->cam_uinfo.virtualsensor)
		dcam_sw_aux_ctx->slc_splice_mode = ch_desc.slc_splice_mode;

	if (channel->ultra_ltm_hist_isp_ctx_id >= 0) {
		ch_desc.output_size.w = ch_desc.input_size.w / ratio;
		ch_desc.output_size.h = ch_desc.input_size.h / ratio;
		ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(dcam_sw_aux_ctx,
				DCAM_PATH_CFG_SIZE, DCAM_PATH_BIN, &ch_desc);
	}
		...
		struct channel_context *channel)
{
	...
	struct isp_ctx_base_desc ctx_desc = {0};
	struct isp_init_param init_param;
	struct cam_hw_info *hw = NULL;
	struct dcam_sw_context *dcam_sw_ctx = NULL;
	uint32_t format = 0;
	...
	case CAM_CH_CAP:
		if ((ch_uinfo->src_size.w >= DCAM_HW_SLICE_WIDTH_MAX) && (!module->cam_uinfo.virtualsensor)) {
			dcam_path_id = hw->ip_dcam[0]->dcam_offline_path_id;
			module->auto_3dnr = 0;
		} else
			dcam_path_id = DCAM_PATH_FULL;
		if ((module->simulator || module->cam_uinfo.virtualsensor) &&
			!module->channel[CAM_CH_PRE].enable &&
			module->grp->hw_info->prj_id == QOGIRN6pro &&
	...
	case CAM_CH_RAW:
		if (ch_uinfo->src_size.w >= DCAM_HW_SLICE_WIDTH_MAX
			|| module->raw_callback)
	...
	if (new_dcam_path) {
		...
		if (!ch_desc.is_raw) {
			...
			if (dcam_path_id == DCAM_PATH_VCH2) {
				ch_desc.raw_fmt = DCAM_RAW_PACK_10;
				channel->ch_uinfo.sensor_raw_fmt = DCAM_RAW_PACK_10;
			}
		}
	...
	if (channel->ch_id == CAM_CH_CAP && module->cam_uinfo.dcam_slice_mode && !module->cam_uinfo.is_4in1) {
		uint32_t bin_isp_ctx_id = 0, bin_isp_path_id = ISP_SPATH_CP;
		channel_prev = &module->channel[CAM_CH_PRE];
		if ((channel_prev->enable == 0) && (channel->ch_uinfo.src_size.h > DCAM_SW_SLICE_HEIGHT_MAX))
			module->cam_uinfo.need_nonzsl_cap_statis = 1;
		else
			module->cam_uinfo.need_nonzsl_cap_statis = 0;
		if(!module->cam_uinfo.virtualsensor)
			ret = camcore_bigsize_aux_init(module, channel);
		else
			ret = camcore_virtualsensor_bigsize_aux_init(module, channel);
		if (ret < 0) {
			pr_err("fail to init dcam for 4in1, ret = %d\n", ret);
			goto exit;
		}
		if (module->cam_uinfo.need_nonzsl_cap_statis) {
			...
			if (ret < 0) {
				...
				pr_err("fail to get isp context for ultra cap bin img\n");
				goto exit;
			}
			bin_isp_ctx_id = ret;
			module->isp_dev_handle->isp_ops->set_callback(module->isp_dev_handle,
				bin_isp_ctx_id, camcore_isp_callback, module);
			ctx_desc.mode_ltm = MODE_LTM_PRE;
			ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
				ISP_PATH_CFG_CTX_BASE, bin_isp_ctx_id, 0, &ctx_desc);
			ret = module->isp_dev_handle->isp_ops->get_path(module->isp_dev_handle, bin_isp_ctx_id, bin_isp_path_id);
			if (ret < 0) {
				pr_err("fail to get isp path %d from context %d\n", bin_isp_path_id, bin_isp_ctx_id);
				module->isp_dev_handle->isp_ops->put_context(module->isp_dev_handle, bin_isp_ctx_id);
				camcore_bigsize_aux_deinit(module);
				goto exit;
			}
			channel->ultra_ltm_hist_isp_ctx_id = (int32_t)(bin_isp_ctx_id);
			channel->ultra_ltm_hist_isp_path_id = (int32_t)(bin_isp_path_id);
		}
	}
...
static int camcore_raw_proc_done(struct camera_module *module)
{
	...
	if (ch->ultra_ltm_hist_isp_ctx_id >= 0) {
		...
		module->isp_dev_handle->isp_ops->put_path(module->isp_dev_handle,
			ch->ultra_ltm_hist_isp_ctx_id, ch->ultra_ltm_hist_isp_path_id);
		for (i = 0; i < ISP_LTM_BUF_NUM; i++) {
			if (ch->ltm_bufs[LTM_RGB][i])
				camcore_k_frame_put(ch->ltm_bufs[LTM_RGB][i]);
			ch->ltm_bufs[LTM_RGB][i] = NULL;
		}
		module->isp_dev_handle->isp_ops->put_context(module->isp_dev_handle, ch->ultra_ltm_hist_isp_ctx_id);
	}
	...
	ch->ultra_ltm_hist_isp_ctx_id = -1;
	ch->ultra_ltm_hist_isp_path_id = -1;
	...
	dev->sw_ctx[module->offline_cxt_id].slice_num = 0;
	dev->sw_ctx[module->offline_cxt_id].slice_count = 0;
		...
		struct isp_raw_proc_info *proc_info)
{
	...
	if((ch->dcam_path_id == DCAM_PATH_FULL || ch->dcam_path_id == DCAM_PATH_BIN) && module->cam_uinfo.is_4in1 == 1) {
		...
		ch->ch_uinfo.dcam_raw_fmt = ch_desc.raw_fmt;
	} else if ((proc_info->dcam_raw_fmt >= DCAM_RAW_PACK_10) && (proc_info->dcam_raw_fmt < DCAM_RAW_MAX))
		ch_desc.raw_fmt = proc_info->dcam_raw_fmt;
	else {
		ch_desc.raw_fmt = dev->hw->ip_dcam[0]->raw_fmt_support[0];
	...
	if (proc_info->src_size.width >= DCAM_HW_SLICE_WIDTH_MAX) {
		dev->sw_ctx[module->offline_cxt_id].pack_bits = DCAM_RAW_PACK_10;
		ch->ch_uinfo.sensor_raw_fmt = DCAM_RAW_PACK_10;
	}
	...
	if (ch_desc.output_size.h > DCAM_SW_SLICE_HEIGHT_MAX) {
		...
		ch_desc.output_size.w = ch_desc.input_size.w / ratio;
		ch_desc.output_size.h = ch_desc.input_size.h / ratio;
		ret = module->dcam_dev_handle->dcam_pipe_ops->get_path(sw_ctx, DCAM_PATH_BIN);
		if (ret)
			pr_err("fail to get bin path for ltm hist\n");
		...
		ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(sw_ctx,
				DCAM_PATH_CFG_SIZE, DCAM_PATH_BIN, &ch_desc);
		statis_isp_id = module->isp_dev_handle->isp_ops->get_context(module->isp_dev_handle, &init_param);
		if (statis_isp_id < 0) {
			pr_err("fail to get isp context\n");
			goto fail_ispctx;
		}
		ch->ultra_ltm_hist_isp_ctx_id = statis_isp_id;
		module->isp_dev_handle->isp_ops->set_callback(module->isp_dev_handle,
			statis_isp_id, camcore_isp_callback, module);
		ctx_desc.mode_ltm = MODE_LTM_PRE;
		ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
			ISP_PATH_CFG_CTX_BASE, statis_isp_id, 0, &ctx_desc);
		ret = module->isp_dev_handle->isp_ops->get_path(module->isp_dev_handle, statis_isp_id, ISP_SPATH_CP);
		ch->ultra_ltm_hist_isp_path_id = ISP_SPATH_CP;
	}
		...
		struct isp_raw_proc_info *proc_info)
{
	...
	mid_frame->need_ltm_map = module->cam_uinfo.is_rgb_ltm;
	...
	if (ch->ultra_ltm_hist_isp_ctx_id >= 0) {
		...
		struct camera_frame *bin_frame = NULL;
		struct camera_frame *res_frame = NULL;
		bin_width = cal_sprd_raw_pitch(proc_info->src_size.width / ratio, DCAM_RAW_PACK_10);
		bin_height = proc_info->src_size.height / ratio;
		bin_size = bin_width * bin_height;
		bin_size = ALIGN(bin_size, CAM_BUF_ALIGN_SIZE);
		bin_frame = cam_queue_empty_frame_get();
		memcpy(bin_frame, mid_frame, sizeof(struct camera_frame));
		memset(&bin_frame->buf, 0, sizeof(struct camera_buf));
		bin_frame->width = proc_info->src_size.width / ratio;
		bin_frame->height = proc_info->src_size.height / ratio;
		bin_frame->nonzsl_xtm.large_size_x = proc_info->src_size.width;
		bin_frame->nonzsl_xtm.large_size_y = proc_info->src_size.height;
		bin_frame->nonzsl_xtm.use_for_xtm_hist = 1;
		bin_frame->need_ltm_hist = module->cam_uinfo.is_rgb_ltm;
		bin_frame->need_gtm_map = module->cam_uinfo.is_rgb_gtm;
		ret = cam_buf_alloc(&bin_frame->buf, (size_t)bin_size, module->iommu_enable);
		if (ret) {
			pr_err("fail to alloc mid yuv buffer\n");
			goto dcam_out_fail;
		}
		ret |= module->dcam_dev_handle->dcam_pipe_ops->cfg_path(sw_ctx,
			DCAM_PATH_CFG_OUTPUT_BUF, DCAM_PATH_BIN, bin_frame);

		res_frame = cam_queue_empty_frame_get();
		res_frame->channel_id = ch->ch_id;
		bin_size = bin_size * 3 / 2;
		bin_size = ALIGN(bin_size, CAM_BUF_ALIGN_SIZE);
		ret = cam_buf_alloc(&res_frame->buf, bin_size, module->iommu_enable);
		if (ret) {
			pr_err("fail to alloc statis yuv buf, size %d\n", module->idx, bin_size);
			cam_queue_empty_frame_put(res_frame);
		}
		cam_buf_iommu_map(&res_frame->buf, CAM_IOMMUDEV_ISP);
		ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle, ISP_PATH_CFG_OUTPUT_BUF,
			ch->ultra_ltm_hist_isp_ctx_id, ch->ultra_ltm_hist_isp_path_id, res_frame);

		if (ch->ltm_rgb) {
			int i = 0;
			uint32_t ltm_size = 64 * 128 * 2;
			struct camera_frame *ltm_frame = NULL;
			ltm_size = ALIGN(ltm_size, CAM_BUF_ALIGN_SIZE);
			for (i = 0; i < ISP_LTM_BUF_NUM; i++) {
				ltm_frame = cam_queue_empty_frame_get();
				ret = cam_buf_alloc(&ltm_frame->buf, ltm_size, module->iommu_enable);
				if (ret) {
					pr_err("fail to alloc ltm buf: %d ch %d\n", i, ch->ch_id);
					cam_queue_empty_frame_put(ltm_frame);
					continue;
				}
				ch->ltm_bufs[LTM_RGB][i] = ltm_frame;
				ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
					ISP_PATH_CFG_RGB_LTM_BUF,
					ch->isp_ctx_id, ch->isp_path_id,
					ltm_frame);
				ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
					ISP_PATH_CFG_RGB_LTM_BUF,
					ch->ultra_ltm_hist_isp_ctx_id, ch->ultra_ltm_hist_isp_path_id,
		...
		struct isp_raw_proc_info *proc_info)
{
	...
	if (ch_pre->enable) {
		...
		if (!ch_pre->compress_input)
			sw_ctx->fbc_info.is_compress = 0;
	}
	/*for capture path dcam size config*/
	ch_cap = &module->channel[CAM_CH_CAP];
	if (ch_cap->enable) {
		if (module->simu_fid >= 1 && module->cam_uinfo.dcam_slice_mode) {
			pr_debug("big size only process one frame\n");
			return 0;
		}
	...
	if (ret) {
		...
		goto virtual_sensor_proc_fail;
	}


	return ret;
virtual_sensor_proc_fail:
	cam_buf_ionbuf_put(&src_frame->buf);
...
static int camcore_csi_switch_connect(struct camera_module *module, uint32_t mode)
{
	...
	csi_switch.csi_id = module->dcam_idx;
	csi_switch.dcam_id= sw_ctx->hw_ctx_id;
	csi_switch.mipi_cap_pre_skip_num = sw_ctx->cap_info.frm_skip;
...
static int camcore_offline_proc(void *param)
{
	...
	if (need_slice) {
		dcam_offline_slice_info_cal(pctx, pframe, lbuf_width, pm);/*prepare param for every slice*/
		if (!use_fmcu) {
...
rewait:
		...
		if (!pframe) {
			...
			read_op.parm.frame.tile_width = pframe->tile_width;
			read_op.parm.frame.tile_height = pframe->tile_height;
			read_op.parm.frame.tile_num_x = pframe->tile_x_num;
			read_op.parm.frame.tile_num_y = pframe->tile_y_num;
			/*
