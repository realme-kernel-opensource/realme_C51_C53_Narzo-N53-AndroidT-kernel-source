		...
		unsigned long arg)
{
	...
	switch (statis_buf.type) {
		case STATIS_INIT:
			...
			if (hw->ip_isp->frbg_hist_support || hw->ip_isp->rgb_gtm_support || hw->ip_isp->rgb_ltm_support) {
				ch = &module->channel[CAM_CH_PRE];
				if (!ch->enable && (module->simulator || module->cam_uinfo.dcam_slice_mode == CAM_OFFLINE_SLICE_HW))
		...
		case STATIS_LTMHIST:
			...
			if (hw->ip_isp->rgb_ltm_support > 0) {
				/*ltm in isp*/
				ch = &module->channel[CAM_CH_PRE];
				if (!ch->enable && module->simulator) {
					pr_info("prev channel disable, can not set ltm buf\n");
					break;
				}
				...
				/*ltm in dcam*/
				pr_debug("dcam ltm statis buf cfg\n");
			}
			break;
		...
		unsigned long arg)
{
	...
	pr_info("4in1:[%d], rgb_ltm[%d], yuv_ltm[%d], gtm[%d], dual[%d], dec %d, raw_alg_type:%d, zoom_conflict_with_ltm %d,"
		" %d. dcam_raw %d, param_frame_sync:%d,virtualsensor %d, master_flag:%d, ipg_skip_first_frm %d\n",
		...
		module->master_flag, module->cam_uinfo.ipg_skip_first_frm);
		...
		unsigned long arg)
{
	...
	if ((atomic_read(&module->state) != CAM_IDLE) &&
		(atomic_read(&module->state) != CAM_CFG_CH)) {
		pr_err("fail to get correct state %d\n", atomic_read(&module->state));
		ret = -EFAULT;
		goto exit;
	}
		...
		unsigned long arg)
{
	...
	channel->ultra_ltm_hist_isp_ctx_id = -1;
	channel->ultra_ltm_hist_isp_path_id = -1;
		...
		unsigned long arg)
{
	...
	if (module->cam_uinfo.dcam_slice_mode && (module->aux_dcam_dev || module->cam_uinfo.virtualsensor))
	...
	for (i = 0; i < CAM_CH_MAX; i++) {
		...
		if ((ch->ch_id == CAM_CH_CAP) && ch->ultra_ltm_hist_isp_ctx_id >= 0)
			module->isp_dev_handle->isp_ops->put_context(module->isp_dev_handle,
								ch->ultra_ltm_hist_isp_ctx_id);
			...
			if (module->cam_uinfo.is_rgb_ltm) {
				for (j = 0; j < ISP_LTM_BUF_NUM; j++) {
					if (ch->ltm_bufs[LTM_RGB][j]) {
						if ((ch->ch_id == CAM_CH_PRE) || (ch->ultra_ltm_hist_isp_ctx_id >= 0))
	...
	for (i = 0; i < CAM_CH_MAX; i++) {
		ch = &module->channel[i];
		if (ch->res_frame) {
			cam_buf_ionbuf_put(&ch->res_frame->buf);
			cam_queue_empty_frame_put(ch->res_frame);
		}
		...
		unsigned long arg)
{
	...
	if (proc_info.cmd == RAW_PROC_PRE) {
		struct isp_raw_proc_info *u_proc_info = (struct isp_raw_proc_info __user *)arg;
		ret = camcore_raw_pre_proc(module, &proc_info);
		if (module->channel[CAM_CH_CAP].ultra_ltm_hist_isp_ctx_id >= 0) {
			uint32_t ratio = 2, align_coeff_w = 1, align_coeff_h = 1, width_aligned = 0, height_aligned = 0;
			if (proc_info.src_size.height > DCAM_SW_SLICE_HEIGHT_MAX)
				ratio = 4;
			align_coeff_w = 2 * ratio * 14;
			align_coeff_h = 2 * ratio * 14;
			width_aligned = (proc_info.src_size.width / align_coeff_w) * align_coeff_w;
			height_aligned = (proc_info.src_size.height / align_coeff_h) * align_coeff_h;
			put_user(width_aligned, &u_proc_info->isp_xtm_aligned_size.width);
			put_user(height_aligned, &u_proc_info->isp_xtm_aligned_size.height);
		} else {
			put_user(proc_info.dst_size.width, &u_proc_info->isp_xtm_aligned_size.width);
			put_user(proc_info.dst_size.height, &u_proc_info->isp_xtm_aligned_size.height);
		}
...
{
	int ret = 0;
	uint32_t for_fdr = 0, for_capture = 0, loop = 0;
	...
	if (param.status == 0) {
		...
		if (module->cam_uinfo.is_raw_alg && param.scene_id == PM_SCENE_OFFLINE_CAP) {
			mutex_lock(&sw_ctx->blkpm_dcam_lock);
			do {
			...
			} while (loop++ < sw_ctx->blk_param_queue.cnt);
			mutex_unlock(&sw_ctx->blkpm_dcam_lock);
		}
	}
		...
		if (module->isp_dev_handle->cfg_handle) {
			if (param.scene_id == PM_SCENE_PRE) {
				...
				if (channel_cap->enable == 0)
					ret = module->isp_dev_handle->isp_ops->ioctl(module->isp_dev_handle,
						channel->isp_ctx_id, ISP_IOCTL_CFG_LTMMAP_BUF_RECYCLE, param_frame);
				else
					ret = module->isp_dev_handle->isp_ops->ioctl(module->isp_dev_handle,
						channel->isp_ctx_id, ISP_IOCTL_CFG_LTMMAP_BUF_CFG, param_frame);
				...
				if (param_frame) {
					param_frame->blkparam_info.param_block->ltm_rgb_info.ltm_map.mfd = 0;
					...
					if (channel_pre->enable == 1) {
						ret = module->isp_dev_handle->isp_ops->ioctl(module->isp_dev_handle,
							channel_pre->isp_ctx_id, ISP_IOCTL_CFG_LTMMAP_BUF_RECYCLE, param_frame);
					} else {
						if (param.scene_id == PM_SCENE_OFFLINE_CAP)
							ret = module->isp_dev_handle->isp_ops->ioctl(module->isp_dev_handle,
								channel->isp_fdrh_ctx, ISP_IOCTL_CFG_LTMMAP_BUF_RECYCLE, param_frame);
						else
							ret = module->isp_dev_handle->isp_ops->ioctl(module->isp_dev_handle,
								channel->isp_ctx_id, ISP_IOCTL_CFG_LTMMAP_BUF_RECYCLE, param_frame);
					}
					...
					if (channel->ultra_ltm_hist_isp_ctx_id >= 0) {
						struct isp_sw_context *statis_pctx = NULL;
						struct camera_frame *statis_param = NULL;
						statis_pctx = module->isp_dev_handle->sw_ctx[channel->ultra_ltm_hist_isp_ctx_id];
						statis_param = cam_queue_empty_blk_param_get(&statis_pctx->param_share_queue);
						if (!statis_param) {
							mutex_lock(&statis_pctx->blkpm_q_lock);
							statis_param = cam_queue_dequeue(&statis_pctx->param_buf_queue, struct camera_frame, list);
							mutex_unlock(&statis_pctx->blkpm_q_lock);
						}
						memcpy(&statis_pctx->isp_k_param, &pctx->isp_k_param, sizeof(struct isp_k_block));
						if (statis_param) {
							memcpy(statis_param->blkparam_info.param_block, param_frame->blkparam_info.param_block, sizeof(struct isp_k_block));
							statis_param->blkparam_info.update = param_frame->blkparam_info.update;
							statis_param->fid = param_frame->fid;
							cam_queue_enqueue(&statis_pctx->param_buf_queue, &statis_param->list);
						}
					}
