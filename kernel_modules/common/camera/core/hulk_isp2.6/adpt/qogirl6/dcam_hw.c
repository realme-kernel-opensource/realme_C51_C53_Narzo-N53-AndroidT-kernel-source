...
static int dcamhw_start(void *handle, void *arg)
{
	...
	if (parm->raw_callback == 1 || parm->ipg_skip_first_frm)
...
static int dcamhw_path_start(void *handle, void *arg)
{
	...
	case  DCAM_PATH_FULL:
		...
		if (patharg->pack_bits == 0 || patharg->pack_bits == 1) {
			/*10bit*/
...
static int dcamhw_slice_fetch_set(void *handle, void *arg)
{
	...
	struct img_trim *cur_out_trim = NULL;
	struct dcam_store_slice_info *bin_store = NULL;
	struct dcam_fetch_info *fetch = NULL;
	struct dcam_hw_slice_fetch *slicearg = NULL;
	uint32_t fetch_pitch, prev_picth, bfp;
	uint32_t fetch_offset = 0, store_offset = 0, bin_store_offset = 0, bin_pitch = 0;
	uint32_t bin_bfp = 0, full_bfp = 0, full_pitch = 0;
	...
	cur_out_trim = slicearg->cur_slice_out_trim;
	idx = slicearg->idx;

	bin_store = &slicearg->bin_slice_info;

	if (bin_store->pack_bits == RAW_PACK10) {
		bin_pitch = (bin_store->whole_size.w * 10 + 127) / 128;
		bin_bfp = 5;
	} else if (bin_store->pack_bits == RAW_8) {
		bin_pitch = (bin_store->whole_size.w * 8 + 127) / 128;
		bin_bfp = 4;
	} else {
		bin_pitch = (bin_store->whole_size.w * 16 + 127) / 128;
		bin_bfp = 8;
	}

	if (slicearg->full_store_fmt == RAW_PACK10) {
		full_pitch = (fetch->size.w * 10 + 127) / 128;
		full_bfp = 5;
	} else if (slicearg->full_store_fmt == RAW_8) {
		full_pitch = (fetch->size.w * 8 + 127) / 128;
		full_bfp = 4;
	} else {
		full_pitch = (fetch->size.w * 16 + 127) / 128;
		full_bfp = 8;
	}
	...
	if (slicearg->dcam_slice_mode == CAM_OFFLINE_SLICE_SW) {
		...
		if ((slicearg->slice_count <= slicearg->w_slice_num)
			&& (slicearg->w_slice_num != slicearg->slice_num)) {
			fetch_offset = fetch->size.w * cur_slice->start_y * bfp / 4;
			store_offset = cal_sprd_raw_pitch(fetch->size.w, slicearg->full_store_fmt) * (cur_out_trim->start_y + cur_slice->start_y);
			bin_store_offset = bin_pitch * 16 * (cur_out_trim->start_y + cur_slice->start_y) / bin_store->ratio;
			pr_debug("bin start %d %d, pitch %x, offset %x\n",
				cur_slice->start_x, cur_slice->start_y, bin_pitch, bin_store_offset);
		}
		pr_debug("dcam%d, slice %d, start_x %d, size_x %d size_y %d relative_offset %d, fetch_offset %d, store_offset %d\n",
				idx, (slicearg->slice_num - slicearg->slice_count), cur_slice->start_x, cur_slice->size_x,
				cur_slice->size_y,  slicearg->relative_offset, fetch_offset, store_offset);

		if (slicearg->slice_count % slicearg->w_slice_num == 0) {
			DCAM_REG_MWR(idx, DCAM_BAYER_INFO_CFG, BIT_5 | BIT_4, (fetch->pattern & 3) << 4);
			DCAM_AXIM_MWR(IMG_FETCH_CTRL, BIT_1 | BIT_0, fetch->pack_bits);
			DCAM_AXIM_MWR(IMG_FETCH_CTRL, BIT_3 | BIT_2, fetch->endian << 2);
			...
			DCAM_AXIM_WR(IMG_FETCH_RADDR, fetch->addr.addr_ch0 + fetch_offset);
			DCAM_AXIM_WR(IMG_FETCH_SIZE,
			...
			DCAM_REG_MWR(idx, DCAM_FULL_CFG,
				(0x7FF << 20) | (1 << 1), (full_pitch << 20) | (1 << 1));
			DCAM_REG_WR(idx, DCAM_FULL_CROP_START, (cur_out_trim->start_y << 16));
			DCAM_REG_WR(idx, DCAM_FULL_CROP_SIZE,
				((cur_out_trim->size_y & 0x1fff) << 16) | (cur_slice->size_x & 0x1fff));
			reg_val = DCAM_REG_RD(idx, DCAM_FULL_BASE_WADDR);
			DCAM_REG_WR(idx, DCAM_FULL_BASE_WADDR, reg_val + store_offset - slicearg->relative_offset * full_bfp / 4);
			slicearg->relative_offset = cur_slice->start_x;

			/* cfg bin path */
			if (bin_store->enable == 0)
				return 0;
			DCAM_REG_MWR(idx, DCAM_CAM_BIN_CFG, 0x3ff00000, bin_pitch << 20);
			DCAM_REG_MWR(idx, DCAM_CAM_BIN_CFG, BIT_1, BIT_1);
			DCAM_REG_WR(idx, DCAM_CAM_BIN_CROP_START, (cur_out_trim->start_y << 16));
			...
			reg_val = reg_val + bin_store_offset - slicearg->relative_bin_offset * bin_bfp / 4;
			DCAM_REG_WR(idx, DCAM_BIN_BASE_WADDR0, reg_val);
			slicearg->relative_bin_offset = cur_slice->start_x / bin_store->ratio;

			pr_debug("bin store size %d %d, pitch %d\n", bin_store->store_size.w, bin_store->store_size.h, bin_pitch);
		} else if (((slicearg->slice_count + 1) % slicearg->w_slice_num) < slicearg->w_slice_num) {
			DCAM_REG_MWR(idx, DCAM_MIPI_CAP_CFG, BIT_30, 0x1 << 30);
			DCAM_AXIM_WR(IMG_FETCH_RADDR, fetch->addr.addr_ch0 + (cur_slice->start_x - DCAM_OVERLAP) * bfp / 4 + fetch_offset);
			DCAM_AXIM_WR(IMG_FETCH_SIZE, (cur_slice->size_y << 16) | ((cur_slice->size_x + DCAM_OVERLAP) & 0x1fff));
			DCAM_AXIM_WR(IMG_FETCH_X, fetch_pitch << 16);

			/* cfg full path */
			DCAM_REG_MWR(idx, DCAM_FULL_CFG,
				(0x7FF << 20) | (1 << 1), (full_pitch << 20) | (1 << 1));
			DCAM_REG_WR(idx, DCAM_FULL_CROP_START, (DCAM_OVERLAP & 0x1fff) | (cur_out_trim->start_y << 16));
			DCAM_REG_WR(idx, DCAM_FULL_CROP_SIZE, ((cur_out_trim->size_y & 0x1fff) << 16) | (cur_slice->size_x & 0x1fff));
			reg_val = DCAM_REG_RD(idx, DCAM_FULL_BASE_WADDR);
			DCAM_REG_WR(idx, DCAM_FULL_BASE_WADDR, reg_val + (cur_slice->start_x - slicearg->relative_offset ) * full_bfp / 4);
			slicearg->relative_offset = cur_slice->start_x;
			...
			DCAM_REG_MWR(idx, DCAM_CAM_BIN_CFG, 0x3ff00000, bin_pitch << 20);
			DCAM_REG_MWR(idx, DCAM_CAM_BIN_CFG, BIT_1, BIT_1);
			DCAM_REG_WR(idx, DCAM_CAM_BIN_CROP_START, (bin_store->overlap & 0x1fff) | (cur_out_trim->start_y << 16));
			DCAM_REG_WR(idx, DCAM_CAM_BIN_CROP_SIZE,
				((bin_store->store_size.h & 0x1fff) << 16) | (bin_store->store_size.w & 0x1fff));

			reg_val = DCAM_REG_RD(idx, DCAM_BIN_BASE_WADDR0);
			reg_val = reg_val + (cur_slice->start_x / bin_store->ratio - slicearg->relative_bin_offset) * bin_bfp / 4;
			DCAM_REG_WR(idx, DCAM_BIN_BASE_WADDR0, reg_val);
			slicearg->relative_bin_offset = cur_slice->start_x / bin_store->ratio;
			pr_debug("bin store start %d size %d %d, pitch %d\n", bin_store->overlap, bin_store->store_size.w, bin_store->store_size.h, bin_pitch);
		} else {
			DCAM_REG_MWR(idx, DCAM_MIPI_CAP_CFG, BIT_30, 0x0 << 30);

			DCAM_AXIM_WR(IMG_FETCH_RADDR, fetch->addr.addr_ch0 + (cur_slice->start_x - DCAM_OVERLAP) * bfp / 4 + fetch_offset);

			DCAM_AXIM_WR(IMG_FETCH_SIZE, (cur_slice->size_y << 16) | ((cur_slice->size_x + DCAM_OVERLAP * 2) & 0x1fff));

			DCAM_AXIM_WR(IMG_FETCH_X, fetch_pitch << 16);

			/* cfg full path */
			DCAM_REG_MWR(idx, DCAM_FULL_CFG, (0x7FF << 20) | (1 << 1), (full_pitch << 20) | (1 << 1));
			DCAM_REG_WR(idx, DCAM_FULL_CROP_START, (DCAM_OVERLAP & 0x1fff) | (cur_out_trim->start_y << 16));
			DCAM_REG_WR(idx, DCAM_FULL_CROP_SIZE,
				((cur_out_trim->size_y & 0x1fff) << 16) | (cur_slice->size_x & 0x1fff));
			reg_val = DCAM_REG_RD(idx, DCAM_FULL_BASE_WADDR);
...
static int dcamhw_csi_connect(void *handle, void *arg)
{
	...
	if (csi_switch->mipi_cap_pre_skip_num == 0)
