				...
				enum dcam_cb_type type)
{
	...
	if (((type == DCAM_CB_DATA_DONE) || (type == DCAM_CB_VCH2_DONE))
		&& (!sw_ctx->slc_splice_mode || (frame->sw_slice_no + 1 == frame->sw_slice_num))) {
...
static void dcamint_sensor_sof(void *param, struct dcam_sw_context *sw_ctx)
{
	...
	} else if (sw_ctx->cap_info.ipg_skip_first_frm) {
		DCAM_REG_MWR(dcam_hw_ctx->hw_ctx_id, DCAM_INT_EN, BIT(DCAM_SENSOR_SOF), 0);
		 csi_api_off_ipg();
	}
}
...
static void dcamint_sensor_eof(void *param, struct dcam_sw_context *sw_ctx)
{
	...
	if (sw_ctx->cap_info.ipg_skip_first_frm) {
		DCAM_REG_MWR(dcam_hw_ctx->hw_ctx_id, DCAM_INT_EN, BIT(DCAM_SENSOR_EOF), 0);
	}
}
...
static void dcamint_full_path_done(void *param, struct dcam_sw_context *sw_ctx)
{
	...
	struct camera_frame *pframe = NULL;
	struct dcam_path_desc *path = NULL;
	struct dcam_path_desc *bin_path = NULL;
	struct dcam_3dnrmv_ctrl *mv_state = NULL;
	uint32_t mv_ready = 0, trigger_isp = 0, full_slice_count = 0;
	...
	bin_path = &sw_ctx->path[DCAM_PATH_BIN];
	if (sw_ctx->offline) {
		if (sw_ctx->dcam_slice_mode != CAM_OFFLINE_SLICE_SW) {
			if (sw_ctx->slice_num > 0) {
				uint32_t full_done = 0, bin_done = 0;
				full_done = atomic_inc_return(&dcam_hw_ctx->path_done[DCAM_PATH_FULL]);
				bin_done = atomic_read(&dcam_hw_ctx->path_done[DCAM_PATH_BIN]);
				full_slice_count = sw_ctx->slice_count - 1;
				pr_info("dcam%d offline slice%d done.\n",
					dcam_hw_ctx->hw_ctx_id, (sw_ctx->slice_num - sw_ctx->slice_count));

				if (sw_ctx->slc_splice_mode || (full_slice_count == 0))
					trigger_isp = 1;
				if ((atomic_read(&bin_path->user_cnt) && (full_done == bin_done)) ||
					(atomic_read(&bin_path->user_cnt) == 0)) {
	...
	if ((frame = dcamint_frame_prepare(dcam_hw_ctx, sw_ctx, DCAM_PATH_FULL))) {
		if (sw_ctx->is_4in1) {
			...
			frame->irq_type = CAMERA_IRQ_IMG;
		} else if (sw_ctx->virtualsensor)
			frame->irq_type = CAMERA_IRQ_IMG;

		if (sw_ctx->slc_splice_mode && sw_ctx->slice_num && sw_ctx->dcam_slice_mode == CAM_OFFLINE_SLICE_HW) {
			frame->dcam_idx = dcam_hw_ctx->hw_ctx_id;
			frame->sw_slice_num = sw_ctx->slice_num;
			frame->sw_slice_no = sw_ctx->slice_num - sw_ctx->slice_count - 1;
			frame->slice_trim = sw_ctx->slice_info.slice_trim[frame->sw_slice_no];
			frame->slice_out_trim = sw_ctx->slice_info.slice_out_trim[frame->sw_slice_no];
			if (full_slice_count) {
				pframe = cam_queue_empty_frame_get();
				if (pframe) {
					pframe->channel_id = frame->channel_id;
					pframe->priv_data = sw_ctx;
					pframe->not_srcbuf = 1;
					pframe->need_ltm_hist = frame->need_ltm_hist;
					pframe->need_ltm_map = frame->need_ltm_map;
					pframe->need_gtm_hist = frame->need_gtm_hist;
					pframe->need_gtm_map = frame->need_gtm_map;
					pframe->gtm_mod_en = frame->gtm_mod_en;
					pframe->nonzsl_xtm = frame->nonzsl_xtm;
					pframe->width = frame->width;
					pframe->height = frame->height;
					memcpy(&pframe->buf, &frame->buf, sizeof(struct camera_buf));
					cam_queue_enqueue(&path->out_buf_queue, &pframe->list);
				}
				dcam_path_store_frm_set(sw_ctx, &sw_ctx->path[DCAM_PATH_FULL]);
			}
		}
	...
	if (sw_ctx->offline) {
		...
		if (!sw_ctx->rps && !sw_ctx->slice_count)
			dcam_core_context_unbind(sw_ctx);
		complete(&sw_ctx->frm_done);
	}
	/*for virtual sensor ret buf and start next frame*/
	if (sw_ctx->virtualsensor) {
		if (!sw_ctx->slice_count) {
			frame = cam_queue_dequeue(&sw_ctx->proc_queue, struct camera_frame, list);
			if (frame) {
...
static void dcamint_bin_path_done(void *param, struct dcam_sw_context *sw_ctx)
{
	...
	struct camera_frame *pframe = NULL;
	struct dcam_3dnrmv_ctrl *mv_state = NULL;
	uint32_t mv_ready = 0, trigger_isp = 0, bin_slice_count = 0;
	int i = 0, cnt = 0;
	...
	full_path = &sw_ctx->path[DCAM_PATH_FULL];
	...
	if (sw_ctx->offline) {
		if (sw_ctx->dcam_slice_mode != CAM_OFFLINE_SLICE_SW) {
			if (sw_ctx->slice_num > 0) {
				uint32_t full_done = 0, bin_done = 0;
				full_done = atomic_read(&dcam_hw_ctx->path_done[DCAM_PATH_FULL]);
				bin_done = atomic_inc_return(&dcam_hw_ctx->path_done[DCAM_PATH_BIN]);
				bin_slice_count = sw_ctx->slice_count - 1;
				if (sw_ctx->slc_splice_mode || (bin_slice_count == 0))
					trigger_isp = 1;
				if ((atomic_read(&full_path->user_cnt) && (full_done == bin_done)) ||
					(atomic_read(&full_path->user_cnt) == 0)) {
					complete(&sw_ctx->slice_done);
					sw_ctx->slice_count--;
				}
				pr_info("dcam%d offline slice%d done.bin done %d, slice num %d\n",
					dcam_hw_ctx->hw_ctx_id, (sw_ctx->slice_num - sw_ctx->slice_count), bin_done, sw_ctx->slice_num);
				if (!trigger_isp)
					return;
			}
		}
	...
	if (sw_ctx->virtualsensor) {
		if (sw_ctx->dcam_slice_mode != CAM_OFFLINE_SLICE_SW) {
			if (sw_ctx->slice_num > 0) {
				...
				pr_debug("dcam%d offline slice%d done.bin done %d, slice num %d\n",
					dcam_hw_ctx->hw_ctx_id, (sw_ctx->slice_num - sw_ctx->slice_count), bin_done, sw_ctx->slice_num);
				if (!trigger_isp)
					return;
			}
	...
	if ((frame = dcamint_frame_prepare(dcam_hw_ctx, sw_ctx, DCAM_PATH_BIN))) {
		...
		if (sw_ctx->slc_splice_mode && sw_ctx->slice_num && sw_ctx->dcam_slice_mode == CAM_OFFLINE_SLICE_HW) {
			frame->dcam_idx = dcam_hw_ctx->hw_ctx_id;
			frame->sw_slice_num = sw_ctx->slice_num;
			frame->sw_slice_no = sw_ctx->slice_num - bin_slice_count - 1;
			frame->slice_trim = sw_ctx->slice_info.slice_trim[frame->sw_slice_no];
			frame->slice_out_trim = sw_ctx->slice_info.slice_out_trim[frame->sw_slice_no];
			if (bin_slice_count) {
				pframe = cam_queue_empty_frame_get();
				...
				dcam_path_store_frm_set(sw_ctx, &sw_ctx->path[DCAM_PATH_BIN]);
			}
		}
	...
	pr_err("Running with virtualsensor_cap_en %d", atomic_read(&sw_ctx->virtualsensor_cap_en));
}
