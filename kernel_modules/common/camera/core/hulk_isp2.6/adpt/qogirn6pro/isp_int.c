...
static void ispint_rgb_ltm_hists_done(enum isp_context_hw_id hw_idx, void *isp_handle)
{
	...
	if (rgb_ltm->calc_mode == LTM_SW_CALC) {
		...
		if (!frame) {
			pr_warn("isp ctx_id[%d] ltmhist_out_queue buffer\n", idx);
			return;
		} else {
			pr_debug("idx %d ctx->ltm_frame.tile_width %d %d %d %d\n", idx, frame->tile_width,frame->tile_height,frame->tile_x_num,frame->tile_y_num);
