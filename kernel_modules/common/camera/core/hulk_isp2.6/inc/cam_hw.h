...
enum isp_k_ltm_blk_idx {
	ISP_K_LTM_HIST_SET,
	ISP_K_LTM_MAPPING_SET,
	ISP_K_LTM_MAX
};
...
enum isp_hw_cfg_cmd {
	...
	ISP_HW_CFG_SLICE_LTM_INFO,
	ISP_HW_CFG_LTM_FUNC_GET,
	ISP_HW_CFG_MAX
};
...
struct dcam_store_slice_info {
	uint32_t enable;
	uint32_t pack_bits;
	uint32_t overlap;
	uint32_t ratio;
	struct img_size store_size;
	struct img_size whole_size;
};
...
struct dcam_hw_slice_fetch {
	...
	uint32_t relative_bin_offset;
	uint32_t full_store_fmt;
	...
	struct dcam_store_slice_info bin_slice_info;
};
...
struct isp_hw_ltm_func {
	enum isp_k_ltm_blk_idx index;
	isp_k_blk_func k_blk_func;
};
