...
abnormal_reg_trace:
	...
	for (addr = ISP_LTM_MAP_PARAM4 + ISP_LTM_MAP_RGB_BASE;
		addr <= (ISP_LTM_MAP_PARAM4 + ISP_LTM_MAP_PARAM5);
		addr += 16) {
...
static int isphw_path_store(void *handle, void *arg)
{
	...
	pr_debug("store size %d %d, picth %d\n", store_info->size.w, store_info->size.h, store_info->pitch.pitch_ch0);
...
static int isphw_slice_store(void *handle, void *arg)
{
	...
	ISP_REG_WR(store->ctx_id, addr, cmd);
	pr_debug("store size %d %d, addr %x %x, border %x\n", store->slc_store->size.w, store->slc_store->size.h, store->slc_store->addr.addr_ch0,
		store->slc_store->addr.addr_ch1, cmd);
...
static int isphw_slice_fetch(void *handle, void *arg)
{
	...
	cmd = ((fetch->fetch_info->size.h & 0xFFFF) << 16) |
			(fetch->fetch_info->size.w & 0xFFFF);
	ISP_REG_WR(fetch->ctx_id, addr, cmd);
	pr_debug("size %d %d, addr %x\n", fetch->fetch_info->size.w, fetch->fetch_info->size.h, fetch->fetch_info->addr.addr_ch0);
...
static int isphw_slice_spath_store(void *handle, void *arg)
{
	...
	pr_debug("slice addr %x %x, size %d %d, border %d %d %d %d\n",
		spath->slc_store->addr.addr_ch0, spath->slc_store->addr.addr_ch1,
		spath->slc_store->size.w, spath->slc_store->size.h, spath->slc_store->border.up_border,
		spath->slc_store->border.down_border, spath->slc_store->border.left_border,
		spath->slc_store->border.right_border);
	return 0;
}
