		...
		unsigned long arg)
{
	...
	cam_queue_clear(&module->dump_xtm_hist_q, struct camera_frame, list);
...
cfg_ch_done:
	...
	cam_queue_init(&module->dump_xtm_hist_q, 10, camcore_k_frame_put);
		...
		unsigned long arg)
{
	...
	thrd = &module->dump_xtm_hist_thrd;
	sprintf(thrd->thread_name, "cam%d_dump_xtm_hist", module->idx);
	ret = camcore_thread_create(module, thrd, camcore_dump_xtm_hist_proc);
	if (ret)
		goto stop_thrd;
...
stop_thrd:
	...
	camcore_thread_stop(&module->dump_xtm_hist_thrd);
	return ret;
}
