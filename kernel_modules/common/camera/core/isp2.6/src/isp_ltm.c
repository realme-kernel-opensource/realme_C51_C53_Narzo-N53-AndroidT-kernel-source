...
static void ispltm_sync_fid_set(void *handle)
{
	...
	if (ltm_ctx->sub_img_num)
		pr_info("isp %d ltm hist done sub img %d\n", ltm_ctx->ctx_id, fid);
			...
			struct isp_ltm_stat_info *tuning)
{
	...
	if (ctx->sub_img_num) {
		...
		pr_debug("statis width %d %d\n", ctx->frame_width_stat, ctx->frame_height_stat);
...
static int ispltm_pipe_proc(void *handle, void *param)
{
	...
	case MODE_LTM_CAP:
		...
		pr_info("LTM capture ctx_id %d, map %d\n", ctx->ctx_id, ctx->map.bypass);
		if (!ctx->map.bypass) {
		...
		enum isp_ltm_cfg_cmd cmd, void *param)
{
	...
	case ISP_LTM_CFG_SUB_IMG_IDX:
		...
		if (ltm_ctx->sub_img_num)
			pr_info("ctx %d, sub img %d/%d\n", ltm_ctx->ctx_id, ltm_ctx->sub_img_idx, ltm_ctx->sub_img_num);
		break;
	default:
