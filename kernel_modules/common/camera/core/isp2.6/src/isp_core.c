		...
		struct camera_frame *pframe)
{
	...
	store_start_x = pframe->slice_trim.start_x;
	store_start_y = pframe->slice_trim.start_y;
	store_size_x = pframe->slice_trim.size_x;
	store_size_y = pframe->slice_trim.size_y;

	store->pitch.pitch_ch0 = pframe->width;
	store->pitch.pitch_ch1 = pframe->width;
	...
	if (slice_num > SLICE_NUM_MAX) {
		...
		store->slice_offset.addr_ch0 = store_start_x + store_start_y * pframe->width;
		store->slice_offset.addr_ch1 = store_start_x + store_start_y * pframe->width / 2;
		store->slice_offset.addr_ch2 = 0;
	}
		...
		struct camera_frame *pframe, struct offline_tmp_param *tmp)
{
	...
	if (stream) {
		...
		for (i = 0; i < ISP_SPATH_NUM; i++) {
			...
			if (pctx->ch_id == 3)
				pr_info("isp %d out_size %d %d crop_szie %d %d %d %d\n",
		...
		struct camera_frame *pframe, struct offline_tmp_param *tmp)
{
	...
	if (pctx->ch_id == 3)
		pr_debug("fetch whole size %d %d %d %d\n", pipe_in->fetch.in_trim.start_x, pipe_in->fetch.in_trim.start_y,
			pipe_in->fetch.in_trim.size_x, pipe_in->fetch.in_trim.size_y);
	if (pipe_src->fetch_path_sel == ISP_FETCH_PATH_FBD) {
	...
	for (i = 0; i < ISP_SPATH_NUM; i++) {
		...
		out_frame->sensor_time = pframe->sensor_time;
		out_frame->boot_sensor_time = pframe->boot_sensor_time;
		out_frame->nonzsl_xtm.cur_img_idx = pframe->nonzsl_xtm.cur_img_idx;
		...
		if (pctx->ch_id == 3)
			pr_debug("isp %d, store %x %x size %d %d\n", pctx->ctx_id, pipe_in->store[i].store.addr.addr_ch0,
				pipe_in->store[i].store.addr.addr_ch1, pipe_in->store[i].store.size.w, pipe_in->store[i].store.size.h);
