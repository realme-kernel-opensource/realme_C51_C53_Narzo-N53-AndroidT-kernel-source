...
struct camera_module {
	...
	struct cam_thread_info dump_xtm_hist_thrd;

	struct camera_queue dump_xtm_hist_q;
		...
		struct camera_frame *pframe, struct channel_context *channel)
{
	int ret = 0;
	struct camera_frame *pframe1 = NULL;
	uint32_t tile_x = 0, tile_y = 0, img0_size_x = 0, img0_size_y = 0, img1_start_x = 0, img2_start_y = 0, ratio = 1;
	...
	if (pframe->nonzsl_xtm.use_for_xtm_hist == 0) {
		tile_x = (width + 55) / 56 * 4;
		tile_y = (height + 27) / 28 * 2;
		img0_size_x = 8 * tile_x;
		img0_size_y = 8 * tile_y;
		img1_start_x = width - img0_size_x;
		img2_start_y = height - img0_size_y;
		sub_img_trim[0].start_x = 0;
		sub_img_trim[0].start_y = 0;
		sub_img_trim[0].size_x = img0_size_x;
		sub_img_trim[0].size_y = img0_size_y;
		sub_img_trim[1].start_x = img1_start_x;
		sub_img_trim[1].start_y = 0;
		sub_img_trim[1].size_x = width - img1_start_x;
		sub_img_trim[1].size_y = img0_size_y;
		sub_img_trim[2].start_x = 0;
		sub_img_trim[2].start_y = img2_start_y;
		sub_img_trim[2].size_x = img0_size_x;
		sub_img_trim[2].size_y = height - img2_start_y;
		sub_img_trim[3].start_x = img1_start_x;
		sub_img_trim[3].start_y = img2_start_y;
		sub_img_trim[3].size_x = width - img1_start_x;
		sub_img_trim[3].size_y = height - img2_start_y;
		...
		ratio= large_size_x / width;
		tile_x = (large_size_x + 55) / 56 * 4;
		tile_y = (large_size_y + 27) / 28 * 2;
	...
	if (pframe->nonzsl_xtm.use_for_xtm_hist) {
		...
		if (dcam_output_end_pos_y >= subimg_end_y &&
			...
			pr_debug("send bin sub img %d to isp\n", ltm_statis_subimg_idx);
		...
		if (dcam_output_end_pos_y >= subimg_end_y &&
			...
			pr_debug("send full sub img %d to isp in_queue\n", ltm_subimg_idx);
...
static void cam_write_bin_image_to_file(uint8_t *buffer,
...
static int xtm_bin_image_cnt = 0;
static int camcore_dump_xtm_hist_proc(void *param)
{
	struct camera_module *module = (struct camera_module *)param;
	struct camera_frame *pframe = NULL;
	...
	if (!param)
		return 0;
	pr_info("dump bin image\n");
	pframe = cam_queue_dequeue_peek(&module->dump_xtm_hist_q, struct camera_frame, list);
	if (!pframe)
		return 0;

	channel = &module->channel[CAM_CH_CAP];
	pack_bits = channel->ch_uinfo.dcam_raw_fmt;
	size = cal_sprd_raw_pitch(pframe->width, pack_bits) * pframe->height;
	sprintf(file_name, "/data/ylog/xtm_bin_image_No%d", xtm_bin_image_cnt);
	cam_write_bin_image_to_file((char *)pframe->buf.addr_k[0], size, file_name);
	xtm_bin_image_cnt++;
	...
	struct channel_context *channel)
{
	...
	if (channel->ultra_ltm_hist_isp_ctx_id >= 0) {
		...
		ctx_size.src.w = ch_desc.input_size.w / 4;
		ctx_size.src.h = ch_desc.input_size.h / 4;
		ctx_size.crop.start_x = 0;
		ctx_size.crop.start_y = 0;
		ctx_size.crop.size_x = ch_desc.input_size.w / ratio;
		ctx_size.crop.size_y = ch_desc.input_size.h / ratio;
		...
		path_desc.output_size.w = ch_uinfo->dst_size.w / ratio;
		path_desc.output_size.h = ch_uinfo->dst_size.h / ratio;
		...
		path_trim.start_x = 0;
		path_trim.start_y = 0;
		path_trim.size_x = ctx_size.crop.size_x;
		path_trim.size_y = ctx_size.crop.size_y;
		ret = module->isp_dev_handle->isp_ops->cfg_path(module->isp_dev_handle,
		...
		struct isp_raw_proc_info *proc_info)
{
	...
	if (proc_info->src_size.width >= DCAM_HW_SLICE_WIDTH_MAX) {
		dev->sw_ctx[module->offline_cxt_id].pack_bits = 0;
		ch->ch_uinfo.sensor_raw_fmt = 0;
	}
	ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(sw_ctx,
		DCAM_PATH_CFG_BASE, ch->dcam_path_id, &ch_desc);

	pr_info("dcam fetch packbit %d path%d sensor_raw_fmt %d, dcam_raw_fmt %d, default raw fmt %d\n",
	...
	ch_desc.priv_size_data = NULL;
	if ((ch_desc.output_size.h > DCAM_SW_SLICE_HEIGHT_MAX) && 
		ch->ultra_ltm_hist_isp_ctx_id < 0)
		ch_desc.slc_splice_mode = ISP_SPLICE;
	else
		ch_desc.slc_splice_mode = DCAM_SPLICE;
	ret = module->dcam_dev_handle->dcam_pipe_ops->cfg_path(sw_ctx,
