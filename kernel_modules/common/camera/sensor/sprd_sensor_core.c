...
#include <linux/i2c.h>
#include <../../../../../bsp/kernel5.4/kernel5.4/include/linux/hardware_info.h>
...
static struct mutex vga_sensor_mutex;
...
static char _main_camera_sn[30];
static char _sub_camera_sn[30];
//static char _main2_camera_sn[30];
//static char _main3_camera_sn[30];
	...
	size_t size)
{
	...
	//if( NULL != temp_p )
	//{
	// 	memcpy(_main2_camera_sn, temp_p+4, 17);
	...
	//if( NULL != temp_p )
	//{
	//	memcpy(_main3_camera_sn, temp_p+4, 17);
...
static void sprd_sensor_core_module_exit(void)
{
	...
	if (p_data) {
		...
		mutex_destroy(&vga_sensor_mutex);
