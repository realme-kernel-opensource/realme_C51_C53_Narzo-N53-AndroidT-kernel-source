...
static struct platform_driver mtx_dvfs_driver = {
    ...
    .driver =
        {
            .name = "mtx-dvfs", .of_match_table = mtx_dvfs_of_match,
        },
};
...
static int dvfs_gov_get_target(struct devfreq *devfreq,
    ...
    if (module->dvfs_enable) {
        ...
        *freq = adjusted_freq;
        ...
        pr_err("mtx_dvfs_gov, %s: dvfs unenable", __func__);

     pr_info("mtx_dvfs_gov, %s: *freq:%lu", __func__, *freq);
...
static struct attribute_group dev_attr_group = {
    .name = "mtx_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "mtx_coeff", .attrs = coeff_entries,
};
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("MTX_DVFS_OPS");
