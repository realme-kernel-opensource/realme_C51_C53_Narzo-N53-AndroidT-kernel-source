...
static const struct of_device_id dcam_axi_dvfs_of_match[] = {
    {.compatible = "sprd,hwdvfs-dcam-axi"}, {},
};
MODULE_DEVICE_TABLE(of, dcam_axi_dvfs_of_match);

static struct platform_driver dcam_axi_dvfs_driver = {
    .probe = dvfs_probe,
    .remove = dvfs_remove,
    .driver =
        {
            .name = "dcam_axi-dvfs", .of_match_table = dcam_axi_dvfs_of_match,
        },
};
...
static int dvfs_gov_get_target(struct devfreq *devfreq,
    ...
    if (module->dvfs_enable) {
        ...
        *freq = adjusted_freq;
        ...
        pr_err("dcam_axi_dvfs_gov, %s: dvfs unenable", __func__);

    pr_info("dcam_axi_dvfs_gov, %s: *freq:%lu", __func__, *freq);
...
static struct attribute_group dev_attr_group = {
    .name = "dcam-axi_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "dcam-axi_coeff", .attrs = coeff_entries,
};

struct devfreq_governor dcam_axi_dvfs_gov = {
    .name = "dcam_axi_dvfs",
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("AXI_DVFS_OPS");
...
int dcam_axi_dvfs_init(void) {
    int ret;
    ret = register_ip_dvfs_ops(&dcam_axi_dvfs_ops);
    if (ret) {
        pr_err("%s: failed to add ops: %d\n", __func__, ret);
        return ret;
    }

    ret = devfreq_add_governor(&dcam_axi_dvfs_gov);
    if (ret) {
        pr_err("%s: failed to add governor: %d\n", __func__, ret);
        return ret;
    }
    ret = platform_driver_register(&dcam_axi_dvfs_driver);
    if (ret)
        devfreq_remove_governor(&dcam_axi_dvfs_gov);
    return ret;
}

void dcam_axi_dvfs_exit(void) {
    int ret;
    platform_driver_unregister(&dcam_axi_dvfs_driver);

    ret = devfreq_remove_governor(&dcam_axi_dvfs_gov);
    if (ret)
        pr_err("%s: failed to remove governor: %d\n", __func__, ret);

    ret = unregister_ip_dvfs_ops(&dcam_axi_dvfs_ops);
    if (ret)
