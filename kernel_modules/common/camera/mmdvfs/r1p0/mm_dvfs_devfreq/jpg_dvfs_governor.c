...
static struct platform_driver jpg_dvfs_driver = {
    ...
    .driver =
        {
            .name = "jpg-dvfs", .of_match_table = jpg_dvfs_of_match,
        },
};
...
static int dvfs_gov_get_target(struct devfreq *devfreq,
    ...
    if (module->dvfs_enable) {
        ...
        *freq = adjusted_freq;
        ...
        pr_err("jpg_dvfs_gov, %s: dvfs unenable", __func__);

    pr_info("jpg_dvfs_gov, %s: *freq:%lu", __func__, *freq);
...
static int dvfs_probe(struct platform_device *pdev) {
    ...
    module->devfreq =
        devm_devfreq_add_device(dev, &dvfs_profile, "jpg_dvfs", NULL);
...
static struct attribute_group dev_attr_group = {
    .name = "jpg_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "jpg_coeff", .attrs = coeff_entries,
};
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("JPG_DVFS_OPS");
