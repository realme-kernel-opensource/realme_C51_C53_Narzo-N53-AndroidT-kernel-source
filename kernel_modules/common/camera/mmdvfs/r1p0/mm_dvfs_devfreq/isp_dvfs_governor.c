...
static struct platform_driver isp_dvfs_driver = {
    ...
    .driver =
        {
            .name = "isp-dvfs", .of_match_table = isp_dvfs_of_match,
        },
};
...
static int dvfs_probe(struct platform_device *pdev) {
    ...
    module->devfreq =
        devm_devfreq_add_device(dev, &dvfs_profile, "isp_dvfs", NULL);
...
static struct attribute_group dev_attr_group = {
    .name = "isp_governor", .attrs = dev_entries,
};

static struct attribute_group coeff_attr_group = {
    .name = "isp_coeff", .attrs = coeff_entries,
};
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("ISP_DVFS_OPS");
