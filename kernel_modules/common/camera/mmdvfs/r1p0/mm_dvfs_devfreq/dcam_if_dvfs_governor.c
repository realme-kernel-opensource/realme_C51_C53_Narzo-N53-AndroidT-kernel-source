...
static struct platform_driver dcam_if_dvfs_driver = {
    .probe = dvfs_probe,
    .remove = dvfs_remove,
    .driver =
        {
...
static int dvfs_gov_get_target(struct devfreq *devfreq,
    ...
    if (module->dvfs_enable) {
        ...
        *freq = adjusted_freq;
        ...
        pr_err("dcam_dvfs_gov, %s: dvfs unenable", __func__);

    if(*freq < 307200000) {
        *freq = 307200000;
        pr_info("dcam_dvfs_gov, %s: dcam clk too low, change freq to %lu", __func__, *freq);
    }
    pr_info("dcam_dvfs_gov, %s: *freq:%lu", __func__, *freq);
...
struct devfreq_governor dcam_if_dvfs_gov = {
    .name = "dcam_if_dvfs",
...
static int userspace_init(struct devfreq *devfreq) {
    ...
    module->dvfs_ops = get_ip_dvfs_ops("DCAM_IF_DVFS_OPS");
...
int dcam_if_dvfs_init(void) {
    int ret;
    ret = register_ip_dvfs_ops(&dcam_if_dvfs_ops);
    if (ret) {
        pr_err("%s: failed to add ops: %d\n", __func__, ret);
        return ret;
    }

    ret = devfreq_add_governor(&dcam_if_dvfs_gov);
    if (ret) {
        pr_err("%s: failed to add governor: %d\n", __func__, ret);
        return ret;
    }
    ret = platform_driver_register(&dcam_if_dvfs_driver);
    if (ret)
        devfreq_remove_governor(&dcam_if_dvfs_gov);
    return ret;
}

void dcam_if_dvfs_exit(void) {
    int ret;
    platform_driver_unregister(&dcam_if_dvfs_driver);

    ret = devfreq_remove_governor(&dcam_if_dvfs_gov);
    if (ret)
        pr_err("%s: failed to remove governor: %d\n", __func__, ret);

    ret = unregister_ip_dvfs_ops(&dcam_if_dvfs_ops);
    if (ret)
