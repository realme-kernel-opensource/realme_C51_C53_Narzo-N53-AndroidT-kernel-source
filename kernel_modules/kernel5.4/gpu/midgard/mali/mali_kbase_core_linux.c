...
int gpu_voltage_cur = 0;
module_param(gpu_voltage_cur, int, S_IRUSR | S_IRGRP | S_IROTH); /* r-r-r-- */
MODULE_PARM_DESC(gpu_voltage_cur, "GPU gpu_voltage_cur");
...
int gpu_power_alawys_on = 0;
module_param(gpu_power_alawys_on, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH); /* rw-rw-r-- */
MODULE_PARM_DESC(gpu_power_alawys_on, "GPU power always on");

#ifdef CONFIG_MALI_HOTPLUG
int gpu_support_hotplug = 1;
module_param(gpu_support_hotplug, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH); /* rw-rw-r-- */
MODULE_PARM_DESC(gpu_support_hotplug, "GPU support hotplug");
#endif
...
/**
 * set_dvfs_curstate - Store callback for the dvfs current state.
 * @dev:   The device with sysfs file is for
 * @attr:  The attributes of the sysfs file
 * @buf:   The value written to the sysfs file
 * @count: The number of bytes written to the sysfs file
 *
 * This function is called when the dvfs current state is written to.
 *
 * Return: @ret if the function succeeded. An error code on failure.
 */
static ssize_t set_dvfs_curstate(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
 ...
 * Return: NA.
 */
static ssize_t show_dvfs_curstate(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret;
...
static DEVICE_ATTR(dvfs_curstate, S_IRUGO | S_IWUSR, show_dvfs_curstate,
		set_dvfs_curstate);
#endif
/**
