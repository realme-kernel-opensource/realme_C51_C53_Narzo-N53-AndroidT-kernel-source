...
bool is_gpu_powered_down(struct kbase_device *kbdev);
...
void kbasep_js_kctx_term(struct kbase_context *kctx)
{
	...
	WARN_ON(kbase_ctx_flag(kctx, KCTX_SCHEDULED));
	WARN_ON(js_kctx_info->ctx.nr_jobs != 0);
						...
						int js)
{
	...
	for (i = KBASE_JS_ATOM_SCHED_PRIO_FIRST; i < KBASE_JS_ATOM_SCHED_PRIO_COUNT; i++) {
		...
		if (atomic_read(&kctx->const_exe_count[js]) >= 5 &&
...
void kbase_js_sched(struct kbase_device *kbdev, int js_mask)
{
	...
	if (kbdev->pm.backend.callback_power_on_first_part) {
		kbdev->pm.backend.callback_power_on_first_part(kbdev);
		power_flag = 1;
	}
	...
	while (js_mask) {
		...
		while (1) {
			...
			if (!kbase_ctx_flag(kctx, KCTX_ACTIVE)) {
				...
				if (kbase_pm_context_active_handle_suspend_without_dcdc_enable(
									kbdev,
			...
			if(power_flag) {
				if (kbdev->pm.backend.callback_power_on_clear_flag)
					kbdev->pm.backend.callback_power_on_clear_flag(kbdev);
				power_flag = 0;
			}
