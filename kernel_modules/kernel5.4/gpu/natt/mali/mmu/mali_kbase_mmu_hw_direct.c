		...
		unsigned int as_nr)
{
	...
	if (WARN_ON_ONCE(max_loops == 0)) {
		dev_err(kbdev->dev,
			...
			as_nr);
		/*if ((kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_STATUS)) & 0x1) &&
			...
			!(kbase_reg_read(kbdev, JOB_SLOT_REG(0, JS_STATUS))) &&
			!(kbase_reg_read(kbdev, JOB_SLOT_REG(1, JS_STATUS))) &&
			!(kbase_reg_read(kbdev, JOB_SLOT_REG(2, JS_STATUS))) &&
			(kbase_reg_read(kbdev, GPU_CONTROL_REG(MMU_IRQ_STATUS)) & 0xff0000)) {
				dev_err(kbdev->dev, "gpu & job reg is ok, but mmu has bus error\n");
#ifdef SPRD_SUPPORT_FAULT_KEYWORD
				time[GPU_MMU_BUS_ERROR] = seconds_right_now();
#endif
				return -2;
		}*/
		return -1;
