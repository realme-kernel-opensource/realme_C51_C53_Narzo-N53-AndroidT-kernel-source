...
static void kbase_fence_debug_wait_timeout(struct kbase_jd_atom *katom)
{
	...
	mutex_lock(&katom->kctx->kbdev->pm.lock);
	if(katom->kctx->kbdev->pm.backend.gpu_powered)
	{
		kbase_debug_dump_registers(katom->kctx->kbdev);
	}
	mutex_unlock(&katom->kctx->kbdev->pm.lock);
