...
/*******************************************************************************
 *
 * Function         send_bt_a2dp_status
 *
 * Description      notify wcn bsp of current bluetooth a2dp status.
 *
 * Returns          void
 *
 ******************************************************************************/
static void send_bt_a2dp_status(const unsigned char *buf)
{
    if((buf[1] == 0x57) && (buf[2] == 0xfc) && (buf[3] == 0x03)){
        if(buf[6] == 0xff){
            sprdwcn_bus_pm_qos_set(BT_A2DP,1);
            dev_unisoc_bt_info(ttyBT_dev,"sprdwcn_bus_pm_qos_set BT_A2DP = 1.\n");
        }else if(buf[6] == 0x00){
            sprdwcn_bus_pm_qos_set(BT_A2DP,0);
            dev_unisoc_bt_info(ttyBT_dev,"sprdwcn_bus_pm_qos_set BT_A2DP = 0.\n");
        }
    }
}
...
/*******************************************************************************
 *
 * Function         send_bt_opp_status
 *
 * Description      notify wcn bsp of current bluetooth opp status.
 *
 * Returns          void
 *
 ******************************************************************************/
static void send_bt_opp_status(const unsigned char *buf)
{
    if((buf[1] == 0x59) && (buf[2] == 0xfc) && (buf[3] == 0x09) &&((buf[10] == 0x01) || (buf[10] == 0x02))){
        if(buf[12] == 0x01){
            sprdwcn_bus_pm_qos_set(BT_OPP,1);
            dev_unisoc_bt_info(ttyBT_dev,"sprdwcn_bus_pm_qos_set BT_OPP = 1.\n");
        }else if(buf[12] == 0x00){
            sprdwcn_bus_pm_qos_set(BT_OPP,0);
            dev_unisoc_bt_info(ttyBT_dev,"sprdwcn_bus_pm_qos_set BT_OPP = 0.\n");
        }
    }
}
			...
			const unsigned char *buf, int count)
{
	...
	if (log_level == MTTY_LOG_LEVEL_VER) {
		...
		send_bt_a2dp_status(buf);
		send_bt_opp_status(buf);
