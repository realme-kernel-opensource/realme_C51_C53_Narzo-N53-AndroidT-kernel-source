			...
			struct sprd_qos_t *qos, int num, int channel)
{
	...
	while ((msg = sc2332_qos_peek_msg(qos, &pkts))) {
		...
		if (num-- == 0) {
			/*msg still on qos->list[x].head when sblock is not enough*/
			spin_lock_bh(&qos->txlist->busylock);
			tmp_msg = qos->list[msg->index].head;
			qos->list[msg->index].head = msg;
			msg->next = tmp_msg;
		...
		if (!ret) {
			...
			dev_err(&hif->pdev->dev,
				"%s mode %d, ret %d.\n", __func__, msg->mode, ret);
			/* original logic: msg still on qos->list[x].head when sent failed */
			spin_lock_bh(&qos->txlist->busylock);
			tmp_msg = qos->list[msg->index].head;
			qos->list[msg->index].head = msg;
			msg->next = tmp_msg;
...
int sc2332_sipc_probe(struct platform_device *pdev)
{
