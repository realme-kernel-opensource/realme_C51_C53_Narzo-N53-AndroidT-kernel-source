...
void sc2355_sipc_throughput_ctl_core_pd(struct sprdwl_priv *priv, unsigned int len)
{
	g_tp_config.tx_bytes += len;
	if (time_after(jiffies, g_tp_config.last_time +  msecs_to_jiffies(1000))){ 
		spin_lock_bh(&g_tp_config.lock);
		g_tp_config.last_time = jiffies;
		if (g_tp_config.tx_bytes >= DISABLE_PD_THRESHOLD) {
			if (!g_tp_config.disable_pd_flag)	{
				g_tp_config.disable_pd_flag = true;
...
static netdev_tx_t sprdwl_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	skb_len = skb->len;
	sc2355_sipc_throughput_ctl_core_pd(vif->priv, skb_len);	
#if !defined(SC2355_FTR)
...
#ifdef ACS_SUPPORT
void sipc_clean_survey_info_list(struct sprdwl_vif *vif)
{
...
void sipc_transfer_survey_info(struct sprdwl_vif *vif)
{
