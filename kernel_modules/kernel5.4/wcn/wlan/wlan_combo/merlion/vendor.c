					    ...
					    const void *data, int len)
{
	...
	if (nla_put_u32(reply_radio, SPRDWL_LL_STATS_TYPE,
			SPRDWL_NL80211_VENDOR_SUBCMD_LL_STATS_GET_TYPE_RADIO))
		goto out_put_fail;
	...
	if (nla_put_u32(reply_iface, SPRDWL_LL_STATS_TYPE,
			SPRDWL_NL80211_VENDOR_SUBCMD_LL_STATS_GET_TYPE_IFACE))
		goto out_put_fail;
...
static int sprdwl_vendor_monitor_rssi(struct wiphy *wiphy,
		...
		const void *data, int len)
{
	struct nlattr *tb[SPRDWL_ATTR_RSSI_MONITORING_MAX + 1];
	uint32_t control;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
	if (nla_parse(tb, SPRDWL_ATTR_RSSI_MONITORING_MAX, data, len, rssi_monitoring_policy, NULL)) {
#else
	if (nla_parse(tb, SPRDWL_ATTR_RSSI_MONITORING_MAX, data, len, rssi_monitoring_policy)) {
#endif
		wl_err("Invalid ATTR\n");
		return -EINVAL;
	}

	if (!tb[SPRDWL_ATTR_RSSI_MONITORING_REQUEST_ID]) {
		wl_err("attr request id failed\n");
		return -EINVAL;
	}

	if (!tb[SPRDWL_ATTR_RSSI_MONITORING_CONTROL]) {
		wl_err("attr control failed\n");
		return -EINVAL;
	}

	req.request_id = nla_get_u32(tb[SPRDWL_ATTR_RSSI_MONITORING_REQUEST_ID]);
	control = nla_get_u32(tb[SPRDWL_ATTR_RSSI_MONITORING_CONTROL]);

	if (control == WLAN_RSSI_MONITORING_START) {
		req.control = true;
		if (!tb[SPRDWL_ATTR_RSSI_MONITORING_MIN_RSSI]) {
			wl_err("get min rssi fail\n");
			return -EINVAL;
		}

		if (!tb[SPRDWL_ATTR_RSSI_MONITORING_MAX_RSSI]) {
			wl_err("get max rssi fail\n");
			return -EINVAL;
		}

		req.min_rssi = nla_get_s8(tb[SPRDWL_ATTR_RSSI_MONITORING_MIN_RSSI]);
		req.max_rssi = nla_get_s8(tb[SPRDWL_ATTR_RSSI_MONITORING_MAX_RSSI]);
				       ...
				       const void *data, int len)
{
	...
	nla_for_each_attr(pos, (void *)data, len, rem_len) {
		...
		case SPRD_WLAN_VENDOR_ATTR_EPNO_MIN5GHZ_RSSI:
			epno_params.min5ghz_rssi = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_EPNO_MIN24GHZ_RSSI:
			epno_params.min24ghz_rssi = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_EPNO_INITIAL_SCORE_MAX:
			epno_params.initial_score_max = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_EPNO_CURRENT_CONNECTION_BONUS:
			epno_params.current_connection_bonus = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_EPNO_SAME_NETWORK_BONUS:
			epno_params.same_network_bonus = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_EPNO_SECURE_BONUS:
			epno_params.secure_bonus = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_EPNO_BAND5GHZ_BONUS:
			epno_params.band5ghz_bonus = nla_get_u32(pos);
			break;

		case SPRD_WLAN_VENDOR_ATTR_PNO_SET_LIST_PARAM_NUM_NETWORKS:
		...
		case SPRD_WLAN_VENDOR_ATTR_PNO_SET_LIST_PARAM_EPNO_NETWORKS_LIST:
			i = 0;
			nla_for_each_nested(outer_iter, pos, rem_outer_len) {
				...
				nla_for_each_nested(inner_iter, outer_iter,
					...
					type = nla_type(inner_iter);
					switch (type) {
					case SPRD_WLAN_VENDOR_ATTR_PNO_SET_LIST_PARAM_EPNO_NETWORK_SSID:
						memcpy(epno_network->ssid,
						       nla_data(inner_iter),
						       IEEE80211_MAX_SSID_LEN);
						break;

					case SPRD_WLAN_VENDOR_ATTR_PNO_SET_LIST_PARAM_EPNO_NETWORK_FLAGS:
						epno_network->flags =
						    nla_get_u8(inner_iter);
						break;

					case SPRD_WLAN_VENDOR_ATTR_PNO_SET_LIST_PARAM_EPNO_NETWORK_AUTH_BIT:
...
int sprdwl_report_acs_lte_event(struct sprdwl_vif *vif)
{
    ...
    payload = 4;
    reply = cfg80211_vendor_event_alloc(wiphy, &vif->wdev, payload,
							SPRDWL_ACS_LTE_EVENT_INDEX, GFP_KERNEL);
    if (!reply) {
...
const struct wiphy_vendor_command sprdwl_vendor_cmd[] = {
	...
	{/*14*/
		...
		.policy = sprdwl_ll_stats_policy,
		.maxattr = SPRDWL_LL_STATS_SET_MAX,
	},
	{/*15*/
		{
		    .vendor_id = OUI_SPREAD,
		...
		.maxattr = SPRDWL_NL80211_VENDOR_SUBCMD_LL_STATS_TYPE_MAX,
	},
	{/*16*/
		{
		    .vendor_id = OUI_SPREAD,
		...
		.maxattr = SPRDWL_LL_STATS_CLR_MAX,
	},
	{/*20*/
		{
		    .vendor_id = OUI_SPREAD,
		...
		.maxattr = GSCAN_ATTR_SUBCMD_CONFIG_PARAM_MAX,
	},
	{/*21*/
		{
		    .vendor_id = OUI_SPREAD,
	...
	{/*39*/
		...
		.maxattr = WLAN_VENDOR_ATTR_SET_SCANNING_MAC_OUI_MAX,
	},
	{/*42*/
		{
			.vendor_id = OUI_SPREAD,
		...
		.maxattr = SPRDWL_ATTR_CO_MATRIX_MAX,
	},
	{/*55*/
		{
		    .vendor_id = OUI_SPREAD,
	...
	{/*62*/
		...
		.maxattr = SPRDWL_ATTR_WIFI_LOGGER_START_GET_MAX,
	},
	{/*63*/
		{
		    .vendor_id = OUI_SPREAD,
	...
	{/*69*/
		...
		.maxattr = SPRD_WLAN_VENDOR_ATTR_PNO_MAX,
	},
	{/*70*/
		{
		    .vendor_id = OUI_SPREAD,
	...
	{/*76 */
		...
		.maxattr = SPRDWL_ATTR_LOGGER_MAX,
	},
	{/*77*/
		{
		    .vendor_id = OUI_SPREAD,
	...
	{/*80*/
		...
		.policy = rssi_monitoring_policy,
		.maxattr = SPRDWL_ATTR_RSSI_MONITORING_MAX,
	},
	{/*82*/
		{
		    .vendor_id = OUI_SPREAD,
		...
		.maxattr = SPRDWL_ATTR_ND_OFFLOAD_MAX,
	},
	{/*85 */
		{
		    .vendor_id = OUI_SPREAD,
		...
		.maxattr = WLAN_GET_WAKE_STATS_MAX,
	},
	{/*146 */
		{
		    .vendor_id = OUI_SPREAD,
		...
		.maxattr = WLAN_ATTR_SAR_LIMITS_MAX,
	},
...
static const struct nl80211_vendor_cmd_info sprdwl_vendor_events[] = {
	...
	[SPRDWL_VENDOR_EVENT_EPNO_FOUND_INDEX] = {
		.vendor_id = OUI_SPREAD,
		.subcmd = SPRDWL_VENDOR_SUBCMD_PNO_NETWORK_FOUND,
	},
