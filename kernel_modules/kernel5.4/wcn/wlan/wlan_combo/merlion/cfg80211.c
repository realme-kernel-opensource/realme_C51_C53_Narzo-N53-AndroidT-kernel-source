				    ...
				    struct cfg80211_ap_settings *settings)
{
	...
	if ((vif->mode == SPRDWL_MODE_AP) &&
	    ...
	    fw_ver == 1) {
		sipc_clean_survey_info_list(vif);
	}
#endif
...
void sprdwl_scan_timeout(struct timer_list *t)
{
	...
	if (priv->scan_request) {
		...
		drv_ver = (api + WIFI_CMD_SCAN)->drv_version;
			fw_ver = min(fw_ver, drv_ver);
		if (fw_ver == 1)
			sipc_clean_survey_info_list(priv->scan_vif);
#endif /* ACS_SUPPORT */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
		cfg80211_scan_done(priv->scan_request, &info);
#else
				...
				struct wireless_dev *wdev)
{
	...
	struct api_version_t *api = NULL;//(&priv->sync_api)->api_array;
	u8 fw_ver = 0, drv_ver = 0;
	struct sprdwl_intf *intf;
...
#ifdef WMMAC_WFA_CERTIFICATION
	wmm_params = (struct wmm_params_element *)sipc_get_wmm_ie(conn_info->resp_ie,
