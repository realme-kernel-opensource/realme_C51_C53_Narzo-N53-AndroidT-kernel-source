...
struct throughput_sta g_tp_config;
			    ...
			    unsigned long tx_start_time)
{
	...
	intf->stats.tx_cost_time += timespec64_to_ns(&tx_end) - tx_start_time;
#else
...
void sprdwl_throughput_performance(unsigned long len)
{ 
        g_tp_config.rx_bytes += len;
	if (time_after(jiffies, g_tp_config.rx_last_time +  msecs_to_jiffies(1000))) {
		spin_lock(&g_tp_config.lock);
		g_tp_config.rx_last_time = jiffies;
		if (g_tp_config.rx_bytes >= DISABLE_PD_THRESHOLD) {
			if (!g_tp_config.rx_disable_pd_flag) {
				g_tp_config.rx_disable_pd_flag = true;
...
void sprdwl_count_rx_tp(struct sprdwl_intf *intf, int len)
{
	...
	sprdwl_throughput_performance(len);
