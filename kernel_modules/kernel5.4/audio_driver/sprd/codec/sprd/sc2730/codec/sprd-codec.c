...
static int sprd_codec_spk_pga_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol);
static void sprd_codec_sdm_init(struct snd_soc_component *codec);

static int sprd_codec_get_ctrl(struct snd_soc_component *codec, char *name);
static void sprd_codec_psg_state_init(struct snd_soc_component *codec);
...
static void codec_digital_reg_restore(struct snd_soc_component *codec)
{
...
static struct snd_kcontrol *sprd_codec_find_ctrl(struct snd_soc_component *codec,
						 char *name)
{
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	snd_soc_component_update_bits(codec, mc->reg,
		mask << mc->shift,
		val << mc->shift);
...
static int sprd_codec_spk_pga_update(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
...
static void update_switch(struct snd_soc_component *codec, u32 path, u32 on)
{
	u32 val = 0;
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC9), path, val);
}

static inline void sprd_codec_vcm_v_sel(struct snd_soc_component *codec, int v_sel)
{
...
static int sprd_das_dc_os_en(struct snd_soc_component *codec, int on)
{
	int mask, val;

	mask = DAS_OS_EN;
	val = on ? mask : 0;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC6), mask, val);

	return 0;
}

static void sprd_das_dc_os_set(struct snd_soc_component *codec)
{
	int mask, val;
...
static void load_ocp_pfw_cfg(struct sprd_codec_priv *sprd_codec)
{
	...
	for (i = 0;
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU22), 0xFFFF, val);

		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU23),
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC17),
			PA_RSV(0xFFFF), 0);
		usleep_range(100, 110);
		break;
	case SND_SOC_DAPM_POST_PMU:
		usleep_range(100, 110);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC19),
...
static void sprd_codec_pa_boost(struct snd_soc_component *codec, int pa_d_en)
{
	int mask;
	int value;
	int i = 0, state = 0;
	...
	value = 0x3FA1;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_WR_PROT0), mask, value);
	/*
	...
	if (pa_d_en) {
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL1), mask, mask);
		mask = CLK_PABST_EN | CLK_PABST_32K_EN;
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CLK0), mask, mask);
		mask = PA_VCM_BYP_SEL;
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC15), mask, 0);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU6),
			PABST_V_INIT(0xffff), 0);

		/* set vcal */
		mask = PABST_V_CAL(0xffff);
		value = PABST_V_CAL(sprd_codec->aud_pabst_vcal >> 10);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU4), mask, value);
		...
		sprd_codec_wait(10);
		while (i++ < 10) {
			state = snd_soc_component_read32(codec, SOC_REG(ANA_STS8)) &
				PABST_SOFT_DVLD;
			if (state)
				break;
			sprd_codec_wait(1);
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU20), mask, 0);
		mask = CLK_PABST_EN | CLK_PABST_32K_EN;
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CLK0), mask, 0);
		mask = DIG_CLK_PABST_EN;
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL1), mask, 0);
	}
}

/* inter PA */
static inline void sprd_codec_pa_d_en(struct snd_soc_component *codec, int on,
				      int force)
{
	int mask;
	int val;
	...
	val = on ? mask : 0;
	if (force)
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC14), mask, val);
	if (on)
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL2), FGU_DIG_EN,
			    on ? FGU_DIG_EN : 0);
}

static inline void sprd_codec_pa_emi_rate(struct snd_soc_component *codec, int rate)
{
	int mask;
	int val;
	...
	val = PA_EMI_L(rate);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC16), mask, val);
}

static inline void sprd_codec_pa_dtri_f_sel(struct snd_soc_component *codec,
					    int f_sel)
{
	int mask;
	int val;
...
static inline void sprd_codec_pa_en(struct snd_soc_component *codec, int on)
{
	int mask;
	int val;
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
...
static void spk_pa_short_check(struct sprd_codec_priv *sprd_codec)
{
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC14), PA_EN, 0);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC20),
		PA_SHORTH_DET_EN, PA_SHORTH_DET_EN);
	sprd_codec_wait(1);
	flag = snd_soc_component_read32(codec, SOC_REG(ANA_CDC14)) | PA_SHT_FLAG(0xFFFF);
	if (flag) {
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL1),
		DCL_EN, DCL_EN);
	mask = ANA_CLK_EN | CLK_DIG_6M5_EN | CLK_DCL_6M5_EN | CLK_DCL_32K_EN;
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CLK0), mask, mask);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL1),
		DIG_CLK_IMPD_EN, DIG_CLK_IMPD_EN);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_CLK0),
		CLK_IMPD_EN, CLK_IMPD_EN);


	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0),
		PGA_ADC_IBIAS_EN, PGA_ADC_IBIAS_EN);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU0),
		VB_EN, VB_EN);

	snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC20),
		PA_IMPD_DET_EN, PA_IMPD_DET_EN);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL0),
		RSTN_AUD_DIG_IMPD_ADC, RSTN_AUD_DIG_IMPD_ADC);

	snd_soc_component_update_bits(codec, SOC_REG(ANA_IMPD0),
		IMPD_ADC_EN, IMPD_ADC_EN);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_IMPD1),
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_IMPD2),
		mask, val);
	sprd_codec_wait(60);

	val = snd_soc_component_read32(codec, SOC_REG(ANA_STS14)) | IMPD_ADC_DATO(0xFFFF);

	/* yintang: TBD. calculate the impd per the val */
#endif
}

static void spk_pa_config(struct snd_soc_component *codec,
			 struct inter_pa setting)
{
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	if (on) {
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC14),
			PA_DPOP_BYP_EN, 0);
		sprd_codec_wait(1);
		...
		sprd_codec_wait(60);
		while (i++ < 135) {
			state = snd_soc_component_read32(codec, SOC_REG(ANA_STS11)) &
					     PA_AB_DPOP_DVLD;
			if (state)
				break;
			sprd_codec_wait(1);
...
static void sprd_codec_sdm_ramp(struct snd_soc_component *codec, bool on)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	int val = 0;
	...
	if (on) {
		snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_L), 0xffff, 0);
		snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_H), 0xffff, 0);

		while (val <= SDM_RAMP_MAX) {
			snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_L),
					0xffff, val);
			val += 0x20;
		...
		val = snd_soc_component_read32(codec, SOC_REG(AUD_DAC_SDM_L));
		while (val > 0x20) {
			val -= 0x20;
			snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_L),
					0xffff, val);
...
static void sprd_dalr_dc_os_set(struct snd_soc_component *codec)
{
	...
	if (NEED_SDM_RAMP(sprd_codec, codec)) {
		val = DALR_OS(0);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC5), mask, val);
...
static int sprd_codec_set_ad_sample_rate(struct snd_soc_component *codec,
					 u32 rate,
					 int mask, int shift)
{
	int set;
...
static void sprd_codec_sdm_init(struct snd_soc_component *codec)
{
	...
	if (NEED_SDM_RAMP(sprd_codec, codec)) {
		...
		snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_L),
			0xffff, 0x9999);
		snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_SDM_H), 0xff, 0x1);
	}
}

static void sprd_codec_power_disable(struct snd_soc_component *codec)
{
...
static void sprd_codec_power_enable(struct snd_soc_component *codec)
{
...
static int sprd_codec_digital_open(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	int ret = 0;
	...
	snd_soc_component_update_bits(codec, SOC_REG(AUD_DAC_CTL), BIT(DAC_MUTE_EN), 0);
...
static void sprd_codec_irq_oxp_enable(struct snd_soc_component *codec)
{
	unsigned int val, irq_bits;

	/* Enable the audo protect in digital part. */
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_PRT_CFG_0),
	...
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_CLR), val, val);
	val = irq_bits << AUD_A_INT_EN;
	snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_INT_MODULE_CTRL), val, val);
}
#endif

static void codec_digital_reg_enable(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
...
static void codec_digital_reg_disable(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL9), msk, 0);
		val = RCV_DCCAL_SEL(1);
		msk = RCV_DCCAL_SEL(0xFFFF);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL7), msk, val);

		val = RSTN_AUD_DCL_32K | RSTN_AUD_DIG_DRV_SOFT |
			RSTN_AUD_HPDPOP | RSTN_AUD_DIG_DCL;
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL0), val, val);
...
static void sprd_codec_set_fgu_high_thrd(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);

	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL3), FGU_HIGH_THRD(0xFFFF),
			    FGU_HIGH_THRD(sprd_codec->fgu.high_thrd));
}

static void sprd_codec_set_fgu_low_thrd(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);

	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL4), FGU_LOW_THRD(0xFFFF),
			    FGU_LOW_THRD(sprd_codec->fgu.low_thrd));
}

static unsigned int sprd_codec_get_fgu_vbatt(struct snd_soc_component *codec)
{
	u32 vbatt;

	vbatt = snd_soc_component_read32(codec, SOC_REG(ANA_STS7));

	return FGU_V_DAT(vbatt);
}

static void sprd_codec_set_fgu_v_unit(struct snd_soc_component *codec)
{
	u32 fgu_4p2, v_unit;
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL2), FGU_V_UNIT(0xffff),
			    FGU_V_UNIT(v_unit));
}

static void sprd_codec_psg_state_transf(struct snd_soc_component *codec,
					unsigned int state)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	struct fgu *fgu = &sprd_codec->fgu;
...
static void sprd_codec_psg_process(struct snd_soc_component *codec, int hi_lo)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	int state = -1;
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	if (on) {
		...
		ret = snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL6),
			0xFF, val);
			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	if (on) {
		...
		ret = snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL5),
			0xFF, val);

		val = HPRCV_OPA_IBIAS_MODE_SEL | HPRCV_DCCALI_IBIAS_MODE_SEL;
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC13), val, val);
	}
	return ret;
}
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	ADEBUG();
	if (on)
		ret = snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC11),
			0xFF, 0x5650);
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADL_CLK_RST,
			ADL_CLK_RST);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADL_CLK_RST, 0);
		/*
		 * ADL_RST should keep be 0-1-0,
		 * and 1 state should keep 20 cycle of 6.5M clock at least.
		 */
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADL_RST, 0);
		udelay(4);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADL_RST, ADL_RST);
		udelay(4);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADL_RST, 0);
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADR_CLK_RST, 0);

		/*
		 * ADL_RST should keep be 0-1-0,
		 * and 1 state should keep 20 cycle of 6.5M clock at least.
		 */
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADR_RST, 0);
		udelay(4);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADR_RST, ADR_RST);
		udelay(4);
		break;
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC0), ADR_RST, 0);
...
static inline int adie_audio_codec_adie_loop_clk_en(
		struct snd_soc_component *codec, int on)
{
	int ret = 0;

	if (on)
		ret = snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_CLK_EN),
		     ...
		     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_component_update_bits(codec, SOC_REG(AUD_CFGA_LP_MODULE_CTRL),
		     ...
		     struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(AUD_LOOP_CTL),
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_component_update_bits(codec, SOC_REG(ANA_CDC13),
				    HPRCV_OPA_IBIAS_MODE_SEL |
				    HPRCV_DCCALI_IBIAS_MODE_SEL, 0);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL10),
			CP_AD_CMP_CAL_AVG(0xFFFF), CP_AD_CMP_CAL_AVG(1));
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL10),
			CP_AD_CMP_AUTO_CAL_EN, CP_AD_CMP_AUTO_CAL_EN);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL11),
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits(codec, SOC_REG(ANA_DCL14),
		...
		usleep_range(100, 110);
		snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU7),
...
static void cp_short_check(struct sprd_codec_priv *sprd_codec)
{
	...
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU7),
		mask1, mask1);
	snd_soc_component_update_bits(codec, SOC_REG(ANA_PMU8),
		mask2, 0);
...
static SOC_ENUM_SINGLE_VIRT_DECL(hpl_rcv_enum, hpl_rcv_mux_texts);
...
#define SPRD_CODEC_MUX_KCTL(xname, xkctl, xreg, xshift, xtext) \
	static SOC_ENUM_SINGLE_DECL(xkctl##_enum, xreg, xshift, xtext); \
...
static int sprd_codec_da_mode_switch(struct snd_soc_component *codec, bool on)
{
	struct snd_kcontrol *kctrl;
	struct snd_ctl_elem_value ucontrol = {
...
static irqreturn_t sprd_codec_dp_irq(int irq, void *dev_id)
{
	...
	if (BIT(DAC_MUTE_U_MASK) & mask)
		mask = BIT(DAC_MUTE_U);
	snd_soc_component_update_bits(codec, SOC_REG(AUD_INT_EN), mask, 0);
			  ...
			  struct snd_info_buffer *buffer)
{
	...
	aud_glb_reg = devm_kzalloc(card->dev,
...
free:
	devm_kfree(card->dev, aud_glb_reg);
}
...
static int sprd_codec_soc_probe(struct snd_soc_component *codec)
{
	struct sprd_codec_priv *sprd_codec = snd_soc_component_get_drvdata(codec);
	int ret;
