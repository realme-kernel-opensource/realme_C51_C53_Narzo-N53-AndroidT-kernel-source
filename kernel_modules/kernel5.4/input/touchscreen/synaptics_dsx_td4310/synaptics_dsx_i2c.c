				...
				unsigned short length)
{
	...
	retval = synaptics_rmi4_set_page(rmi4_data, addr);
	if (retval != PAGE_SELECT_LEN) {
		dev_err(&client->dev, "%s: set page fail, retval = %d\n",
			__func__, retval);
		retval = -EIO;
		goto exit;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	/* byte0 indicate gesture type, byte1~byte4 are gesture parameter */
	unsigned char detected_gestures[F12_GESTURE_DETECTION_LEN];
	unsigned short data_addr;
	int x;
	int y;
	int wx;
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		/*
		 * here is demo only, customer could decode gesture data and do
		 * whatever they want
		 */
		if (detected_gestures[0]
				&& (detected_gestures[0] != F12_UDG_DETECT)) {
			input_report_key(rmi4_data->input_dev, KEY_POWER, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_POWER, 0);
			input_sync(rmi4_data->input_dev);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%s\n", rmi4_data->sensor_sleep ? "suspend" : "resume");;
}
...
{
	struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data *)p;
		...
		struct synaptics_dsx_platform_data *board)
{
	...
	board->reset_on_state = 0;
	board->irq_flags = IRQF_ONESHOT | IRQF_TRIGGER_LOW;

	return 0;
}

static int synaptics_rmi4_gpio_request(
		struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	const struct synaptics_dsx_platform_data *board;

	if (!rmi4_data || !rmi4_data->board)
		return -ENODEV;

	board = rmi4_data->board;
	retval = gpio_request(board->irq_gpio, "synaptics-int");
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to request int(%d) gpio\n",
				__func__, board->irq_gpio);
		return -EIO;
	}

	retval = gpio_direction_input(board->irq_gpio);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to set int gpio to input\n",
				__func__);
		goto err_then_free_int;
	}

	retval = gpio_request(board->reset_gpio, "synaptics-rst");
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to request rst(%d) gpio\n",
				__func__, board->reset_gpio);
		goto err_then_free_int;
	}

	retval = gpio_direction_output(board->reset_gpio,
		!board->reset_on_state);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to set rst gpio to output\n",
				__func__);
		goto err_then_free_rst;
	}

	return retval;

err_then_free_rst:
	gpio_free(board->reset_gpio);

err_then_free_int:
	gpio_free(board->irq_gpio);

	return retval;
}

static void synaptics_rmi4_gpio_free(
		struct synaptics_rmi4_data *rmi4_data)
{
	if (rmi4_data && rmi4_data->board) {
		...
		struct synaptics_rmi4_data *rmi4_data)
{
	...
	gpio_set_value(board->reset_gpio, !board->reset_on_state);
	msleep(board->reset_delay_ms);
	gpio_set_value(board->reset_gpio, board->reset_on_state);
	msleep(board->reset_active_ms);
	gpio_set_value(board->reset_gpio, !board->reset_on_state);
	msleep(board->reset_delay_ms);
		...
		const struct i2c_device_id *dev_id)
{
	...
	retval = synaptics_rmi4_parse_dt(&client->dev, board);
	if (retval < 0) {
		dev_err(&client->dev,
	...
	retval = synaptics_rmi4_gpio_request(rmi4_data);
	if (retval < 0) {
		dev_err(&client->dev,
	...
	retval = synaptics_rmi4_power_on(rmi4_data);
	if (retval < 0) {
		dev_err(&client->dev,
				"%s: Failed to power on device\n",
				__func__);
		goto err_power_on_device;
	}
	...
	rmi4_data->irq = gpio_to_irq(board->irq_gpio);
	retval = request_threaded_irq(rmi4_data->irq, NULL,
		synaptics_rmi4_irq_handler, board->irq_flags,
		"synaptics-ts", rmi4_data);
	if (retval < 0) {
		dev_err(&client->dev,
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	dev_err(&rmi4_data->i2c_client->dev,"%s\n", __func__);
