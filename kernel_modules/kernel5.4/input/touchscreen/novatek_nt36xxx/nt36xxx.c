...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ret1 = of_property_read_u32(np, "novatek,touch_max_width", &ts->abs_x_max);
	if (ret1) {
		NVT_LOG("touch_max_width not found in dts,using default %d\n",
                       TOUCH_DEFAULT_MAX_WIDTH);
	} else {
		NVT_LOG("touch_max_width found in dts, abs_x_max=%d\n",
                        ts->abs_x_max);
	}

	ret1 = of_property_read_u32(np, "novatek,touch_max_height", &ts->abs_y_max);
        if (ret1) {
        	NVT_LOG("touch_max_height not found in dts, using default %d\n",
			TOUCH_DEFAULT_MAX_HEIGHT);
        } else {
		NVT_LOG("touch_max_height found in dts, abs_y_max=%d\n",
                        ts->abs_y_max);
	}
