...
#include "ats_core.h"
static char *lcd_names_support[] = {
	"lcd_nt35532_truly_mipi_fhd",
	"lcd_nt35695_truly_mipi_fhd",
	"lcd_ili9881c_3lane_mipi_fhd",
	"lcd_ili9881c_truly_mipi_hd",
	"lcd_r61350_truly_mipi_hd",
...
static void ts_suspend_cali_autotest(struct device *dev);
 ...
 * poll worker
 */
static void ts_poll_handler(struct timer_list *t)
{
	struct ts_data *pdata = from_timer(pdata, t, poll_timer);
...
int ts_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (is_in_tui()) {
		dev_info(dev, "waiting for tui to exit first!\n");
		notify_cancel_tui();
		...
		if (tp_i2c_safaMode) {
			dev_info(dev, "in TUI, cannot suspend!\n");
			return 0;
		}
	}
#endif
	...
	dev_info(dev, "[%s] [%d] [%d]\n", __func__,
					ts_get_mode(pdata, TSMODE_CONTROLLER_EXIST),
...
int ts_resume(struct platform_device *pdev)
{
	...
	if (ts_get_mode(pdata, TSMODE_CONTROLLER_EXIST)
		...
		&& pdata->controller->handle_event){
			...
			TSEVENT_RESUME, NULL);
			if (ret != TSRESULT_EVENT_HANDLED) {
				dev_info(dev, "resume is not handled\n");
				return -ENODEV;
			}
		}
	...
	if (pdata->controller) {
		...
		if (retval < 0) {
			...
			if (ts_get_mode(pdata, TSMODE_AUTO_UPGRADE_FW)) {
				if (strncmp(current_mode, "cali", strlen("cali"))) {
					ts_request_firmware_upgrade(pdata, NULL, true);
				} else {
					dev_info(dev, "ignored fw upgrade(1) by cali mode.\n");
					ts_set_mode(pdata, TSMODE_CONTROLLER_STATUS, true);
				}
			}
		} else if (ts_get_mode(pdata, TSMODE_AUTO_UPGRADE_FW)) {
			if (strncmp(current_mode, "cali", strlen("cali")) &&
						strncmp(current_mode, "autotest", strlen("autotest")))
							ts_request_firmware_upgrade(pdata, NULL, false);
			else {
				dev_info(dev, "ignored fw upgrade(2) by cali/bbat mode.\n");
				ts_set_mode(pdata, TSMODE_CONTROLLER_STATUS, true);
			}
		} else {
			ts_set_mode(pdata, TSMODE_CONTROLLER_STATUS, true);
