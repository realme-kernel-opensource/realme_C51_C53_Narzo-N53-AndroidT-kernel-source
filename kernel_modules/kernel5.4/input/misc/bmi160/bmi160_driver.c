...
#define DRIVER_VERSION "0.0.38.2"
...
static int bmi_input_init(struct bmi_client_data *client_data)
{
	...
	dev->name = ACC_SENSOR_INPUT_NAME;
...
static int bmi_gyro_input_init(struct bmi_client_data *client_data)
{
	...
	if (NULL == dev)
		return -ENOMEM;
	dev->name = GYRO_SENSOR_INPUT_NAME;
	dev->id.bustype = BUS_I2C;
	input_set_capability(dev, EV_ABS, ABS_MISC);
...
static ssize_t bmi160_chip_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "bmi160");
}
...
static ssize_t bmi160_acc_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
	
	return snprintf(buf, 16, "%d\n", atomic_read(&client_data->accel_en));
}

static int bmi160_accel_set_enable(
	struct bmi_client_data *client_data, bool enable);

static ssize_t bmi160_acc_enable_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
	...
	err = kstrtobool(buf, &acc_enable);
	if (err)
		return err;
	if ((acc_enable == 0) || (acc_enable == 1))
		err = bmi160_accel_set_enable(client_data, acc_enable);
	if (err)
		return -EINVAL;
...
static ssize_t bmi160_acc_delay_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);
...
static ssize_t bmi160_acc_delay_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int err;
	unsigned long acc_delay;
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);

	err = kstrtoul(buf, 10, &acc_delay);
	if (err)
		return err;

	err = bmi160_accel_set_poll_delay(client_data, acc_delay);
	if (err)
		return -EINVAL;
		...
		const char *buf, size_t count)
{
	...
	err = kstrtobool(buf, &gyro_enable);
	if (err)
		return err;
	if ((gyro_enable == 0) || (gyro_enable == 1))
		err = bmi160_gyro_set_enable(client_data, gyro_enable);
	if (err)
		return -EINVAL;
...
{
	int err;
	unsigned long gyro_delay;
	struct input_dev *input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(input);

	err = kstrtoul(buf, 10, &gyro_delay);
	if (err)
		return err;

	err = bmi160_gyro_set_poll_delay(client_data, gyro_delay);
	if (err)
		return -EINVAL;
...
static DEVICE_ATTR(fifo_flush, 0220,
		NULL, bmi160_fifo_flush_store);
static DEVICE_ATTR(fifo_bytecount, 0660,
		bmi160_fifo_bytecount_show, bmi160_fifo_bytecount_store);
static DEVICE_ATTR(fifo_data_sel, 0660,
...
static DEVICE_ATTR(mag_compensate, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
...
#if defined(BMI160_ENABLE_INT1) || defined(BMI160_ENABLE_INT2)
static DEVICE_ATTR(enable_int, 0220,
		NULL, bmi_enable_int_store);
static DEVICE_ATTR(anymot_duration, 0660,
		bmi160_anymot_duration_show, bmi160_anymot_duration_store);
static DEVICE_ATTR(anymot_threshold, 0660,
...
int bmi_probe(struct bmi_client_data *client_data, struct device *dev)
{
	...
	if (err < 0)
		goto exit_err_sysfs;
	kobject_uevent(&client_data->input->dev.kobj, KOBJ_ADD);

	/* gyro input device init */
	err = bmi_gyro_input_init(client_data);
	if (err < 0)
		goto exit_err_gyro_input_register;
	err = sysfs_create_group(&client_data->gyro_input->dev.kobj,
			&bmi160_gyro_attribute_group);
	if (err < 0)
		goto exit_err_gyro_sysfs;
	kobject_uevent(&client_data->gyro_input->dev.kobj, KOBJ_ADD);
...
#if defined(BMI160_ENABLE_INT1) || defined(BMI160_ENABLE_INT2)
		client_data->gpio_pin = of_get_gpio(dev->of_node, 0);
	...
	BMI_CALL_API(set_accel_range)(BMI160_ACCEL_RANGE_4G);
	/*defalut odr 200HZ*/
	BMI_CALL_API(set_accel_output_data_rate)(BMI160_ACCEL_OUTPUT_DATA_RATE_200HZ);
	/* set accel bandwidth to OSR4 mode*/
	BMI_CALL_API(set_accel_bw)(BMI160_GYRO_OSR4_MODE);

	/* set accel range to 2,000 degrees per second*/
	BMI_CALL_API(set_gyro_range)(BMI160_GYRO_RANGE_2000_DEG_SEC);
	/*defalut odr 200HZ*/
	BMI_CALL_API(set_gyro_output_data_rate)(BMI160_GYRO_OUTPUT_DATA_RATE_200HZ);
	...
	INIT_DELAYED_WORK(&client_data->gyro_poll_work,
		bmi160_gyro_work_fn);
...
exit_err_gyro_sysfs:
	input_unregister_device(client_data->gyro_input);

exit_err_gyro_input_register:
